...
### 1. Nhận `DELETE /api/v1/admin/discussions/{discussion_id}` từ actor `Admin` trong AC-37.
...
- Source: `AC-37` / ERD / API Index theo nội dung bước.
### 4. Thực hiện đọc/ghi trên entity được ERD hỗ trợ: `discussions`, `users`, `courses`. Exact SELECT/INSERT/UPDATE/DELETE condition chỉ được chốt khi AC khóa đủ; phần chưa có evidence được đánh `TBD`.
...
|---|---|---|---|
| `discussions` | DELETE / exact operation TBD | ERD-backed entity | Exact predicate/mutation only where AC explicitly fixes it |
