...
### 1. Nhận `GET /api/v1/users/me/achievements` từ actor `Student` trong AC-18.

- Source: `AC-18` / ERD / API Index theo nội dung bước.
### 2. Kiểm tra authentication/authorization phù hợp precondition của AC-18; HTTP/error code cụ thể để `TBD` nếu AC không chỉ rõ.

- Source: `AC-18` / ERD / API Index theo nội dung bước.
### 3. Thực hiện đọc/ghi trên entity được ERD hỗ trợ: `courses`, `enrollments`, `lessons`, `lesson_progress`, `quiz_attempts`, `assignment_submissions`. Exact SELECT/INSERT/UPDATE/DELETE condition chỉ được chốt khi AC khóa đủ; phần chưa có evidence được đánh `TBD`.
