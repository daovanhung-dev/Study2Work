...
### 1. Nhận `PUT /api/v1/admin/users/{user_id}` từ actor `Admin` trong AC-31.

- Source: `AC-31` / ERD / API Index theo nội dung bước.
### 2. Kiểm tra authentication/authorization phù hợp precondition của AC-31; HTTP/error code cụ thể để `TBD` nếu AC không chỉ rõ.

- Source: `AC-31` / ERD / API Index theo nội dung bước.
### 3. Đọc path parameter: `user_id`; type/range validation chưa được diagram khóa thì để `TBD`.

- Source: `AC-31` / ERD / API Index theo nội dung bước.
### 4. Request body field-level chưa đủ nguồn: không suy diễn; đánh dấu `TBD / SOURCE_REQUIRED`.

- Source: `AC-31` / ERD / API Index theo nội dung bước.
### 5. Thực hiện đọc/ghi trên entity được ERD hỗ trợ: `users`. Exact SELECT/INSERT/UPDATE/DELETE condition chỉ được chốt khi AC khóa đủ; phần chưa có evidence được đánh `TBD`.
...
|---|---|---|---|
| `users` | PUT / exact operation TBD | ERD-backed entity | Exact predicate/mutation only where AC explicitly fixes it |
