...
### 1. Nhận `GET /api/v1/mentor/courses` từ actor `Mentor` trong AC-21.

- Source: `AC-21` / ERD / API Index theo nội dung bước.
### 2. Kiểm tra authentication/authorization phù hợp precondition của AC-21; HTTP/error code cụ thể để `TBD` nếu AC không chỉ rõ.

- Source: `AC-21` / ERD / API Index theo nội dung bước.
### 3. Thực hiện đọc/ghi trên entity được ERD hỗ trợ: `courses`, `users`. Exact SELECT/INSERT/UPDATE/DELETE condition chỉ được chốt khi AC khóa đủ; phần chưa có evidence được đánh `TBD`.
