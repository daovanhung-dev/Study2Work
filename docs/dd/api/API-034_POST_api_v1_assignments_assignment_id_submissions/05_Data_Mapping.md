...
### 1. Nhận `POST /api/v1/assignments/{assignment_id}/submissions` từ actor `Student` trong AC-17.
...
- Source: `AC-17` / ERD / API Index theo nội dung bước.
### 3. Đọc path parameter: `assignment_id`; type/range validation chưa được diagram khóa thì để `TBD`.

- Source: `AC-17` / ERD / API Index theo nội dung bước.
### 4. Request body field-level chưa đủ nguồn: không suy diễn; đánh dấu `TBD / SOURCE_REQUIRED`.

- Source: `AC-17` / ERD / API Index theo nội dung bước.
### 5. Thực hiện đọc/ghi trên entity được ERD hỗ trợ: `assignments`, `assignment_submissions`, `users`. Exact SELECT/INSERT/UPDATE/DELETE condition chỉ được chốt khi AC khóa đủ; phần chưa có evidence được đánh `TBD`.
