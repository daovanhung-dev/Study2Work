...
### 1. Nhận `POST /api/v1/payments` từ actor `Student` trong AC-12.
...
- Source: `AC-12` / ERD / API Index theo nội dung bước.
### 4. Thực hiện đọc/ghi trên entity được ERD hỗ trợ: `payments`, `users`, `courses`. Exact SELECT/INSERT/UPDATE/DELETE condition chỉ được chốt khi AC khóa đủ; phần chưa có evidence được đánh `TBD`.
...
|---|---|---|---|
| `payments` | POST / exact operation TBD | ERD-backed entity | Exact predicate/mutation only where AC explicitly fixes it |
