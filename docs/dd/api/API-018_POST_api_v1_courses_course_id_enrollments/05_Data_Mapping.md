...
### 1. Nhận `POST /api/v1/courses/{course_id}/enrollments` từ actor `Student` trong AC-12.

- Source: `AC-12` / ERD / API Index theo nội dung bước.
### 2. Kiểm tra authentication/authorization phù hợp precondition của AC-12; HTTP/error code cụ thể để `TBD` nếu AC không chỉ rõ.
...
- Source: `AC-12` / ERD / API Index theo nội dung bước.
### 5. Thực hiện đọc/ghi trên entity được ERD hỗ trợ: `enrollments`, `courses`, `users`. Exact SELECT/INSERT/UPDATE/DELETE condition chỉ được chốt khi AC khóa đủ; phần chưa có evidence được đánh `TBD`.
...
|---|---|---|---|
| `enrollments` | POST / exact operation TBD | ERD-backed entity | Exact predicate/mutation only where AC explicitly fixes it |
