...
### 1. Nhận `GET /api/v1/courses/{course_id}` từ actor `Guest` trong AC-05.
...
- Source: `AC-05` / ERD / API Index theo nội dung bước.
### 3. Read course and mentor summary.

- Source: `AC-05` / ERD / API Index theo nội dung bước.
### 4. Require course to exist and be PUBLISHED for guest flow.

- Source: `AC-05` / ERD / API Index theo nội dung bước.
### 5. Tạo response chỉ từ output đã được nguồn xác nhận; field chưa xác nhận giữ `TBD` và không tự bổ sung.

- Source: `AC-05` / ERD / API Index theo nội dung bước.
### 6. Nếu gặp nhánh lỗi explicit, kết thúc theo error mapping tại `06_Error.md`.
