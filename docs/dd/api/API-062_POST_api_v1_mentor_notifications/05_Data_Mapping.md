...
### 1. Nhận `POST /api/v1/mentor/notifications` từ actor `Mentor` trong AC-25.

- Source: `AC-25` / ERD / API Index theo nội dung bước.
### 2. Kiểm tra authentication/authorization phù hợp precondition của AC-25; HTTP/error code cụ thể để `TBD` nếu AC không chỉ rõ.
...
|---|---|---|---|
| `notifications` | POST / exact operation TBD | ERD-backed entity | Exact predicate/mutation only where AC explicitly fixes it |
