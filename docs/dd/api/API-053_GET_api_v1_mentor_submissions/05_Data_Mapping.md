...
### 1. Nhận `GET /api/v1/mentor/submissions` từ actor `Mentor` trong AC-23.
...
- Source: `AC-23` / ERD / API Index theo nội dung bước.
### 3. Thực hiện đọc/ghi trên entity được ERD hỗ trợ: `assignment_submissions`, `assignments`, `users`. Exact SELECT/INSERT/UPDATE/DELETE condition chỉ được chốt khi AC khóa đủ; phần chưa có evidence được đánh `TBD`.
