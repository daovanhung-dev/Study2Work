...
### 1. Nhận `GET /api/v1/courses/{course_id}/enrollment-status` từ actor `Student` trong AC-12.
...
- Source: `AC-12` / ERD / API Index theo nội dung bước.
### 4. Thực hiện đọc/ghi trên entity được ERD hỗ trợ: `enrollments`, `courses`, `users`. Exact SELECT/INSERT/UPDATE/DELETE condition chỉ được chốt khi AC khóa đủ; phần chưa có evidence được đánh `TBD`.
