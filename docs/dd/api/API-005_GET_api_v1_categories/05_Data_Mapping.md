...
### 1. Nhận `GET /api/v1/categories` từ actor `Guest` trong AC-03.

- Source: `AC-03` / ERD / API Index theo nội dung bước.
### 2. Nguồn dữ liệu/persistence không có table hợp lệ trong ERD V1: `SOURCE_REQUIRED`; không tạo SQL giả.

- Source: `AC-03` / ERD / API Index theo nội dung bước.
### 3. Tạo response chỉ từ output đã được nguồn xác nhận; field chưa xác nhận giữ `TBD` và không tự bổ sung.
