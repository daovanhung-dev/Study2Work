...
### 1. Nhận `PATCH /api/v1/notifications/{notification_id}/read` từ actor `Student` trong AC-20.

- Source: `AC-20` / ERD / API Index theo nội dung bước.
### 2. Kiểm tra authentication/authorization phù hợp precondition của AC-20; HTTP/error code cụ thể để `TBD` nếu AC không chỉ rõ.

- Source: `AC-20` / ERD / API Index theo nội dung bước.
### 3. Đọc path parameter: `notification_id`; type/range validation chưa được diagram khóa thì để `TBD`.

- Source: `AC-20` / ERD / API Index theo nội dung bước.
### 4. Request body field-level chưa đủ nguồn: không suy diễn; đánh dấu `TBD / SOURCE_REQUIRED`.

- Source: `AC-20` / ERD / API Index theo nội dung bước.
### 5. Thực hiện đọc/ghi trên entity được ERD hỗ trợ: `notifications`, `users`. Exact SELECT/INSERT/UPDATE/DELETE condition chỉ được chốt khi AC khóa đủ; phần chưa có evidence được đánh `TBD`.
...
|---|---|---|---|
| `notifications` | PATCH / exact operation TBD | ERD-backed entity | Exact predicate/mutation only where AC explicitly fixes it |
