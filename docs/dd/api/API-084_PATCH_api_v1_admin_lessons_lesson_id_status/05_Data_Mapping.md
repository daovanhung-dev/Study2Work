...
### 1. Nhận `PATCH /api/v1/admin/lessons/{lesson_id}/status` từ actor `Admin` trong AC-34.
...
- Source: `AC-34` / ERD / API Index theo nội dung bước.
### 5. Thực hiện đọc/ghi trên entity được ERD hỗ trợ: `lessons`, `courses`, `lesson_progress`, `enrollments`, `users`. Exact SELECT/INSERT/UPDATE/DELETE condition chỉ được chốt khi AC khóa đủ; phần chưa có evidence được đánh `TBD`.
...
|---|---|---|---|
| `lessons` | PATCH / exact operation TBD | ERD-backed entity | Exact predicate/mutation only where AC explicitly fixes it |
| `courses` | PATCH / exact operation TBD | ERD-backed entity | Exact predicate/mutation only where AC explicitly fixes it |
| `lesson_progress` | PATCH / exact operation TBD | ERD-backed entity | Exact predicate/mutation only where AC explicitly fixes it |
| `enrollments` | PATCH / exact operation TBD | ERD-backed entity | Exact predicate/mutation only where AC explicitly fixes it |
