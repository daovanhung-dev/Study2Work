...
### 1. Nhận `DELETE /api/v1/admin/lessons/{lesson_id}` từ actor `Admin` trong AC-34.
...
- Source: `AC-34` / ERD / API Index theo nội dung bước.
### 4. Thực hiện đọc/ghi trên entity được ERD hỗ trợ: `lessons`, `courses`. Exact SELECT/INSERT/UPDATE/DELETE condition chỉ được chốt khi AC khóa đủ; phần chưa có evidence được đánh `TBD`.
...
|---|---|---|---|
| `lessons` | DELETE / exact operation TBD | ERD-backed entity | Exact predicate/mutation only where AC explicitly fixes it |
