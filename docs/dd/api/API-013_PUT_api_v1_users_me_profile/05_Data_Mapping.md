...
### 1. Nhận `PUT /api/v1/users/me/profile` từ actor `Student` trong AC-11.

- Source: `AC-11` / ERD / API Index theo nội dung bước.
### 2. Kiểm tra authentication/authorization phù hợp precondition của AC-11; HTTP/error code cụ thể để `TBD` nếu AC không chỉ rõ.

- Source: `AC-11` / ERD / API Index theo nội dung bước.
### 3. Request body field-level chưa đủ nguồn: không suy diễn; đánh dấu `TBD / SOURCE_REQUIRED`.

- Source: `AC-11` / ERD / API Index theo nội dung bước.
### 4. Thực hiện đọc/ghi trên entity được ERD hỗ trợ: `users`. Exact SELECT/INSERT/UPDATE/DELETE condition chỉ được chốt khi AC khóa đủ; phần chưa có evidence được đánh `TBD`.
