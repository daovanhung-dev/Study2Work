# PLAN 36 — Work — Job pause, resume và close
...
|---|---|
| Plan ID | `PLAN-36` |
| API trong phạm vi | `API-WRK-048`, `API-WRK-049`, `API-WRK-050` |
| Số API | 3 |
...
Tạo DD cho lifecycle sau publish.

**Luồng nghiệp vụ trọng tâm:** Pause ẩn public ngay bằng DB predicate; resume recheck expiry/policy/entitlement; close terminal nhưng giữ application/ATS/chat history.

**Điểm cần khóa:** API-WRK-050 thiếu lỗi/màn hình trực tiếp; terminal no-reopen và state machine canonical.

**Dependency:** Plans 22,34–35.

## 3. Phạm vi

**In-scope**

- `API-WRK-048` — `POST /api/v1/enterprises/{enterpriseId}/jobs/{jobId}/pause`.
- `API-WRK-049` — `POST /api/v1/enterprises/{enterpriseId}/jobs/{jobId}/resume`.
- `API-WRK-050` — `POST /api/v1/enterprises/{enterpriseId}/jobs/{jobId}/close`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L309`.
- **Actor/quyền:** `PERM-WRK-013`.
- **Input/validation → output:** Reason, If-Match → PAUSED.
- **Xử lý và dữ liệu:** TX valid`PUBLISHED→PAUSED`; remove public/index async but API predicate hides immediately.
- **Vận hành:** Idempotency; audit/event.
- **Lỗi đặc thù:** `INVALID_JOB_TRANSITION`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L310`.
- **Actor/quyền:** `PERM-WRK-013`.
- **Input/validation → output:** If-Match → PUBLISHED.
- **Xử lý và dữ liệu:** Recheck expiry/policy/entitlement/current revision under lock.
- **Vận hành:** Idempotency; audit/index.
- **Lỗi đặc thù:** `JOB_EXPIRED`, `JOB_POLICY_FAILED`.
- **Màn hình/consumer:** `SCR-WRK-033`.
- **Capability/use case:** `CAP-WRK-004`, `UC-WRK-004`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L311`.
- **Actor/quyền:** `PERM-WRK-013`.
- **Input/validation → output:** Reason `FILLED                                                           | CANCELLED                                                                                                                                                                                                       | OTHER`, If-Match → CLOSED.
- **Xử lý và dữ liệu:** TX terminal transition; no new application; existing ATS/chat retained, chat becomes read-only only when each application terminal.
...
- **Màn hình/consumer:** `SCR-WRK-033` — DERIVED từ 05_DAC_TA_MAN_HINH.md:L136/L251.
- **Capability/use case:** `CAP-WRK-004`, `UC-WRK-004`.
