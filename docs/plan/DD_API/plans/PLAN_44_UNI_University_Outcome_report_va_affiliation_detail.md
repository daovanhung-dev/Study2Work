# PLAN 44 — University — Outcome report và affiliation detail
...
|---|---|
| Plan ID | `PLAN-44` |
| API trong phạm vi | `API-UNI-015`, `API-UNI-016` |
| Số API | 2 |
| Read-only / Mutation | 2 / 0 |
...
Tạo DD cho báo cáo tổng hợp privacy-safe và xem affiliation theo scope.

**Luồng nghiệp vụ trọng tâm:** Report chạy theo cohort/program/time, ngưỡng nhóm >=10 và không row-level PII; affiliation detail tenant predicate + consent state.

**Điểm cần khóa:** K-anonymity threshold, async snapshot/expiry, consent redaction.

**Dependency:** Plans 42–43.

## 3. Phạm vi

**In-scope**

- `API-UNI-015` — `GET /api/v1/universities/{universityId}/reports/outcomes`.
- `API-UNI-016` — `GET /api/v1/universities/{universityId}/affiliations/{id}`.
...
- Screens: `SCR-UNI-004`, `SCR-UNI-005`, `SCR-UNI-011`.
- Acceptance: `TC-UNI-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L359`.
- **Actor/quyền:** `PERM-UNI-030`.
- **Input/validation → output:** Date/cohort/program/dimensions → aggregate metrics.
- **Xử lý và dữ liệu:** Query daily metrics/consented anonymous facts by tenant; suppress group`<10`; no raw application/chat/evidence.
- **Vận hành:** Private; query audit; export async.
- **Lỗi đặc thù:** `PRIVACY_THRESHOLD_NOT_MET`, `DATE_RANGE_TOO_LARGE`.
- **Màn hình/consumer:** `SCR-UNI-011`.
- **Capability/use case:** `CAP-UNI-003`, `UC-UNI-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L360`.
- **Actor/quyền:** `PERM-UNI-031` + specific active consent.
- **Input/validation → output:** → allowlisted learner status for approved purpose.
- **Xử lý và dữ liệu:** Tenant/affiliation + consent purpose/expiry predicate; field minimization; never Study evidence/CV/chat.
- **Vận hành:** Every access privacy-audited; no-store.
- **Lỗi đặc thù:** `CONSENT_REQUIRED`, `AFFILIATION_NOT_FOUND`.
