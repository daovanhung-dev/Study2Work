# PLAN 12 — Study — Assessment definition và draft
...
|---|---|
| Plan ID | `PLAN-12` |
| API trong phạm vi | `API-STU-023`, `API-STU-024`, `API-STU-025`, `API-STU-026` |
| Số API | 4 |
...
Tạo DD cho lấy assessment, đọc/lưu/xóa draft trước khi submit.

**Luồng nghiệp vụ trọng tâm:** Assessment read theo placement/version; draft upsert theo learner+assessment, validate schema/file references; delete chỉ draft chưa seal.

**Điểm cần khóa:** Draft SEALED không sửa, không cấp attemptNo, If-Match/idempotency nếu source yêu cầu.

**Dependency:** Plan 13 attempts; Plan 14 file pipeline.

## 3. Phạm vi

**In-scope**

- `API-STU-023` — `GET /api/v1/assessments/{assessmentId}`.
- `API-STU-024` — `GET /api/v1/assessments/{assessmentId}/draft`.
- `API-STU-025` — `PUT /api/v1/assessments/{assessmentId}/draft`.
- `API-STU-026` — `DELETE /api/v1/assessments/{assessmentId}/draft`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L153`.
- **Actor/quyền:** Learner with scope access.
- **Input/validation → output:** → type, instructions, limits, rubric; quiz excludes correctness.
- **Xử lý và dữ liệu:** Authorize exact placement scope/enrollment; join one and only one placement.
- **Vận hành:** Private.
- **Lỗi đặc thù:** `ASSESSMENT_NOT_ACCESSIBLE`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L154`.
- **Actor/quyền:** Learner owner.
- **Input/validation → output:** → draft/version or null.
- **Xử lý và dữ liệu:** Unique learner+assessment; sensitive answer no-store.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L155`.
- **Actor/quyền:** Learner owner.
- **Input/validation → output:** Type-specific answer,`If-Match`; text <=20k; link HTTPS <=2048; file ID clean/owned; quiz option IDs valid → draft.
- **Xử lý và dữ liệu:** TX L draft; validate assessment version and type; file may be attached only when CLEAN; upsert version.
- **Vận hành:** Idempotency optional; audit metadata, not answer content.
- **Lỗi đặc thù:** `DRAFT_TYPE_MISMATCH`, `LINK_INVALID`, `FILE_NOT_CLEAN`, `VERSION_CONFLICT`.
- **Màn hình/consumer:** `SCR-STU-017`.
- **Capability/use case:** `CAP-STU-004`, `UC-STU-004`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L156`.
- **Actor/quyền:** Learner owner.
- **Input/validation → output:** `If-Match` → null.
- **Xử lý và dữ liệu:** TX soft-delete/unlink mutable draft; never delete sealed attempts/file audit.
- **Vận hành:** Idempotent; audit.
- **Lỗi đặc thù:** `VERSION_CONFLICT`, `DRAFT_ALREADY_SEALED`.
- **Màn hình/consumer:** `SCR-STU-017`.
- **Capability/use case:** `CAP-STU-004`, `UC-STU-004`.
