# PLAN 32 — Work — Enterprise tenant, workspace, profile và verification submission
...
|---|---|
| Plan ID | `PLAN-32` |
| API trong phạm vi | `API-WRK-035`, `API-WRK-036`, `API-WRK-037`, `API-WRK-038` |
| Số API | 4 |
...
Tạo DD cho tạo tenant, lấy workspace, sửa employer profile và nộp hồ sơ verification.

**Luồng nghiệp vụ trọng tâm:** Create owner membership; workspace aggregate tenant/permission/entitlement; profile If-Match; verification case mới chứa documents CLEAN và append-only decision history.

**Điểm cần khóa:** Tenant scoping, at least one owner, PII/document security, no cross-tenant lookup.
...
- `API-WRK-035` — `POST /api/v1/enterprises`.
- `API-WRK-036` — `GET /api/v1/enterprises/{enterpriseId}/workspace`.
- `API-WRK-037` — `PATCH /api/v1/enterprises/{enterpriseId}`.
- `API-WRK-038` — `POST /api/v1/enterprises/{enterpriseId}/verification-submissions`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L291`.
- **Actor/quyền:** Authenticated.
- **Input/validation → output:** Legal/public name, tax ID, contact, terms → tenant`PENDING_VERIFICATION` + owner membership.
- **Xử lý và dữ liệu:** TX normalize tax ID, prevent active duplicate, create tenant/member; owner cannot self-verify.
- **Vận hành:** Idempotency required; audit.
- **Lỗi đặc thù:** `ENTERPRISE_DUPLICATE`, `LEGAL_INFO_INVALID`.
- **Màn hình/consumer:** `SCR-WRK-030`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L292`.
- **Actor/quyền:** Active member.
- **Input/validation → output:** → tenant profile, verification, own permission, plan/limits.
- **Xử lý và dữ liệu:** Every query predicate tenant+membership active.
- **Vận hành:** Private no-store.
- **Lỗi đặc thù:** `TENANT_NOT_FOUND`.
- **Màn hình/consumer:** `SCR-WRK-031`.
- **Capability/use case:** `CAP-WRK-003`, `UC-WRK-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L293`.
- **Actor/quyền:** `PERM-WRK-001`.
- **Input/validation → output:** Public/contact fields, If-Match → profile.
- **Xử lý và dữ liệu:** TX tenant predicate; legal fields after verified require new verification case.
- **Vận hành:** Idempotency; audit before/after.
- **Lỗi đặc thù:** `VERSION_CONFLICT`, `REVERIFICATION_REQUIRED`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L294`.
- **Actor/quyền:** `PERM-WRK-002`.
- **Input/validation → output:** Legal fields, CLEAN private document IDs, consent → case`PENDING_REVIEW`.
- **Xử lý và dữ liệu:** TX validate owner/files; append verification snapshot; tenant remains`PENDING_VERIFICATION` until admin sets `VERIFIED` or `REJECTED`.
- **Vận hành:** Idempotency; reviewer queue/audit.
