# PLAN 39 — Work ATS — Status transition và offer
...
|---|---|
| Plan ID | `PLAN-39` |
| API trong phạm vi | `API-WRK-058`, `API-WRK-059` |
| Số API | 2 |
...
Tạo DD cho transition ATS và issue offer version.

**Luồng nghiệp vụ trọng tâm:** Transition khóa application/current version, enforce state machine/artifacts/permission, append history/outbox; offer append-only version và đồng bộ application state trong một TX.

**Điểm cần khóa:** AI principal không được transition, terminal guards, offer history immutable.

**Dependency:** Plan 38; interview Plan 41.

## 3. Phạm vi

**In-scope**

- `API-WRK-058` — `POST /api/v1/enterprises/{enterpriseId}/applications/{applicationId}/status-transitions`.
- `API-WRK-059` — `POST /api/v1/enterprises/{enterpriseId}/applications/{applicationId}/offers`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L326`.
- **Actor/quyền:** Assigned recruiter + transition permission.
- **Input/validation → output:** `fromVersion`, target status, reason, rubric/decision declaration → application.
- **Xử lý và dữ liệu:** TX L application; enforce state machine and required artifacts; append immutable history/decision; human actor mandatory for OFFERED/HIRED/REJECTED; AI cannot call.
- **Vận hành:** Idempotency; audit before/after; notifications; chat read-only on terminal.
- **Lỗi đặc thù:** `INVALID_APPLICATION_TRANSITION`, `VERSION_CONFLICT`, `HUMAN_DECISION_REQUIRED`, `REASON_REQUIRED`.
- **Màn hình/consumer:** `SCR-WRK-039`, `SCR-WRK-040`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L327`.
- **Actor/quyền:** `PERM-WRK-034` + assigned.
- **Input/validation → output:** Structured offer, expiry, clean document → OFFERED snapshot.
- **Xử lý và dữ liệu:** TX require current status exactly`INTERVIEWING`; append immutable offer and transition; compensation permission separated. Không được nhảy trực tiếp từ SHORTLISTED sang OFFERED.
- **Vận hành:** Idempotency; high-risk audit/notification.
- **Lỗi đặc thù:** `OFFER_INVALID`, `INVALID_APPLICATION_TRANSITION`.
