# PLAN 28 — Work — Application context và submit application
...
|---|---|
| Plan ID | `PLAN-28` |
| API trong phạm vi | `API-WRK-022`, `API-WRK-023` |
| Số API | 2 |
...
Tạo DD cho preflight apply và transaction tạo application/snapshots/evidence request.

**Luồng nghiệp vụ trọng tâm:** Context kiểm eligibility/current revision/default CV; submit advisory-lock candidate+job, snapshot immutable job/CV/profile/portfolio, tạo application/history/answers/consent/evidence PENDING/outbox, không gọi Study trong TX.

**Điểm cần khóa:** Cross-service outage không rollback apply, unique candidate/job, idempotency bắt buộc.

**Dependency:** Plans 21,57; Plan 39 ATS.

## 3. Phạm vi

**In-scope**

- `API-WRK-022` — `GET /api/v1/jobs/{jobId}/application-context`.
- `API-WRK-023` — `POST /api/v1/jobs/{jobId}/applications`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L260`.
- **Actor/quyền:** Candidate.
- **Input/validation → output:** → eligibility, questions, current revision, default CV, evidence integration state.
- **Xử lý và dữ liệu:** R published job revision, existing application unique, candidate/CV; no Study call.
- **Vận hành:** Private no-store.
- **Lỗi đặc thù:** `JOB_NOT_ACCEPTING_APPLICATIONS`, `APPLICATION_ALREADY_EXISTS`, `PROFILE_REQUIREMENT_FAILED`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L261`.
- **Actor/quyền:** Candidate.
- **Input/validation → output:** Job revision ID, published CV revision, cover letter <=5000, answers, selected Study evidence IDs+versions, consent text/policy, portfolio IDs → application.
- **Xử lý và dữ liệu:** TX advisory lock candidate+job; revalidate job; unique`(candidate,job)`; snapshot job/CV/profile/portfolio; store selected IDs, consent/request PENDING; create application SUBMITTED, ATS history and outbox. Never calls Study in TX/outage does not fail apply.
- **Vận hành:** Idempotency required; application/evidence-export/notification events; audit.
- **Lỗi đặc thù:** `JOB_REVISION_STALE`, `APPLICATION_ALREADY_EXISTS`, `CV_NOT_PUBLISHED`, `APPLICATION_ANSWER_INVALID`.
- **Màn hình/consumer:** `SCR-WRK-017`.
- **Capability/use case:** `CAP-WRK-005`, `UC-WRK-005`, `CAP-WRK-006`, `UC-WRK-006`.
