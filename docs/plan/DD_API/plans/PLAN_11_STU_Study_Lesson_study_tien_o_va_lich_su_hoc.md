# PLAN 11 — Study — Lesson study, tiến độ và lịch sử học
...
|---|---|
| Plan ID | `PLAN-11` |
| API trong phạm vi | `API-STU-019`, `API-STU-020`, `API-STU-021`, `API-STU-022` |
| Số API | 4 |
| Read-only / Mutation | 3 / 1 |
...
Tạo DD cho tải lesson, cập nhật progress fact, tổng hợp progress và learning history.

**Luồng nghiệp vụ trọng tâm:** Read lesson theo enrollment/version; progress mutation ghi fact monotonic/idempotent và outbox; summary/history đọc snapshot/fact đúng owner.

**Điểm cần khóa:** Monotonic progress, completion invariant, concurrent update và snapshot rebuild.

**Dependency:** Plan 10; Plan 58 progress rebuild.

## 3. Phạm vi

**In-scope**

- `API-STU-019` — `GET /api/v1/lessons/{lessonId}/study`.
- `API-STU-020` — `PATCH /api/v1/lessons/{lessonId}/progress`.
- `API-STU-021` — `GET /api/v1/me/progress/summary`.
- `API-STU-022` — `GET /api/v1/me/learning-history`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L149`.
- **Actor/quyền:** Learner enrolled in owning version.
- **Input/validation → output:** → sanitized blocks, resources, atomic progress and assessment gates.
- **Xử lý và dữ liệu:** Authorize via enrollment→pinned course version→lesson; resource URLs signed on demand.
- **Vận hành:** Private no-store.
- **Lỗi đặc thù:** `LESSON_NOT_ACCESSIBLE`, `RESOURCE_NOT_CLEAN`.
- **Màn hình/consumer:** `SCR-STU-016`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L150`.
- **Actor/quyền:** Enrolled learner.
- **Input/validation → output:** `If-Match`, block facts `{blockId,status,position?,completedAt?}` → lesson/course/path snapshots.
- **Xử lý và dữ liệu:** TX L enrollment/progress rows; validate block belongs pinned lesson; merge monotonic max; recompute rule from facts; upsert snapshots; never trust client percent.
- **Vận hành:** Idempotency recommended; outbox completion/activity; audit adjustment only.
- **Lỗi đặc thù:** `PROGRESS_REGRESSION`, `VERSION_CONFLICT`, `BLOCK_NOT_IN_LESSON`.
- **Màn hình/consumer:** `SCR-STU-016`.
- **Capability/use case:** `CAP-STU-003`, `UC-STU-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L151`.
- **Actor/quyền:** Learner.
- **Input/validation → output:** → active path, course cards, pending reviews, completion facts.
- **Xử lý và dữ liệu:** Indexed queries by user; percentages are rebuildable snapshots with`calculatedAt`.
- **Vận hành:** Private; 15s user cache invalidated by outbox.
- **Lỗi đặc thù:** —.
- **Màn hình/consumer:** `SCR-STU-010`, `SCR-STU-019`.
- **Capability/use case:** `CAP-STU-003`, `UC-STU-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L152`.
- **Actor/quyền:** Learner.
- **Input/validation → output:** Page/date/type → immutable activity timeline.
- **Xử lý và dữ liệu:** Query`(user_id,occurred_at desc,id)`; redact expired payload fields.
- **Vận hành:** Private.
- **Lỗi đặc thù:** `DATE_RANGE_TOO_LARGE`.
- **Màn hình/consumer:** `SCR-STU-019`.
- **Capability/use case:** `CAP-STU-003`, `UC-STU-003`.
