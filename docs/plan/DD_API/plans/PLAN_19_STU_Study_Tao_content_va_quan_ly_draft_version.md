# PLAN 19 — Study — Tạo content và quản lý draft version
...
|---|---|
| Plan ID | `PLAN-19` |
| API trong phạm vi | `API-STU-051`, `API-STU-052`, `API-STU-053`, `API-STU-054` |
| Số API | 4 |
...
Tạo DD cho list content admin, tạo stable entity, tạo draft version và sửa draft.

**Luồng nghiệp vụ trọng tâm:** Permission theo kind; create stable+first draft; new version copy từ current; patch draft theo schema/rights/sanitization và version++.

**Điểm cần khóa:** Kind allowlist, nested mapping, immutable published version, If-Match.

**Dependency:** Plan 21 publish.

## 3. Phạm vi

**In-scope**

- `API-STU-051` — `GET /api/v1/admin/content/{kind}`.
- `API-STU-052` — `POST /api/v1/admin/content/{kind}`.
- `API-STU-053` — `POST /api/v1/admin/content/{kind}/{id}/versions`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L191`.
- **Actor/quyền:** `PERM-STU-001`.
- **Input/validation → output:** Kind path/course; page/status/author/q → stable entities + versions.
- **Xử lý và dữ liệu:** Allowlist kind; FTS plus owner/status indexes.
- **Vận hành:** Private; 60/phút.
- **Lỗi đặc thù:** `CONTENT_KIND_INVALID`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L192`.
- **Actor/quyền:** `PERM-STU-001`.
- **Input/validation → output:** Stable metadata + initial draft → entity/version.
- **Xử lý và dữ liệu:** TX unique slug, create stable entity + DRAFT version + author assignment.
- **Vận hành:** Idempotency; audit.
- **Lỗi đặc thù:** `SLUG_CONFLICT`, `CONTENT_INVALID`.
- **Màn hình/consumer:** `SCR-OPS-004`, `SCR-OPS-005`.
- **Capability/use case:** `CAP-STU-006`, `UC-STU-006`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L193`.
- **Actor/quyền:** `PERM-STU-001`.
- **Input/validation → output:** Base published/draft ID, change summary → new DRAFT copy.
- **Xử lý và dữ liệu:** TX L stable entity; copy immutable structure into draft IDs; never edit published rows.
- **Vận hành:** Idempotency; audit.
- **Lỗi đặc thù:** `DRAFT_ALREADY_EXISTS`, `BASE_VERSION_INVALID`.
- **Màn hình/consumer:** `SCR-OPS-004`, `SCR-OPS-005`.
- **Capability/use case:** `CAP-STU-006`, `UC-STU-006`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L194`.
- **Actor/quyền:** Author or`PERM-STU-002`.
- **Input/validation → output:** Metadata/structure patch,`If-Match` → draft/version.
- **Xử lý và dữ liệu:** TX L version; require DRAFT; sanitize Markdown/HTML allowlist; validate exact assessment placement and pinned versions.
- **Vận hành:** Idempotency recommended; audit diff; no cache.
- **Lỗi đặc thù:** `CONTENT_VERSION_NOT_EDITABLE`, `VERSION_CONFLICT`, `PLACEMENT_SCOPE_INVALID`.
- **Màn hình/consumer:** `SCR-OPS-004`, `SCR-OPS-005`, `SCR-OPS-006`.
- **Capability/use case:** `CAP-STU-006`, `UC-STU-006`.
