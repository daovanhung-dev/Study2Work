# PLAN 26 — Work — Portfolio lifecycle
...
|---|---|
| Plan ID | `PLAN-26` |
| API trong phạm vi | `API-WRK-014`, `API-WRK-015`, `API-WRK-016`, `API-WRK-017` |
| Số API | 4 |
...
Tạo DD cho list/create/update/soft-delete portfolio item.

**Luồng nghiệp vụ trọng tâm:** Item LINK hoặc FILE đúng một payload; URL HTTPS không fetch; file phải owner+CLEAN; update If-Match; delete chỉ ẩn presentation, không sửa application snapshot.

**Điểm cần khóa:** Exactly-one payload, visibility, file scan và snapshot immutability.

**Dependency:** Plan 14/Work file model.

## 3. Phạm vi

**In-scope**

- `API-WRK-014` — `GET /api/v1/me/portfolio-items`.
- `API-WRK-015` — `POST /api/v1/me/portfolio-items`.
- `API-WRK-016` — `PATCH /api/v1/me/portfolio-items/{id}`.
- `API-WRK-017` — `DELETE /api/v1/me/portfolio-items/{id}`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L247`.
- **Actor/quyền:** Candidate.
- **Input/validation → output:** Page/type → items.
- **Xử lý và dữ liệu:** R by owner/order.
- **Vận hành:** Private.
- **Lỗi đặc thù:** —.
- **Màn hình/consumer:** `SCR-WRK-014`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L248`.
- **Actor/quyền:** Candidate.
- **Input/validation → output:** Type LINK/FILE, title, HTTPS URL or CLEAN file ID, visibility → item.
- **Xử lý và dữ liệu:** TX validate exactly one payload; link is syntax-only, server does not fetch; file ownership/CLEAN.
- **Vận hành:** Idempotency; audit.
- **Lỗi đặc thù:** `PORTFOLIO_SCOPE_INVALID`, `LINK_INVALID`, `FILE_NOT_CLEAN`.
- **Màn hình/consumer:** `SCR-WRK-014`.
- **Capability/use case:** `CAP-WRK-001`, `UC-WRK-001`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L249`.
- **Actor/quyền:** Owner.
- **Input/validation → output:** Fields, If-Match → item.
- **Xử lý và dữ liệu:** TX predicate owner/id; validate; version++.
- **Vận hành:** Idempotency optional; audit.
- **Lỗi đặc thù:** `VERSION_CONFLICT`, `PORTFOLIO_ITEM_NOT_FOUND`.
- **Màn hình/consumer:** `SCR-WRK-014`.
- **Capability/use case:** `CAP-WRK-001`, `UC-WRK-001`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L250`.
- **Actor/quyền:** Owner.
- **Input/validation → output:** → hidden/deleted timestamp.
- **Xử lý và dữ liệu:** Soft-delete presentation; keep application snapshots immutable.
- **Vận hành:** Idempotent; audit.
- **Lỗi đặc thù:** `PORTFOLIO_ITEM_NOT_FOUND`.
- **Màn hình/consumer:** `SCR-WRK-014`.
- **Capability/use case:** `CAP-WRK-001`, `UC-WRK-001`.
