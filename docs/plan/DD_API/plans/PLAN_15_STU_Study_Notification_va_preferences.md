# PLAN 15 — Study — Notification và preferences
...
|---|---|
| Plan ID | `PLAN-15` |
| API trong phạm vi | `API-STU-034`, `API-STU-035`, `API-STU-036`, `API-STU-037`, `API-STU-038` |
| Số API | 5 |
...
Tạo DD cho cursor list, read/read-all và cấu hình kênh notification.

**Luồng nghiệp vụ trọng tâm:** List bằng signed cursor; mark read monotonic/idempotent; read-all theo cutoff; preferences upsert allowlist.

**Điểm cần khóa:** Cursor khóa filter/sort, dedupe, private no-store và không mất unread state.

**Dependency:** Outbox workers.

## 3. Phạm vi

**In-scope**

- `API-STU-034` — `GET /api/v1/notifications`.
- `API-STU-035` — `POST /api/v1/notifications/{id}/read`.
- `API-STU-036` — `POST /api/v1/notifications/read-all`.
- `API-STU-037` — `GET /api/v1/me/notification-preferences`.
- `API-STU-038` — `PUT /api/v1/me/notification-preferences`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L169`.
- **Actor/quyền:** Learner/staff.
- **Input/validation → output:** Cursor, unread/type → notifications.
- **Xử lý và dữ liệu:** Query`(recipient_id,created_at desc,id)`; exclude expired/deleted presentation rows.
- **Vận hành:** Private; cursor.
- **Lỗi đặc thù:** `CURSOR_INVALID`.
- **Màn hình/consumer:** `SCR-STU-020`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L170`.
- **Actor/quyền:** Recipient.
- **Input/validation → output:** → read timestamp.
- **Xử lý và dữ liệu:** Update predicate recipient+id; monotonic first-read.
- **Vận hành:** Idempotent; no audit needed.
- **Lỗi đặc thù:** `NOTIFICATION_NOT_FOUND`.
- **Màn hình/consumer:** `SCR-STU-020`.
- **Capability/use case:** `CAP-STU-007`, `UC-STU-007`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L171`.
- **Actor/quyền:** Recipient.
- **Input/validation → output:** Optional before timestamp → count.
- **Xử lý và dữ liệu:** Bulk update recipient/unread with index.
- **Vận hành:** Idempotent.
- **Lỗi đặc thù:** —.
- **Màn hình/consumer:** `SCR-STU-020`.
- **Capability/use case:** `CAP-STU-007`, `UC-STU-007`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L172`.
- **Actor/quyền:** User.
- **Input/validation → output:** → channel/category preferences.
- **Xử lý và dữ liệu:** R defaults + overrides.
- **Vận hành:** Private.
- **Lỗi đặc thù:** —.
- **Màn hình/consumer:** `SCR-STU-020`.
- **Capability/use case:** `CAP-STU-007`, `UC-STU-007`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L173`.
- **Actor/quyền:** User.
- **Input/validation → output:** Complete allowlisted matrix, If-Match → preferences.
- **Xử lý và dữ liệu:** TX replace overrides; mandatory security category cannot disable.
- **Vận hành:** Idempotency; audit.
- **Lỗi đặc thù:** `MANDATORY_NOTIFICATION_REQUIRED`, `VERSION_CONFLICT`.
- **Màn hình/consumer:** `SCR-STU-020`.
- **Capability/use case:** `CAP-STU-007`, `UC-STU-007`.
