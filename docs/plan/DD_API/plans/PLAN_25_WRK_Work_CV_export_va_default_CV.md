# PLAN 25 — Work — CV export và default CV
...
|---|---|
| Plan ID | `PLAN-25` |
| API trong phạm vi | `API-WRK-012`, `API-WRK-013` |
| Số API | 2 |
...
Tạo DD cho async export paid/free template và chọn CV mặc định.

**Luồng nghiệp vụ trọng tâm:** Export snapshot revision+template, reserve entitlement atomically, enqueue renderer và hoàn reservation khi lỗi; default đảm bảo một published owner CV.

**Điểm cần khóa:** Entitlement reservation ledger, async status/event, unique default constraint.

**Dependency:** Plans 24,47,50.

## 3. Phạm vi

**In-scope**

- `API-WRK-012` — `POST /api/v1/me/cvs/{cvId}/exports`.
- `API-WRK-013` — `PUT /api/v1/me/cvs/{cvId}/default`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L245`.
- **Actor/quyền:** Owner + entitlement if premium.
- **Input/validation → output:** Format PDF, template version → async export job.
- **Xử lý và dữ liệu:** Snapshot immutable CV revision + template; debit entitlement reservation atomically if paid; enqueue render outbox.
- **Vận hành:** Idempotency; 10/hour; event completion/refund reservation on failure.
- **Lỗi đặc thù:** `CV_NOT_PUBLISHED`, `ENTITLEMENT_INSUFFICIENT`, `EXPORT_BUSY`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L246`.
- **Actor/quyền:** Owner.
- **Input/validation → output:** → default CV.
- **Xử lý và dữ liệu:** TX L candidate; verify published owner CV; clear/set one default under unique constraint.
- **Vận hành:** Idempotency; audit.
- **Lỗi đặc thù:** `CV_NOT_PUBLISHED`.
- **Màn hình/consumer:** `SCR-WRK-013`.
- **Capability/use case:** `CAP-WRK-001`, `UC-WRK-001`.
