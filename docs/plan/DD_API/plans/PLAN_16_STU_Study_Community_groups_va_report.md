# PLAN 16 — Study — Community groups và report
...
|---|---|
| Plan ID | `PLAN-16` |
| API trong phạm vi | `API-STU-039`, `API-STU-040`, `API-STU-041`, `API-STU-042` |
| Số API | 4 |
...
Tạo DD cho xem group, chấp nhận rule, mở external link và báo cáo vi phạm.

**Luồng nghiệp vụ trọng tâm:** Read group theo eligibility; acceptance lưu version; open-link kiểm accepted rule và HTTPS; report tạo moderation case/audit.

**Điểm cần khóa:** Rule version, no server-side fetch, privacy và moderation audit.

**Dependency:** Plan 55/Operations moderation nếu áp dụng.

## 3. Phạm vi

**In-scope**

- `API-STU-039` — `GET /api/v1/community-groups`.
- `API-STU-040` — `POST /api/v1/community-groups/{id}/rule-acceptances`.
- `API-STU-041` — `POST /api/v1/community-groups/{id}/open-link`.
- `API-STU-042` — `POST /api/v1/community-groups/{id}/reports`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L174`.
- **Actor/quyền:** Eligible learner.
- **Input/validation → output:** → groups and accepted current rule version.
- **Xử lý và dữ liệu:** Join group scope through explicit path/course joins + learner eligibility.
- **Vận hành:** Private.
- **Lỗi đặc thù:** —.
- **Màn hình/consumer:** `SCR-STU-021`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L175`.
- **Actor/quyền:** Eligible learner.
- **Input/validation → output:** Rule version → acceptance.
- **Xử lý và dữ liệu:** TX validate exact current immutable rules and eligibility; append acceptance.
- **Vận hành:** Idempotency; audit.
- **Lỗi đặc thù:** `COMMUNITY_NOT_ELIGIBLE`, `RULE_VERSION_STALE`.
- **Màn hình/consumer:** `SCR-STU-021`.
- **Capability/use case:** `CAP-STU-007`, `UC-STU-007`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L176`.
- **Actor/quyền:** Eligible learner đã accept.
- **Input/validation → output:** → one-time redirect URL.
- **Xử lý và dữ liệu:** Revalidate eligibility/current acceptance; append link event; return external Zalo link, no secret at rest in client cache.
- **Vận hành:** Rate 10/giờ; audit.
- **Lỗi đặc thù:** `RULE_ACCEPTANCE_REQUIRED`, `COMMUNITY_NOT_ELIGIBLE`.
- **Màn hình/consumer:** `SCR-STU-021`.
- **Capability/use case:** `CAP-STU-007`, `UC-STU-007`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L177`.
- **Actor/quyền:** Eligible learner.
- **Input/validation → output:** Category, description <=2000, evidence file clean optional → report.
- **Xử lý và dữ liệu:** TX create report + event; no duplicate open report same reporter/group/category.
- **Vận hành:** Idempotency; moderator event/audit.
- **Lỗi đặc thù:** `DUPLICATE_OPEN_REPORT`, `FILE_NOT_CLEAN`.
- **Màn hình/consumer:** `SCR-STU-021`.
- **Capability/use case:** `CAP-STU-007`, `UC-STU-007`.
