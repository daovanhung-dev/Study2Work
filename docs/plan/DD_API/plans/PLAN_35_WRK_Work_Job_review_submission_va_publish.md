# PLAN 35 — Work — Job review submission và publish
...
|---|---|
| Plan ID | `PLAN-35` |
| API trong phạm vi | `API-WRK-046`, `API-WRK-047` |
| Số API | 2 |
...
Tạo DD cho gửi review và publish theo approved review hoặc trusted grant.

**Luồng nghiệp vụ trọng tâm:** Submit kiểm completeness, verified tenant, entitlement và tạo case; publish khóa job/revision, recheck review/grant, automated policy và swap current revision + outbox.

**Điểm cần khóa:** Trusted grant không bypass policy, concurrent reviewer/publisher, search index after commit.

**Dependency:** Plan 54 review/trusted grant.

## 3. Phạm vi

**In-scope**

- `API-WRK-046` — `POST /api/v1/enterprises/{enterpriseId}/jobs/{jobId}/review-submissions`.
- `API-WRK-047` — `POST /api/v1/enterprises/{enterpriseId}/jobs/{jobId}/publish`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L307`.
- **Actor/quyền:** `PERM-WRK-012`.
- **Input/validation → output:** Revision/version, declarations, rights → REVIEW_PENDING.
- **Xử lý và dữ liệu:** TX validate completeness, verified tenant, entitlement; append review case; stable lifecycle REVIEW_PENDING.
- **Vận hành:** Idempotency; reviewer outbox/audit.
- **Lỗi đặc thù:** `JOB_INCOMPLETE`, `REVIEW_ALREADY_PENDING`, `VERSION_CONFLICT`.
- **Màn hình/consumer:** `SCR-WRK-035`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L308`.
- **Actor/quyền:** `PERM-WRK-013`; trusted grant or approved review.
- **Input/validation → output:** Revision, review/grant ID, If-Match → PUBLISHED.
- **Xử lý và dữ liệu:** TX L job/revision; validate current approved review or active time-bounded trusted grant, automated policy, tenant/entitlement; publish immutable revision, supersede old current; trusted never bypasses check.
- **Vận hành:** Idempotency; publish/search-index/audit outbox.
- **Lỗi đặc thù:** `JOB_REVIEW_REQUIRED`, `TRUSTED_GRANT_EXPIRED`, `JOB_POLICY_FAILED`, `VERSION_CONFLICT`.
- **Màn hình/consumer:** `SCR-WRK-035`.
- **Capability/use case:** `CAP-WRK-004`, `UC-WRK-004`.
