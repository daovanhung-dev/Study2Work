# PLAN 13 — Study — Assessment attempt, kết quả và review view
...
|---|---|
| Plan ID | `PLAN-13` |
| API trong phạm vi | `API-STU-027`, `API-STU-028`, `API-STU-029` |
| Số API | 3 |
...
Tạo DD cho submit attempt, liệt kê attempts và xem chi tiết attempt.

**Luồng nghiệp vụ trọng tâm:** Submit khóa learner+assessment, kiểm attempt limit, draft/file CLEAN, tạo immutable attempt/answers, auto-grade hoặc UNDER_REVIEW; reads theo owner/reviewer scope.

**Điểm cần khóa:** Không tạo attempt khi file pending, attemptNo unique, transaction nhiều bảng và outbox.

**Dependency:** Plans 12,14,18.

## 3. Phạm vi

**In-scope**

- `API-STU-027` — `POST /api/v1/assessments/{assessmentId}/attempts`.
- `API-STU-028` — `GET /api/v1/assessments/{assessmentId}/attempts`.
- `API-STU-029` — `GET /api/v1/assessment-attempts/{attemptId}`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L157`.
- **Actor/quyền:** Learner owner.
- **Input/validation → output:** Optional`draftVersion`; direct type payload allowed; consent to submit → immutable attempt/result.
- **Xử lý và dữ liệu:** TX advisory lock learner+assessment; enforce attempts; assign attempt_no; validate/seal payload. Quiz auto-grade to PASSED/FAILED; TEXT/LINK/FILE → UNDER_REVIEW; file must CLEAN.
- **Vận hành:** Idempotency required; events submitted/graded/review_requested; notification async.
- **Lỗi đặc thù:** `ATTEMPT_LIMIT_REACHED`, `FILE_NOT_CLEAN`, `DRAFT_STALE`, `QUIZ_ANSWER_INVALID`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L158`.
- **Actor/quyền:** Learner owner.
- **Input/validation → output:** Page → attempt summaries.
- **Xử lý và dữ liệu:** Query`(user,assessment,attempt_no desc)`; feedback according release policy.
- **Vận hành:** Private.
- **Lỗi đặc thù:** —.
- **Màn hình/consumer:** `SCR-STU-018`.
- **Capability/use case:** `CAP-STU-004`, `UC-STU-004`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L159`.
- **Actor/quyền:** Owner or`PERM-STU-006` reviewer.
- **Input/validation → output:** → immutable submitted answer, grade/reviews; reviewer gets clean file URL separately.
- **Xử lý và dữ liệu:** Owner predicate or assigned review scope; join answer subtype/review history.
- **Vận hành:** Private no-store; access audit for reviewer.
- **Lỗi đặc thù:** `ATTEMPT_NOT_FOUND`, `FILE_NOT_CLEAN`.
