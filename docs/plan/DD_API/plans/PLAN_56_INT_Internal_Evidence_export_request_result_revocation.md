# PLAN 56 — Internal — Evidence export request/result/revocation
...
|---|---|
| Plan ID | `PLAN-56` |
| API trong phạm vi | `API-INT-002`, `API-INT-003`, `API-INT-004`, `API-INT-005` |
| Số API | 4 |
...
Tạo DD cho saga evidence giữa Work và Study.

**Luồng nghiệp vụ trọng tâm:** Work requests selected IDs+versions+consent; Study persists request and prepares minimal snapshot; Work ingests ready/unavailable/revocation idempotently; withdrawal always wins read visibility.

**Điểm cần khóa:** Cross-service signed contract, retries/DLQ, snapshot immutable, no negative inference.

**Dependency:** Plans 21,28,29.

## 3. Phạm vi

**In-scope**

- `API-INT-002` — `POST study /internal/v1/evidence-export-requests`.
- `API-INT-003` — `GET study /internal/v1/evidence-export-requests/{requestId}`.
- `API-INT-004` — `POST work /internal/v1/evidence-export-results`.
- `API-INT-005` — `POST work /internal/v1/evidence-revocations`.
...
- Screens: `SCR-WRK-017`, `SCR-WRK-019`, `SCR-WRK-040`, `SCR-OPS-021`.
- Acceptance: `TC-STU-005`, `TC-WRK-006`, `TC-OPS-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L440`.
- **Actor/quyền:** Work service JWT + detached body signature,`aud=study-internal`.
- **Input/validation → output:** Request ID, application ID, platform subject, selected`{evidenceId,expectedVersion}`, consent version/hash → `202`.
- **Xử lý và dữ liệu:** Unique request ID; verify timestamp/nonce <=5m; store request/outbox. Worker checks Study ownership,`ISSUED`, exact version, not revoked; creates minimal signed result. Never accept candidate ID asserted without token subject mapping.
- **Vận hành:** Unique request ID; verify timestamp/nonce <=5m; store request/outbox. Worker checks Study ownership,`ISSUED`, exact version, not revoked; creates minimal signed result. Never accept candidate ID asserted without token subject mapping.
- **Lỗi đặc thù:** `SIGNATURE_INVALID`, `REPLAY_DETECTED`, `REQUEST_CONFLICT`.
- **Màn hình/consumer:** SYSTEM.
- **Capability/use case:** `CAP-STU-005`, `UC-STU-005`, `CAP-WRK-006`, `UC-WRK-006`, `CAP-OPS-003`, `UC-OPS-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L441`.
- **Actor/quyền:** Work.
- **Input/validation → output:** → request processing status only.
- **Xử lý và dữ liệu:** Caller/request binding; fallback reconciliation, not polling faster than 30s.
- **Vận hành:** Caller/request binding; fallback reconciliation, not polling faster than 30s.
- **Lỗi đặc thù:** `REQUEST_NOT_FOUND`.
- **Màn hình/consumer:** SYSTEM.
- **Capability/use case:** `CAP-STU-005`, `UC-STU-005`, `CAP-WRK-006`, `UC-WRK-006`, `CAP-OPS-003`, `UC-OPS-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L442`.
- **Actor/quyền:** Study service JWT + detached signature.
- **Input/validation → output:** Request/application IDs, per-item READY/UNAVAILABLE + minimal immutable snapshots/checksum/reason-safe code.
- **Xử lý và dữ liệu:** TX unique result/event; lock application request; reject subject/application mismatch; store snapshots encrypted by application; if consent withdrawn store audit receipt but keep hidden; never affect ATS/match.
- **Vận hành:** TX unique result/event; lock application request; reject subject/application mismatch; store snapshots encrypted by application; if consent withdrawn store audit receipt but keep hidden; never affect ATS/match.
- **Lỗi đặc thù:** `RESULT_CONFLICT`, `APPLICATION_NOT_FOUND`, `SIGNATURE_INVALID`.
- **Màn hình/consumer:** SYSTEM.
- **Capability/use case:** `CAP-STU-005`, `UC-STU-005`, `CAP-WRK-006`, `UC-WRK-006`, `CAP-OPS-003`, `UC-OPS-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L443`.
- **Actor/quyền:** Study.
- **Input/validation → output:** Evidence/version/revokedAt/reason-safe/eventId → accepted.
- **Xử lý và dữ liệu:** Dedup event; mark every matching application snapshot REVOKED/hidden; append history, notify candidate/recruiter without negative recommendation.
- **Vận hành:** Dedup event; mark every matching application snapshot REVOKED/hidden; append history, notify candidate/recruiter without negative recommendation.
- **Lỗi đặc thù:** `SIGNATURE_INVALID`, `EVENT_STALE` acknowledged/no-op.
- **Màn hình/consumer:** SYSTEM.
- **Capability/use case:** `CAP-STU-005`, `UC-STU-005`, `CAP-WRK-006`, `UC-WRK-006`, `CAP-OPS-003`, `UC-OPS-003`.
