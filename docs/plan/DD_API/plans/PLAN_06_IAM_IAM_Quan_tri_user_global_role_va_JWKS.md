# PLAN 06 — IAM — Quản trị user, global role và JWKS
...
|---|---|
| Plan ID | `PLAN-06` |
| API trong phạm vi | `API-IAM-021`, `API-IAM-022`, `API-IAM-023`, `API-IAM-024` |
| Số API | 4 |
...
Tạo DD cho tìm kiếm user đã redact, thay đổi trạng thái, gán global role và công bố khóa ES256.

**Luồng nghiệp vụ trọng tâm:** Admin tra cứu theo scope; mutation status/role dùng MFA, If-Match, audit và outbox; JWKS chỉ trả public key active/rollover.

**Điểm cần khóa:** API-IAM-022 thiếu cột màn hình trong API source; self-suspend/last-admin guard; key rollover/cache.

**Dependency:** Plan 02, Plan 56.

## 3. Phạm vi

**In-scope**

- `API-IAM-021` — `GET /api/v1/admin/users`.
- `API-IAM-022` — `PUT /api/v1/admin/users/{userId}/status`.
- `API-IAM-023` — `PUT /api/v1/admin/users/{userId}/global-roles`.
- `API-IAM-024` — `GET /.well-known/jwks.json`.
...
- `01_TONG_QUAN_DU_AN.md:L759` — `CAP-IAM-003` / `UC-IAM-003`; rules ``PERM-IAM-001–004`, `BR-IAM-005–007`, `BR-OPS-001–002``; diagrams ``AC-IAM-001`, `SEQ-IAM-002``; data ``CLS-IAM-001`; `TBL-IAM-012–018``; screens ``SCR-OPS-001–002`, `SCR-OPS-024``; test ``TC-IAM-003``.
...
- `TBL-IAM-012` — `roles` — `03_THIET_KE_CO_SO_DU_LIEU.md:L189 `.
- `TBL-IAM-013` — `permissions` — `03_THIET_KE_CO_SO_DU_LIEU.md:L196 `.
- `TBL-IAM-014` — `role_permissions` — `03_THIET_KE_CO_SO_DU_LIEU.md:L203 `.
- `TBL-IAM-015` — `user_role_assignments` — `03_THIET_KE_CO_SO_DU_LIEU.md:L210 `.
- `TBL-IAM-016` — `idempotency_keys` — `03_THIET_KE_CO_SO_DU_LIEU.md:L219 `.

**Screen và acceptance**

- Screens: `SCR-IAM-003`, `SCR-IAM-004`, `SCR-IAM-005`, `SCR-IAM-006`, `SCR-OPS-001`, `SCR-OPS-002`, `SCR-OPS-024`.
- Acceptance: `TC-IAM-002`, `TC-IAM-003`.
...
- `PERM-IAM-001` — `01_TONG_QUAN_DU_AN.md:L146` — `platform.users.read` — Platform Admin, Security Auditor (chỉ đọc).
- `PERM-IAM-002` — `01_TONG_QUAN_DU_AN.md:L147` — `platform.users.status.manage` — Platform Admin.
- `PERM-IAM-003` — `01_TONG_QUAN_DU_AN.md:L148` — `platform.roles.manage` — Platform Admin.
- `PERM-IAM-004` — `01_TONG_QUAN_DU_AN.md:L149` — `platform.security.audit.read` — Security Auditor, Platform Admin.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L107`.
- **Actor/quyền:** `PERM-IAM-001`.
- **Input/validation → output:** Page/filter status/email hash/role → redacted users.
- **Xử lý và dữ liệu:** Query`(status,created_at,id)` or normalized email exact index; PII view requires separate permission.
- **Vận hành:** No shared cache; audit search; 60/phút.
- **Lỗi đặc thù:** `MFA_REQUIRED`, `PERMISSION_DENIED`.
- **Màn hình/consumer:** `SCR-OPS-001`.
- **Capability/use case:** `CAP-IAM-003`, `UC-IAM-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L108`.
- **Actor/quyền:** `PERM-IAM-002` + MFA.
- **Input/validation → output:** `ACTIVE                                                                                                                                             | SUSPENDED`, reason, `If-Match` → user.
- **Xử lý và dữ liệu:** TX L user; validate transition; authVersion++; revoke sessions on suspend; outbox.
- **Vận hành:** Idempotency; audit before/after; event status changed.
- **Lỗi đặc thù:** `INVALID_ACCOUNT_TRANSITION`, `VERSION_CONFLICT`, `SELF_SUSPEND_FORBIDDEN`.
- **Màn hình/consumer:** `SCR-OPS-001` — DERIVED từ 05_DAC_TA_MAN_HINH.md:L171/L236.
- **Capability/use case:** `CAP-IAM-003`, `UC-IAM-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L109`.
- **Actor/quyền:** `PERM-IAM-003` + recent MFA.
- **Input/validation → output:** Exact role code set, reason,`If-Match` → roles/version.
- **Xử lý và dữ liệu:** TX L user; diff assignments; forbid last platform admin removal; authVersion++; outbox.
- **Vận hành:** Idempotency; full audit; no cache.
- **Lỗi đặc thù:** `LAST_ADMIN_REQUIRED`, `ROLE_NOT_ALLOWED`, `VERSION_CONFLICT`.
- **Màn hình/consumer:** `SCR-OPS-002`.
- **Capability/use case:** `CAP-IAM-003`, `UC-IAM-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L110`.
- **Actor/quyền:** Public.
- **Input/validation → output:** Không input → active/public rollover keys.
- **Xử lý và dữ liệu:** Read signing-key metadata or config; chỉ public parameters; cache key cũ đến hết max token TTL.
- **Vận hành:** CDN 5 phút; ETag.
- **Lỗi đặc thù:** `503` nếu không có active key.
