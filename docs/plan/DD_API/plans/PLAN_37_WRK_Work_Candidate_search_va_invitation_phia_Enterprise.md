# PLAN 37 — Work — Candidate search và invitation phía Enterprise
...
|---|---|
| Plan ID | `PLAN-37` |
| API trong phạm vi | `API-WRK-051`, `API-WRK-052`, `API-WRK-053` |
| Số API | 3 |
...
Tạo DD cho search consented candidates, xem public detail và gửi invitation.

**Luồng nghiệp vụ trọng tâm:** Query tenant permission + consent/search index; sensitive fields excluded; sponsored labels separate; invitation unique/stateful và không tạo application.

**Điểm cần khóa:** Consent gate, rate limit 60/min/member, protected attributes và cross-tenant isolation.

**Dependency:** Plans 23,27; Plan 58 deindex.

## 3. Phạm vi

**In-scope**

- `API-WRK-051` — `POST /api/v1/enterprises/{enterpriseId}/candidate-searches`.
- `API-WRK-052` — `GET /api/v1/enterprises/{enterpriseId}/candidate-searches/{candidatePublicId}`.
- `API-WRK-053` — `POST /api/v1/enterprises/{enterpriseId}/candidate-invitations`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L319`.
- **Actor/quyền:** `PERM-WRK-020` + active search entitlement.
- **Input/validation → output:** Filters/keywords/page → redacted candidates + organic match explanation; sponsored profiles separate/labeled.
- **Xử lý và dữ liệu:** Server tenant membership; query only consent ACTIVE/not expired and DB visibility predicate; search doc excludes contact, CV raw, Study evidence, protected/sensitive fields. Sponsored injection separate from organic score.
- **Vận hành:** Audit query/filter/result IDs; rate 60/min; no shared cache.
- **Lỗi đặc thù:** `SEARCH_ENTITLEMENT_REQUIRED`, `FILTER_NOT_ALLOWED`, `CONSENT_INDEX_LAG` safe empty.
- **Màn hình/consumer:** `SCR-WRK-036`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L320`.
- **Actor/quyền:** `PERM-WRK-020`.
- **Input/validation → output:** → redacted public profile/portfolio preview.
- **Xử lý và dữ liệu:** Resolve rotating public ID scoped to tenant; recheck consent at DB, never expose email/phone/CV/evidence.
- **Vận hành:** Access audit; private no-store.
- **Lỗi đặc thù:** `CANDIDATE_NOT_FOUND`.
- **Màn hình/consumer:** `SCR-WRK-037`.
- **Capability/use case:** `CAP-WRK-002`, `UC-WRK-002`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L321`.
- **Actor/quyền:** `PERM-WRK-021`.
- **Input/validation → output:** Candidate public ID, published job, message <=1000 → invitation.
- **Xử lý và dữ liệu:** Resolve consent candidate; validate same tenant job accepting; create unique open invite candidate/job/tenant; no chat/application.
- **Vận hành:** Idempotency; debit invitation entitlement; notification/audit.
- **Lỗi đặc thù:** `CANDIDATE_NOT_SEARCHABLE`, `INVITATION_EXISTS`, `ENTITLEMENT_INSUFFICIENT`.
- **Màn hình/consumer:** `SCR-WRK-037`, `SCR-WRK-038`.
- **Capability/use case:** `CAP-WRK-002`, `UC-WRK-002`.
