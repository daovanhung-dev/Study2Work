# PLAN 43 — University — Internship, campus distribution, partnership và referral
...
|---|---|
| Plan ID | `PLAN-43` |
| API trong phạm vi | `API-UNI-010`, `API-UNI-011`, `API-UNI-012`, `API-UNI-013`, `API-UNI-014` |
| Số API | 5 |
...
Tạo DD cho chương trình thực tập, phân phối job, partnership và candidate referral.

**Luồng nghiệp vụ trọng tâm:** Program schema/eligibility; campus distribution link job; partnership proposal/respond; referral chỉ khi affiliation+consent active và không auto-apply.

**Điểm cần khóa:** Cross-tenant partnership, consent purpose/expiry, referral state and no hidden application.

**Dependency:** Work jobs/enterprise plans.

## 3. Phạm vi

**In-scope**

- `API-UNI-010` — `POST /api/v1/universities/{universityId}/internship-programs`.
- `API-UNI-011` — `POST /api/v1/universities/{universityId}/campus-job-distributions`.
- `API-UNI-012` — `POST /api/v1/universities/{universityId}/partnerships`.
- `API-UNI-013` — `POST /api/v1/enterprises/{enterpriseId}/university-partnerships/{id}/respond`.
- `API-UNI-014` — `POST /api/v1/universities/{universityId}/referrals`.
...
- `01_TONG_QUAN_DU_AN.md:L779` — `CAP-UNI-002` / `UC-UNI-002`; rules ``BR-UNI-002–003`, `PERM-UNI-020–023`, `PERM-WRK-050``; diagrams ``AC-UNI-001`, `SEQ-UNI-001``; data ``CLS-WRK-002`; `TBL-WRK-026–029`, `TBL-WRK-071``; screens ``SCR-UNI-007–010`, `SCR-WRK-043``; test ``TC-UNI-002``.
...
- `TBL-WRK-026` — `internship_programs` — `03_THIET_KE_CO_SO_DU_LIEU.md:L885 `.
- `TBL-WRK-027` — `campus_job_distributions` — `03_THIET_KE_CO_SO_DU_LIEU.md:L892 `.
- `TBL-WRK-028` — `partnerships` — `03_THIET_KE_CO_SO_DU_LIEU.md:L899 `.
- `TBL-WRK-029` — `candidate_referrals` — `03_THIET_KE_CO_SO_DU_LIEU.md:L906 `.
- `TBL-WRK-071` — `internship_program_participants` — `03_THIET_KE_CO_SO_DU_LIEU.md:L1368 `.

**Screen và acceptance**

- Screens: `SCR-UNI-007`, `SCR-UNI-008`, `SCR-UNI-009`, `SCR-UNI-010`, `SCR-WRK-043`.
- Acceptance: `TC-UNI-002`.
...
- `BR-UNI-003` — `01_TONG_QUAN_DU_AN.md:L412` — Campus distribution/referral không tạo application thay candidate và không chia sẻ evidence tự động. Candidate vẫn chọn job, snapshot và consent như apply thông thường..
- `PERM-UNI-020` — `01_TONG_QUAN_DU_AN.md:L187` — `university.programs.manage` — University Owner/Coordinator.
- `PERM-UNI-021` — `01_TONG_QUAN_DU_AN.md:L188` — `university.campus_jobs.distribute` — University Owner/Coordinator.
- `PERM-UNI-022` — `01_TONG_QUAN_DU_AN.md:L189` — `university.partnerships.manage` — University Owner/Coordinator.
- `PERM-UNI-023` — `01_TONG_QUAN_DU_AN.md:L190` — `university.referrals.manage` — University Owner/Coordinator.
- `PERM-WRK-050` — `01_TONG_QUAN_DU_AN.md:L178` — `work.university_partnerships.respond` — Enterprise Owner/Admin.

## 5. Chi tiết từng API

### API-UNI-010 — `POST /api/v1/universities/{universityId}/internship-programs`

- **Nguồn contract:** `04_DAC_TA_API.md:L354`.
- **Actor/quyền:** `PERM-UNI-020`.
- **Input/validation → output:** Program, dates, eligibility, partner enterprises → program DRAFT.
- **Xử lý và dữ liệu:** TX validate tenant/partner link; structured rules.
- **Vận hành:** Idempotency; audit.
- **Lỗi đặc thù:** `PARTNER_NOT_ACTIVE`, `PROGRAM_INVALID`.
- **Màn hình/consumer:** `SCR-UNI-007`.
- **Capability/use case:** `CAP-UNI-002`, `UC-UNI-002`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L355`.
- **Actor/quyền:** `PERM-UNI-021`.
- **Input/validation → output:** Published job IDs, cohorts, period, message → distribution.
- **Xử lý và dữ liệu:** Validate jobs public/accepting and tenant cohort; create distribution/recipient aggregate; no auto-application.
- **Vận hành:** Idempotency; notification outbox.
- **Lỗi đặc thù:** `JOB_NOT_ACCEPTING_APPLICATIONS`, `COHORT_NOT_FOUND`.
- **Màn hình/consumer:** `SCR-UNI-008`.
- **Capability/use case:** `CAP-UNI-002`, `UC-UNI-002`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L356`.
- **Actor/quyền:** `PERM-UNI-022`.
- **Input/validation → output:** Enterprise ID, scopes, dates → PENDING partnership.
- **Xử lý và dữ liệu:** Create bilateral request; data sharing off until enterprise accepts; exact purpose/scope.
- **Vận hành:** Idempotency; audit/notification.
- **Lỗi đặc thù:** `PARTNERSHIP_EXISTS`, `SCOPE_INVALID`.
- **Màn hình/consumer:** `SCR-UNI-009`.
- **Capability/use case:** `CAP-UNI-002`, `UC-UNI-002`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L357`.
- **Actor/quyền:** Enterprise`PERM-WRK-050`.
- **Input/validation → output:** Accept/decline, scopes/version → status.
- **Xử lý và dữ liệu:** Cross-tenant Work DB transaction because both tenant types live Work DB; memberships checked separately; append history.
- **Vận hành:** Idempotency; audit.
- **Lỗi đặc thù:** `PARTNERSHIP_VERSION_CONFLICT`, `PERMISSION_DENIED`.
- **Màn hình/consumer:** `SCR-WRK-043`.
- **Capability/use case:** `CAP-UNI-002`, `UC-UNI-002`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L358`.
- **Actor/quyền:** `PERM-UNI-023` + active learner consent.
- **Input/validation → output:** Affiliation, published job, note → referral.
- **Xử lý và dữ liệu:** Recheck consent purpose JOB_REFERRAL; referral does not apply or open chat; candidate chooses apply.
- **Vận hành:** Idempotency; privacy audit/notification.
- **Lỗi đặc thù:** `CONSENT_REQUIRED`, `JOB_NOT_ACCEPTING_APPLICATIONS`.
- **Màn hình/consumer:** `SCR-UNI-010`.
- **Capability/use case:** `CAP-UNI-002`, `UC-UNI-002`.
