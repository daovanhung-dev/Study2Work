# PLAN 22 — Work — Public job catalog
...
|---|---|
| Plan ID | `PLAN-22` |
| API trong phạm vi | `API-WRK-001`, `API-WRK-002`, `API-WRK-003`, `API-WRK-004` |
| Số API | 4 |
| Read-only / Mutation | 4 / 0 |
...
Tạo DD cho danh sách/chi tiết job, employer public profile và metadata.

**Luồng nghiệp vụ trọng tâm:** Read current immutable published job revisions, public tenant projection, taxonomy và sponsored placement được chèn riêng/có nhãn.

**Điểm cần khóa:** Organic score không bị quảng cáo thay đổi, CDN/ETag, expiration và public privacy.

**Dependency:** Plans 35–37 job lifecycle; Plan 48 promotions.

## 3. Phạm vi

**In-scope**

- `API-WRK-001` — `GET /api/v1/jobs`.
- `API-WRK-002` — `GET /api/v1/jobs/{jobSlug}`.
- `API-WRK-003` — `GET /api/v1/enterprises/{enterpriseSlug}`.
- `API-WRK-004` — `GET /api/v1/work-metadata`.
...
- `01_TONG_QUAN_DU_AN.md`: không có dòng capability trực tiếp; dùng API catalog và diagram coverage.

**Diagram/Class cần đọc toàn bộ section**

- Không có diagram/class anchor trực tiếp trong traceability; dùng endpoint row và coverage matrix.

**Bảng dữ liệu liên quan theo traceability**

- Table cụ thể phải xác định từ từng endpoint row và schema; không suy đoán.

**Screen và acceptance**

- Screens: `SYSTEM/N/A` theo endpoint.
- Acceptance: `SOURCE_REQUIRED` nếu không có trace row.

## 5. Chi tiết từng API

### API-WRK-001 — `GET /api/v1/jobs`

- **Nguồn contract:** `04_DAC_TA_API.md:L229`.
- **Actor/quyền:** Public.
- **Input/validation → output:** Page, q, location, workMode, employmentType, experience, skills, salary range, sort → labeled job cards.
- **Xử lý và dữ liệu:** R only`PUBLISHED` current immutable revisions, enterprise public projection, active promotions; FTS/trigram + filter indexes. Organic score computed separately; sponsored slots injected and labeled, never alter score.
- **Vận hành:** CDN 60s; ETag; anonymous rate.
- **Lỗi đặc thù:** `INVALID_FILTER`, `INVALID_SORT`.
- **Màn hình/consumer:** `SCR-WRK-001`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L230`.
- **Actor/quyền:** Public.
- **Input/validation → output:** Slug → published revision, enterprise, expiry, sponsored label.
- **Xử lý và dữ liệu:** Unique slug/current revision; increment view via async dedup event, not write in read TX.
- **Vận hành:** CDN 60s; view dedupe user/IP hash/day.
- **Lỗi đặc thù:** `JOB_NOT_FOUND`, `JOB_NO_LONGER_AVAILABLE` metadata only for known saved user.
- **Màn hình/consumer:** `SCR-WRK-002`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L231`.
- **Actor/quyền:** Public.
- **Input/validation → output:** Slug → verified public employer profile + published jobs.
- **Xử lý và dữ liệu:** R public tenant projection; never return members, billing or verification documents.
- **Vận hành:** CDN 5m.
- **Lỗi đặc thù:** `ENTERPRISE_NOT_FOUND`.
- **Màn hình/consumer:** `SCR-WRK-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L232`.
- **Actor/quyền:** Public.
- **Input/validation → output:** Optional groups → skills, industries, locations, employment/work modes.
- **Xử lý và dữ liệu:** R canonical active taxonomy/projections.
- **Vận hành:** Cache 10m, ETag.
- **Lỗi đặc thù:** `METADATA_GROUP_INVALID`.
