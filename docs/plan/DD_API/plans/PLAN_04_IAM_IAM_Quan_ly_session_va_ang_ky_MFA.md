# PLAN 04 — IAM — Quản lý session và đăng ký MFA
...
|---|---|
| Plan ID | `PLAN-04` |
| API trong phạm vi | `API-IAM-015`, `API-IAM-016`, `API-IAM-017`, `API-IAM-018` |
| Số API | 4 |
...
Tạo DD cho danh sách/revoke session và quy trình enroll/confirm MFA.

**Luồng nghiệp vụ trọng tâm:** Người dùng xem session, thu hồi session khác, tạo secret/challenge MFA rồi confirm TOTP và nhận recovery code một lần.

**Điểm cần khóa:** Không revoke nhầm current session, secret chỉ hiển thị trước confirm, recovery code chỉ trả một lần.
...
- `API-IAM-015` — `GET /api/v1/me/sessions`.
- `API-IAM-016` — `DELETE /api/v1/me/sessions/{sessionId}`.
- `API-IAM-017` — `POST /api/v1/me/mfa/enrollments`.
- `API-IAM-018` — `POST /api/v1/me/mfa/enrollments/{id}/confirm`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L101`.
- **Actor/quyền:** Authenticated.
- **Input/validation → output:** Cursor → active sessions, current flag, coarse device/IP.
- **Xử lý và dữ liệu:** R sessions by`(user_id,last_seen_at,id)` cursor index; IP masked.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L102`.
- **Actor/quyền:** Authenticated + owner.
- **Input/validation → output:** UUID → revoked.
- **Xử lý và dữ liệu:** TX update predicate`id AND user_id`; revoke refresh tokens.
- **Vận hành:** Idempotent; audit.
- **Lỗi đặc thù:** `SESSION_NOT_FOUND`; current session uses logout contract.
- **Màn hình/consumer:** `SCR-IAM-006`.
- **Capability/use case:** `CAP-IAM-002`, `UC-IAM-002`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L103`.
- **Actor/quyền:** Privileged user + step-up.
- **Input/validation → output:** Method`TOTP`; device label → secret once + QR payload, recovery codes once.
- **Xử lý và dữ liệu:** Encrypt pending secret with KMS envelope; create enrollment not active until verify.
- **Vận hành:** Idempotency;`no-store`; audit.
- **Lỗi đặc thù:** `MFA_ALREADY_ENABLED`, `STEP_UP_REQUIRED`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L104`.
- **Actor/quyền:** Owner.
- **Input/validation → output:** TOTP code → active enrollment.
- **Xử lý và dữ liệu:** TX L enrollment; verify; activate; store hashed recovery codes; never return secret again.
- **Vận hành:** Idempotency; audit.
- **Lỗi đặc thù:** `MFA_CODE_INVALID`, `MFA_ENROLLMENT_EXPIRED`.
- **Màn hình/consumer:** `SCR-IAM-005`.
- **Capability/use case:** `CAP-IAM-002`, `UC-IAM-002`.
