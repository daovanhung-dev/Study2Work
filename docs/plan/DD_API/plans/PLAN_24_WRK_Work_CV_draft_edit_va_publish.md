# PLAN 24 — Work — CV draft, edit và publish
...
|---|---|
| Plan ID | `PLAN-24` |
| API trong phạm vi | `API-WRK-008`, `API-WRK-009`, `API-WRK-010`, `API-WRK-011` |
| Số API | 4 |
...
Tạo DD cho list CV, tạo stable+draft, sửa typed sections và publish revision.

**Luồng nghiệp vụ trọng tâm:** Quota/entitlement khi tạo; patch draft schema/date/sanitize; publish immutable revision, supersede current và chỉ update search preview khi consent.

**Điểm cần khóa:** Nested sections one-field-per-row, revision state, If-Match và snapshot immutability.

**Dependency:** Plan 25 export/default; AI Plan 50.

## 3. Phạm vi

**In-scope**

- `API-WRK-008` — `GET /api/v1/me/cvs`.
- `API-WRK-009` — `POST /api/v1/me/cvs`.
- `API-WRK-010` — `PATCH /api/v1/me/cvs/{cvId}/revisions/{revisionId}`.
- `API-WRK-011` — `POST /api/v1/me/cvs/{cvId}/revisions/{revisionId}/publish`.
...
- Screens: `SCR-WRK-011`, `SCR-WRK-012`, `SCR-WRK-013`, `SCR-WRK-014`.
- Acceptance: `TC-WRK-001`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L241`.
- **Actor/quyền:** Candidate.
- **Input/validation → output:** Page → CV metadata/default/published version.
- **Xử lý và dữ liệu:** R CV stable rows and versions by owner; no public URL.
- **Vận hành:** Private.
- **Lỗi đặc thù:** —.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L242`.
- **Actor/quyền:** Candidate.
- **Input/validation → output:** Name, template ID, locale → CV + DRAFT revision.
- **Xử lý và dữ liệu:** TX enforce quota/entitlement; create stable CV/revision with empty typed sections.
- **Vận hành:** Idempotency; audit.
- **Lỗi đặc thù:** `CV_QUOTA_EXCEEDED`, `TEMPLATE_ENTITLEMENT_REQUIRED`.
- **Màn hình/consumer:** `SCR-WRK-013`.
- **Capability/use case:** `CAP-WRK-001`, `UC-WRK-001`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L243`.
- **Actor/quyền:** Owner.
- **Input/validation → output:** Typed sections,`If-Match` → draft revision.
- **Xử lý và dữ liệu:** TX L draft; require DRAFT/owner; schema/date/text sanitize; version++.
- **Vận hành:** Autosave idempotency optional; private no-store.
- **Lỗi đặc thù:** `CV_REVISION_NOT_EDITABLE`, `VERSION_CONFLICT`.
- **Màn hình/consumer:** `SCR-WRK-013`.
- **Capability/use case:** `CAP-WRK-001`, `UC-WRK-001`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L244`.
- **Actor/quyền:** Owner.
- **Input/validation → output:** If-Match → immutable PUBLISHED revision.
- **Xử lý và dữ liệu:** TX validate required fields; publish, supersede previous current, update search preview only if explicit consent.
- **Vận hành:** Idempotency required; audit/event.
- **Lỗi đặc thù:** `CV_INCOMPLETE`, `VERSION_CONFLICT`.
- **Màn hình/consumer:** `SCR-WRK-013`.
- **Capability/use case:** `CAP-WRK-001`, `UC-WRK-001`.
