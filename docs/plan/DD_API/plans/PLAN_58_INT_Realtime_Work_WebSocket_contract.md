# PLAN 58 — Realtime — Work WebSocket contract
...
|---|---|
| Plan ID | `PLAN-58` |
| API trong phạm vi | `API-INT-011` |
| Số API | 1 |
| Read-only / Mutation | 1 / 0 |
| Khối lượng lõi | 8 file template trước khi duplicate DB Mapping |
| Mức rủi ro | Rất cao |
| Trạng thái plan | `READY — SOURCE-GROUNDED`; API có source gap phải giữ Draft |

## 2. Mục tiêu và nghiệp vụ

Tạo DD cho handshake, subscription, message/receipt delivery và reconnect semantics của chat.

**Luồng nghiệp vụ trọng tâm:** Authenticate access token, authorize conversation subscription, use server sequence/ack, publish only after DB commit; reconnect fetches REST gap and duplicate events are tolerated.

**Điểm cần khóa:** REST source of truth, lease expiry, backpressure, no message loss claim without DB reconciliation.

**Dependency:** Plan 31 chat REST.

## 3. Phạm vi

**In-scope**

- `API-INT-011` — `GET wss://work-api.study2work.vn/api/v1/realtime`.
...
- Screens: `SCR-WRK-021`, `SCR-WRK-042`, `SCR-OPS-026`.
- Acceptance: `TC-WRK-008`.
