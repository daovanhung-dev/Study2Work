# PLAN 09 — Study — Gợi ý và primary path
...
|---|---|
| Plan ID | `PLAN-09` |
| API trong phạm vi | `API-STU-012`, `API-STU-013`, `API-STU-014`, `API-STU-015` |
| Số API | 4 |
...
Tạo DD cho recommendation read, xem/chọn/đổi primary path và lịch sử.

**Luồng nghiệp vụ trọng tâm:** Recommendation chỉ là giải thích/hỗ trợ; primary path selection/switch khóa theo learner, kiểm onboarding, published version và cooldown; history append-only.

**Điểm cần khóa:** Một active period duy nhất, advisory lock, cooldown 168 giờ, không làm mất progress.

**Dependency:** Plan 08; Plan 10 enrollment.

## 3. Phạm vi

**In-scope**

- `API-STU-012` — `GET /api/v1/me/path-recommendations`.
- `API-STU-013` — `GET /api/v1/me/primary-path`.
- `API-STU-014` — `PUT /api/v1/me/primary-path`.
- `API-STU-015` — `GET /api/v1/me/primary-path/history`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L137`.
- **Actor/quyền:** Learner active + onboarding completed.
- **Input/validation → output:** → top 3 path versions, score bands/reasons/run version.
- **Xử lý và dữ liệu:** R latest immutable recommendation run/items; omit unpublished path and mark stale if catalog changed.
- **Vận hành:** Private no-store.
- **Lỗi đặc thù:** `ONBOARDING_REQUIRED`.
- **Màn hình/consumer:** `SCR-STU-012`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L138`.
- **Actor/quyền:** Learner active.
- **Input/validation → output:** → active period, pinned version, progress,`nextSwitchAllowedAt`.
- **Xử lý và dữ liệu:** R partial-active period + path snapshot; unique active invariant.
- **Vận hành:** Private no-store.
- **Lỗi đặc thù:** —.
- **Màn hình/consumer:** `SCR-STU-013`.
- **Capability/use case:** `CAP-STU-002`, `UC-STU-002`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L139`.
- **Actor/quyền:** Learner active + onboarding complete.
- **Input/validation → output:** `pathVersionId`, expected current period/version, reason optional → new active period.
- **Xử lý và dữ liệu:** TX advisory lock user; idempotency; validate published; initial select or enforce UTC 168h cooldown; close old as SWITCHED_OUT; insert new ACTIVE; preserve all enrollment/progress.
- **Vận hành:** Idempotency required;`study.primary_path.changed`; audit; no cache.
- **Lỗi đặc thù:** `ONBOARDING_REQUIRED`, `PRIMARY_PATH_SWITCH_COOLDOWN`, `PATH_VERSION_NOT_PUBLISHED`, `VERSION_CONFLICT`.
- **Màn hình/consumer:** `SCR-STU-013`.
- **Capability/use case:** `CAP-STU-002`, `UC-STU-002`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L140`.
- **Actor/quyền:** Learner active.
- **Input/validation → output:** Page → immutable periods/change reasons.
- **Xử lý và dữ liệu:** Query`(study_user_id,activated_at desc,id)`.
- **Vận hành:** Private.
- **Lỗi đặc thù:** —.
- **Màn hình/consumer:** `SCR-STU-013`.
- **Capability/use case:** `CAP-STU-002`, `UC-STU-002`.
