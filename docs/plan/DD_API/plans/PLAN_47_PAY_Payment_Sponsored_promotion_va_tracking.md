# PLAN 47 — Payment — Sponsored promotion và tracking
...
|---|---|
| Plan ID | `PLAN-47` |
| API trong phạm vi | `API-PAY-010`, `API-PAY-011`, `API-PAY-012`, `API-PAY-013` |
| Số API | 4 |
...
Tạo DD cho tạo sponsored job/profile promotion và ghi impression/click.

**Luồng nghiệp vụ trọng tâm:** Validate entitlement/budget/resource eligibility; placement stored separate from organic score; impression/click dedupe and privacy-safe telemetry.

**Điểm cần khóa:** Label bắt buộc, maker-checker, no ranking manipulation, event dedupe.

**Dependency:** Plans 22,27,37.
...
- `API-PAY-011` — `POST /api/v1/me/profile-promotions`.
- `API-PAY-012` — `POST /api/v1/promotions/{promotionId}/impressions`.
- `API-PAY-013` — `POST /api/v1/promotions/{promotionId}/clicks`.
...
- Screens: `SCR-WRK-013`, `SCR-WRK-022`, `SCR-WRK-034`, `SCR-WRK-043`.
- Acceptance: `TC-WRK-009`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L384`.
- **Actor/quyền:** `PERM-WRK-060` + entitlement.
- **Input/validation → output:** Kind SPONSORED_JOB, job, budget credit, dates → promotion.
- **Xử lý và dữ liệu:** TX validate published job, label policy, entitlement reserve; organic score field absent from write model.
- **Vận hành:** Idempotency; audit/event.
- **Lỗi đặc thù:** `PROMOTION_ENTITLEMENT_REQUIRED`, `JOB_NOT_PUBLISHED`, `DATE_RANGE_INVALID`.
- **Màn hình/consumer:** `SCR-WRK-043`.
- **Capability/use case:** `CAP-WRK-009`, `UC-WRK-009`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L385`.
- **Actor/quyền:** Candidate opt-in + entitlement.
- **Input/validation → output:** Published profile preview, period → sponsored profile promotion.
- **Xử lý và dữ liệu:** Requires search consent active; reserve credit; immediate stop/deindex on consent opt-out.
- **Vận hành:** Idempotency; consent/audit.
- **Lỗi đặc thù:** `SEARCH_CONSENT_REQUIRED`, `PROMOTION_ENTITLEMENT_REQUIRED`.
- **Màn hình/consumer:** `SCR-WRK-022`.
- **Capability/use case:** `CAP-WRK-009`, `UC-WRK-009`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L386`.
- **Actor/quyền:** Public client with signed placement token.
- **Input/validation → output:** Placement token → accepted.
- **Xử lý và dữ liệu:** Validate signature/expiry; dedupe browser-session+placement; append metric, no PII.
- **Vận hành:** 202; high-volume rate.
- **Lỗi đặc thù:** `PLACEMENT_TOKEN_INVALID`.
- **Màn hình/consumer:** Job/search screens.
- **Capability/use case:** `CAP-WRK-009`, `UC-WRK-009`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L387`.
- **Actor/quyền:** Public client with signed placement token.
- **Input/validation → output:** Placement token → accepted.
- **Xử lý và dữ liệu:** Same as impression; redirect target allowlist; append click.
- **Vận hành:** 202; dedupe/rate.
- **Lỗi đặc thù:** `PLACEMENT_TOKEN_INVALID`.
- **Màn hình/consumer:** Job/search screens.
- **Capability/use case:** `CAP-WRK-009`, `UC-WRK-009`.
