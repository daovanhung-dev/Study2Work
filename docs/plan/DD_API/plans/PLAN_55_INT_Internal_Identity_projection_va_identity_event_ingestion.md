# PLAN 55 — Internal — Identity projection và identity event ingestion
...
|---|---|
| Plan ID | `PLAN-55` |
| API trong phạm vi | `API-INT-001`, `API-INT-006`, `API-INT-007` |
| Số API | 3 |
...
Tạo DD internal cho projection lookup và event consumers ở Work/Study.

**Luồng nghiệp vụ trọng tâm:** mTLS+service JWT+signature/replay; projection read by subject; event ingestion unique eventId/version, stale/duplicate no-op, update local account status/authVersion and audit.

**Điểm cần khóa:** No cross-DB query, monotonic version, replay protection, suspension propagation.

**Dependency:** IAM Plans 2,5,6.

## 3. Phạm vi

**In-scope**

- `API-INT-001` — `GET identity /internal/v1/users/{platformUserId}/projection`.
- `API-INT-006` — `POST work /internal/v1/identity-events`.
- `API-INT-007` — `POST study /internal/v1/identity-events`.
...
- Screens: `SCR-IAM-006`, `SCR-OPS-024`, `SCR-OPS-025`, `SCR-OPS-021`.
- Acceptance: `TC-OPS-002`, `TC-OPS-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L439`.
- **Actor/quyền:** Study/Work service JWT`aud=identity-internal`.
- **Input/validation → output:** Subject + optional known version → minimal status/authVersion/roles/email hash/version.
- **Xử lý và dữ liệu:** Read by PK; used only catch-up when signed event missing; PII minimized; audit caller.
- **Vận hành:** Read by PK; used only catch-up when signed event missing; PII minimized; audit caller.
- **Lỗi đặc thù:** `SUBJECT_NOT_FOUND`, `SERVICE_NOT_AUTHORIZED`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L444`.
- **Actor/quyền:** Identity dispatcher.
- **Input/validation → output:** Signed CloudEvents batch <=100.
- **Xử lý và dữ liệu:** Inbox unique eventId; apply subject projection monotonically by sourceVersion; suspension/deletion blocks APIs and enqueues session/privacy actions.
- **Vận hành:** Inbox unique eventId; apply subject projection monotonically by sourceVersion; suspension/deletion blocks APIs and enqueues session/privacy actions.
- **Lỗi đặc thù:** `EVENT_GAP` triggers catch-up; duplicate/stale no-op.
- **Màn hình/consumer:** SYSTEM.
- **Capability/use case:** `CAP-OPS-002`, `UC-OPS-002`, `CAP-OPS-003`, `UC-OPS-003`.
...
- **Nguồn contract:** `04_DAC_TA_API.md:L445`.
- **Actor/quyền:** Identity dispatcher.
- **Input/validation → output:** Same.
- **Xử lý và dữ liệu:** Same monotonic projection; never creates credential.
- **Vận hành:** Same monotonic projection; never creates credential.
- **Lỗi đặc thù:** Same.
- **Màn hình/consumer:** SYSTEM.
- **Capability/use case:** `CAP-OPS-002`, `UC-OPS-002`, `CAP-OPS-003`, `UC-OPS-003`.
