# Overview - LEARN-PATH-002

| Field | Value |
|---|---|
| API code | `LEARN-PATH-002` |
| API name | `Enroll or assign a learning path and seed initial progress` |
| Module | `LEARNING` |
| Method | `POST` |
...
API này dùng để enroll or assign a learning path and seed initial progress.. Bản DD này là draft từ BD, API checklist và diagrams; chi tiết chưa đủ nguồn được ghi thành `OPEN_QUESTION`.
...
- Tuân thủ rule/focus: Eligibility, uniqueness, idempotency, `learning_path_enrollment` source of access.
...
| API checklist | `docs/checklists/API.md` | `LEARN-PATH-002` canonical row. |
| Activity diagram | `docs/diagrams/02_activity/05_Enroll_va_hoc_lesson.puml` | Flow hành vi liên quan. |
| Sequence diagram | `docs/diagrams/04_sequence/05_Enroll_Learning_Path.puml` | Tương tác runtime liên quan. |
...
- Thành công: response trả envelope với `businessCode` `LEARN-PATH-002-SUCCESS`.
...
|---|---:|---:|---|---|
| `learning_path_enrollment` | Y | Y | `LEARNING` | Lấy từ API checklist và BD data dictionary. |
...
|---|---|---|---|---|
| `LearningPathEnrolled` | Sau khi transaction nguồn commit. | after_commit/outbox/Celery | Consumer liên quan. | Retry theo worker/outbox policy. |
