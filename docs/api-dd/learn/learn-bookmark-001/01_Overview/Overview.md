# Overview - LEARN-BOOKMARK-001

| Field | Value |
|---|---|
| API code | `LEARN-BOOKMARK-001` |
| API name | `Toggle or upsert a lesson bookmark idempotently` |
| Module | `LEARNING` |
| Method | `PUT` |
| Endpoint | `/api/v1/lessons/{lessonId}/bookmark` |
| Primary actor | `Student` |
| Caller app | `Study client phù hợp với actor Student` |
| Auth scheme | `Bearer JWT.` |
| Permission | `learn.learn.bookmark` |
| Status | `DRAFT` |
| Version | `v0.1` |
| Updated | `2026-07-02` |
...
API này dùng để toggle or upsert a lesson bookmark idempotently.. Bản DD này là draft từ BD, API checklist và diagrams; chi tiết chưa đủ nguồn được ghi thành `OPEN_QUESTION`.
...
- Tuân thủ rule/focus: Enforce bookmark uniqueness `(user_id, lesson_id)` and enrollment visibility.
...
| API checklist | `docs/checklists/API.md` | `LEARN-BOOKMARK-001` canonical row. |
...
- Thành công: response trả envelope với `businessCode` `LEARN-BOOKMARK-001-SUCCESS`.
...
|---|---|---|
| `Student` | `learn.learn.bookmark` | Actor chỉ được xem/ghi dữ liệu của chính mình hoặc scope học tập/team được cấp quyền. |

## Data Ownership

| Table / aggregate | Read | Write | Owner module | Note |
|---|---:|---:|---|---|
| `bookmark` | Y | Y | `LEARNING` | Lấy từ API checklist và BD data dictionary. |
...
|---|---|---|---|
| LEARN-BOOKMARK-001-OQ-001 | BA/PO + Tech Lead | OPEN | Xác nhận schema request/response chi tiết trước khi chuyển DD sang IN_REVIEW hoặc APPROVED. |
