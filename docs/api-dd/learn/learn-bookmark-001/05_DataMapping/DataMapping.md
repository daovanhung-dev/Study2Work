# Data Mapping - LEARN-BOOKMARK-001
...
| 4 | Application | Load entity/read model cần thiết. | Repository ports | bookmark | E404 | N |
...
|---|---|---|---|---|---|
| `bookmark` | read/write | id, status, owner/scope, timestamps, business fields cần thiết | bookmark_repository | inside transaction for writes | Cột chi tiết cần review khi APPROVED. |
...
handler(command_or_query, actor_context):
    ...
    if decision.has_writes:
        with transaction:
            ...
            audit_repository.append(trace_id, actor_context, "LEARN-BOOKMARK-001")
...
|---|---|---|---|---|---|---|
| Audit | `LEARN-BOOKMARK-001` | Khi API thành công hoặc bị từ chối bởi rule quan trọng. | traceId, actorId, apiCode, resourceId | transaction/log sink | audit viewer | N/A |
...
|---|---|---|
| `LEARN-BOOKMARK-001-TC-001` | Happy path đúng actor và scope hợp lệ. | Success envelope với `LEARN-BOOKMARK-001-SUCCESS`. |
| `LEARN-BOOKMARK-001-TC-002` | Validation failure. | `422` với safe field error và không ghi partial state. |
| `LEARN-BOOKMARK-001-TC-003` | Missing/invalid auth khi endpoint cần auth. | `401` với `AUTH-RESP-UNAUTHORIZED`. |
| `LEARN-BOOKMARK-001-TC-004` | Permission hoặc ownership/scope bị từ chối. | `403` và không mutation. |
| `LEARN-BOOKMARK-001-TC-005` | Resource không tồn tại hoặc không visible. | `404` với safe message. |
| `LEARN-BOOKMARK-001-TC-006` | State conflict, duplicate hoặc concurrency conflict. | `409` và state cũ được giữ nguyên. |
| `LEARN-BOOKMARK-001-TC-007` | Dependency/system failure. | Safe error envelope có `traceId`; log không lộ secret/PII. |
