# Overview - LEARN-LIVE-001

| Field | Value |
|---|---|
| API code | `LEARN-LIVE-001` |
| API name | `List live sessions within learner or mentor scope` |
| Module | `LEARNING` |
| Method | `GET` |
...
API này dùng để list live sessions within learner or mentor scope.. Bản DD này là draft từ BD, API checklist và diagrams; chi tiết chưa đủ nguồn được ghi thành `OPEN_QUESTION`.
...
- Tuân thủ rule/focus: Enrollment/scope filtering, pagination, visibility of meeting URL.
...
| API checklist | `docs/checklists/API.md` | `LEARN-LIVE-001` canonical row. |
...
- Thành công: response trả envelope với `businessCode` `LEARN-LIVE-001-SUCCESS`.
...
|---|---|---|
| `Student/Mentor` | `learn.learn.live` | Actor chỉ được xem/ghi dữ liệu của chính mình hoặc scope học tập/team được cấp quyền. |

## Data Ownership

| Table / aggregate | Read | Write | Owner module | Note |
|---|---:|---:|---|---|
| `live_session` | Y | N | `LEARNING` | Lấy từ API checklist và BD data dictionary. |
...
|---|---|---|---|
| LEARN-LIVE-001-OQ-001 | BA/PO + Tech Lead | OPEN | Xác nhận schema request/response chi tiết trước khi chuyển DD sang IN_REVIEW hoặc APPROVED. |
