# Data Mapping - MENTOR-REVIEW-001
...
| 4 | Application | Load entity/read model cần thiết. | Repository ports | review, feedback, rubric, skill_assessment, audit_log | E404 | N |
...
handler(command_or_query, actor_context):
    ...
    if decision.has_writes:
        with transaction:
            ...
            audit_repository.append(trace_id, actor_context, "MENTOR-REVIEW-001")
...
|---|---|---|---|---|---|---|
| Audit | `MENTOR-REVIEW-001` | Khi API thành công hoặc bị từ chối bởi rule quan trọng. | traceId, actorId, apiCode, resourceId | transaction/log sink | audit viewer | N/A |
| Event/job | `SubmissionReviewed` | Sau commit thành công. | traceId, actorId, resourceId, occurredAt | outbox/Celery | consumer liên quan | Retry/DLQ theo worker policy |
| Event/job | `SkillLevelUpdated` | Sau commit thành công. | traceId, actorId, resourceId, occurredAt | outbox/Celery | consumer liên quan | Retry/DLQ theo worker policy |

## Tests Derived From Mapping

| Test ID | Scenario | Expected result |
|---|---|---|
| `MENTOR-REVIEW-001-TC-001` | Happy path đúng actor và scope hợp lệ. | Success envelope với `MENTOR-REVIEW-001-SUCCESS`. |
| `MENTOR-REVIEW-001-TC-002` | Validation failure. | `422` với safe field error và không ghi partial state. |
| `MENTOR-REVIEW-001-TC-003` | Missing/invalid auth khi endpoint cần auth. | `401` với `AUTH-RESP-UNAUTHORIZED`. |
| `MENTOR-REVIEW-001-TC-004` | Permission hoặc ownership/scope bị từ chối. | `403` và không mutation. |
| `MENTOR-REVIEW-001-TC-005` | Resource không tồn tại hoặc không visible. | `404` với safe message. |
| `MENTOR-REVIEW-001-TC-006` | State conflict, duplicate hoặc concurrency conflict. | `409` và state cũ được giữ nguyên. |
| `MENTOR-REVIEW-001-TC-007` | Dependency/system failure. | Safe error envelope có `traceId`; log không lộ secret/PII. |
