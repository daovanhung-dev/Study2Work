# Overview - MENTOR-REVIEW-001

| Field | Value |
|---|---|
| API code | `MENTOR-REVIEW-001` |
| API name | `Create a rubric-based review for an assigned submission` |
| Module | `MENTOR` |
| Method | `POST` |
| Endpoint | `/api/v1/submissions/{submissionId}/reviews` |
...
API này dùng để create a rubric-based review for an assigned submission.. Bản DD này là draft từ BD, API checklist và diagrams; chi tiết chưa đủ nguồn được ghi thành `OPEN_QUESTION`.
...
- Tuân thủ rule/focus: Enforce `BR-MENTOR-001`, `BR-MENTOR-002`, `BR-ASSESS-004`; rubric version and required criteria.
...
| API checklist | `docs/checklists/API.md` | `MENTOR-REVIEW-001` canonical row. |
...
- Thành công: response trả envelope với `businessCode` `MENTOR-REVIEW-001-SUCCESS`.
...
| `rubric` | Y | Y | `MENTOR` | Lấy từ API checklist và BD data dictionary. |
| `skill_assessment` | Y | Y | `MENTOR` | Lấy từ API checklist và BD data dictionary. |
...
|---|---|---|---|---|
| `SubmissionReviewed` | Sau khi transaction nguồn commit. | after_commit/outbox/Celery | Consumer liên quan. | Retry theo worker/outbox policy. |
| `SkillLevelUpdated` | Sau khi transaction nguồn commit. | after_commit/outbox/Celery | Consumer liên quan. | Retry theo worker/outbox policy. |

## Open Questions

| ID | Owner | Status | Question |
|---|---|---|---|
| MENTOR-REVIEW-001-OQ-001 | BA/PO + Tech Lead | OPEN | Xác nhận schema request/response chi tiết trước khi chuyển DD sang IN_REVIEW hoặc APPROVED. |
