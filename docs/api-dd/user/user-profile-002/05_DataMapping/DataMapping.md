...
| 4 | Application | Load entity/read model cần thiết. | Repository ports | profile, student_profile, mentor_profile, audit_log | E404 | N |
...
|---|---|---|---|---|---|
| `profile` | read/write | id, status, owner/scope, timestamps, business fields cần thiết | profile_repository | inside transaction for writes | Cột chi tiết cần review khi APPROVED. |
| `student_profile` | read/write | id, status, owner/scope, timestamps, business fields cần thiết | student_profile_repository | inside transaction for writes | Cột chi tiết cần review khi APPROVED. |
| `mentor_profile` | read/write | id, status, owner/scope, timestamps, business fields cần thiết | mentor_profile_repository | inside transaction for writes | Cột chi tiết cần review khi APPROVED. |
...
handler(command_or_query, actor_context):
    ...
    if decision.has_writes:
        with transaction:
            ...
            audit_repository.append(trace_id, actor_context, "USER-PROFILE-002")
...
|---|---|---|---|---|---|---|
| Audit | `USER-PROFILE-002` | Khi API thành công hoặc bị từ chối bởi rule quan trọng. | traceId, actorId, apiCode, resourceId | transaction/log sink | audit viewer | N/A |
...
|---|---|---|
| `USER-PROFILE-002-TC-001` | Happy path đúng actor và scope hợp lệ. | Success envelope với `USER-PROFILE-002-SUCCESS`. |
| `USER-PROFILE-002-TC-002` | Validation failure. | `422` với safe field error và không ghi partial state. |
| `USER-PROFILE-002-TC-003` | Missing/invalid auth khi endpoint cần auth. | `401` với `AUTH-RESP-UNAUTHORIZED`. |
| `USER-PROFILE-002-TC-004` | Permission hoặc ownership/scope bị từ chối. | `403` và không mutation. |
| `USER-PROFILE-002-TC-005` | Resource không tồn tại hoặc không visible. | `404` với safe message. |
| `USER-PROFILE-002-TC-006` | State conflict, duplicate hoặc concurrency conflict. | `409` và state cũ được giữ nguyên. |
| `USER-PROFILE-002-TC-007` | Dependency/system failure. | Safe error envelope có `traceId`; log không lộ secret/PII. |
