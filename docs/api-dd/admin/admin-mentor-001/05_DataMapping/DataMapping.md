...
| 4 | Application | Load entity/read model cần thiết. | Repository ports | mentor_profile, audit_log | E404 | N |
...
handler(command_or_query, actor_context):
    ...
    if decision.has_writes:
        with transaction:
            ...
            audit_repository.append(trace_id, actor_context, "ADMIN-MENTOR-001")
...
|---|---|---|---|---|---|---|
| Audit | `ADMIN-MENTOR-001` | Khi API thành công hoặc bị từ chối bởi rule quan trọng. | traceId, actorId, apiCode, resourceId | transaction/log sink | audit viewer | N/A |
...
|---|---|---|
| `ADMIN-MENTOR-001-TC-001` | Happy path đúng actor và scope hợp lệ. | Success envelope với `ADMIN-MENTOR-001-SUCCESS`. |
| `ADMIN-MENTOR-001-TC-002` | Validation failure. | `422` với safe field error và không ghi partial state. |
| `ADMIN-MENTOR-001-TC-003` | Missing/invalid auth khi endpoint cần auth. | `401` với `AUTH-RESP-UNAUTHORIZED`. |
| `ADMIN-MENTOR-001-TC-004` | Permission hoặc ownership/scope bị từ chối. | `403` và không mutation. |
| `ADMIN-MENTOR-001-TC-005` | Resource không tồn tại hoặc không visible. | `404` với safe message. |
| `ADMIN-MENTOR-001-TC-006` | State conflict, duplicate hoặc concurrency conflict. | `409` và state cũ được giữ nguyên. |
| `ADMIN-MENTOR-001-TC-007` | Dependency/system failure. | Safe error envelope có `traceId`; log không lộ secret/PII. |
