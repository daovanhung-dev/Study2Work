...
| Field | Value |
|---|---|
| API code | `AUTH-LOGOUT-001` |
| API name | `Revoke current session/refresh token` |
| Module | `AUTH` |
| Method | `POST` |
...
API này dùng để revoke current session/refresh token.. Bản DD này là draft từ BD, API checklist và diagrams; chi tiết chưa đủ nguồn được ghi thành `OPEN_QUESTION`.
...
- Tuân thủ rule/focus: Authenticated actor must only revoke own session unless admin flow is separately approved.
...
| API checklist | `docs/checklists/API.md` | `AUTH-LOGOUT-001` canonical row. |
...
- Thành công: response trả envelope với `businessCode` `AUTH-LOGOUT-001-SUCCESS`.
...
|---|---|---|
| `Student/Mentor/Admin` | `auth.auth.logout` | Actor chỉ được xem/ghi dữ liệu của chính mình hoặc scope học tập/team được cấp quyền. |
...
|---|---|---|---|
| AUTH-LOGOUT-001-OQ-001 | BA/PO + Tech Lead | OPEN | Xác nhận schema request/response chi tiết trước khi chuyển DD sang IN_REVIEW hoặc APPROVED. |
