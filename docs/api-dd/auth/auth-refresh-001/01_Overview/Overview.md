# Overview - AUTH-REFRESH-001

| Field | Value |
|---|---|
| API code | `AUTH-REFRESH-001` |
| API name | `Rotate refresh/access token for an active session` |
| Module | `AUTH` |
| Method | `POST` |
...
API này dùng để rotate refresh/access token for an active session.. Bản DD này là draft từ BD, API checklist và diagrams; chi tiết chưa đủ nguồn được ghi thành `OPEN_QUESTION`.

## Study Scope

In scope:

- Thực hiện đúng capability Study của module `AUTH` cho actor `Student/Mentor/Admin`.
- Tuân thủ rule/focus: Refresh token hash lookup, revocation, expiry, rotation replay protection and one-time-use semantics.
...
| API checklist | `docs/checklists/API.md` | `AUTH-REFRESH-001` canonical row. |
...
- Thành công: response trả envelope với `businessCode` `AUTH-REFRESH-001-SUCCESS`.
...
|---|---|---|
| `Student/Mentor/Admin` | `auth.auth.refresh` | Actor chỉ được xem/ghi dữ liệu của chính mình hoặc scope học tập/team được cấp quyền. |
...
|---|---|---|---|
| AUTH-REFRESH-001-OQ-001 | BA/PO + Tech Lead | OPEN | Xác nhận schema request/response chi tiết trước khi chuyển DD sang IN_REVIEW hoặc APPROVED. |
