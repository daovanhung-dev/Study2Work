# Overview - AUTH-VERIFY-001

| Field | Value |
|---|---|
| API code | `AUTH-VERIFY-001` |
| API name | `Validate verification token and activate a pending account` |
| Module | `AUTH` |
| Method | `POST` |
...
API này dùng để validate verification token and activate a pending account.. Bản DD này là draft từ BD, API checklist và diagrams; chi tiết chưa đủ nguồn được ghi thành `OPEN_QUESTION`.
...
- Tuân thủ rule/focus: Token expiry, already-used token, `PENDING -> ACTIVE` transition and safe replay behavior must be explicit.
...
| API checklist | `docs/checklists/API.md` | `AUTH-VERIFY-001` canonical row. |
...
- Thành công: response trả envelope với `businessCode` `AUTH-VERIFY-001-SUCCESS`.
...
|---|---|---|---|---|
| `EmailVerified` | Sau khi transaction nguồn commit. | after_commit/outbox/Celery | Consumer liên quan. | Retry theo worker/outbox policy. |

## Open Questions

| ID | Owner | Status | Question |
|---|---|---|---|
| AUTH-VERIFY-001-OQ-001 | BA/PO + Tech Lead | OPEN | Xác nhận schema request/response chi tiết trước khi chuyển DD sang IN_REVIEW hoặc APPROVED. |
