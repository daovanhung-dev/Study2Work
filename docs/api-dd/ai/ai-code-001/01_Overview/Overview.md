# Overview - AI-CODE-001

| Field | Value |
|---|---|
| API code | `AI-CODE-001` |
| API name | `Explain code or debugging concept from authorized context` |
| Module | `AI` |
| Method | `POST` |
| Endpoint | `/api/v1/ai/code-explanations` |
| Primary actor | `Student/Mentor` |
| Caller app | `Study client phù hợp với actor Student/Mentor` |
| Auth scheme | `Bearer JWT.` |
| Permission | `ai.ai.code` |
| Status | `DRAFT` |
| Version | `v0.1` |
| Updated | `2026-07-02` |
...
API này dùng để explain code or debugging concept from authorized context.. Bản DD này là draft từ BD, API checklist và diagrams; chi tiết chưa đủ nguồn được ghi thành `OPEN_QUESTION`.

## Study Scope

In scope:

- Thực hiện đúng capability Study của module `AI` cho actor `Student/Mentor`.
- Tuân thủ rule/focus: Secret/PII redaction, prompt retention, provider failure/retry, output disclaimer.
...
| API checklist | `docs/checklists/API.md` | `AI-CODE-001` canonical row. |
| Activity diagram | `docs/diagrams/02_activity/14_AI_roadmap_code_explanation.puml` | Flow hành vi liên quan. |
| Sequence diagram | `docs/diagrams/04_sequence/18_AI_Code_Explanation.puml` | Tương tác runtime liên quan. |
...
- Thành công: response trả envelope với `businessCode` `AI-CODE-001-SUCCESS`.
...
|---|---|---|
| `Student/Mentor` | `ai.ai.code` | Actor chỉ được xem/ghi dữ liệu của chính mình hoặc scope học tập/team được cấp quyền. |
...
| `code_explanation_request` | Y | Y | `AI` | Lấy từ API checklist và BD data dictionary. |
...
|---|---|---|---|
| AI-CODE-001-OQ-001 | BA/PO + Tech Lead | OPEN | Xác nhận schema request/response chi tiết trước khi chuyển DD sang IN_REVIEW hoặc APPROVED. |
