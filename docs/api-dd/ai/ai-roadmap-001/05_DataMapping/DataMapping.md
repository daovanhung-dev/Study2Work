...
| 4 | Application | Load entity/read model cần thiết. | Repository ports | ai_request, roadmap_suggestion, ai_insight | E404 | N |
...
|---|---|---|---|---|---|
| `ai_request` | read/write | id, status, owner/scope, timestamps, business fields cần thiết | ai_request_repository | inside transaction for writes | Cột chi tiết cần review khi APPROVED. |
| `roadmap_suggestion` | read/write | id, status, owner/scope, timestamps, business fields cần thiết | roadmap_suggestion_repository | inside transaction for writes | Cột chi tiết cần review khi APPROVED. |
| `ai_insight` | read/write | id, status, owner/scope, timestamps, business fields cần thiết | ai_insight_repository | inside transaction for writes | Cột chi tiết cần review khi APPROVED. |
...
handler(command_or_query, actor_context):
    ...
    if decision.has_writes:
        with transaction:
            ...
            audit_repository.append(trace_id, actor_context, "AI-ROADMAP-001")
...
|---|---|---|---|---|---|---|
| Audit | `AI-ROADMAP-001` | Khi API thành công hoặc bị từ chối bởi rule quan trọng. | traceId, actorId, apiCode, resourceId | transaction/log sink | audit viewer | N/A |
...
|---|---|---|
| `AI-ROADMAP-001-TC-001` | Happy path đúng actor và scope hợp lệ. | Success envelope với `AI-ROADMAP-001-SUCCESS`. |
| `AI-ROADMAP-001-TC-002` | Validation failure. | `422` với safe field error và không ghi partial state. |
| `AI-ROADMAP-001-TC-003` | Missing/invalid auth khi endpoint cần auth. | `401` với `AUTH-RESP-UNAUTHORIZED`. |
| `AI-ROADMAP-001-TC-004` | Permission hoặc ownership/scope bị từ chối. | `403` và không mutation. |
| `AI-ROADMAP-001-TC-005` | Resource không tồn tại hoặc không visible. | `404` với safe message. |
| `AI-ROADMAP-001-TC-006` | State conflict, duplicate hoặc concurrency conflict. | `409` và state cũ được giữ nguyên. |
| `AI-ROADMAP-001-TC-007` | Dependency/system failure. | Safe error envelope có `traceId`; log không lộ secret/PII. |
