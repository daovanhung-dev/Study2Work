# Overview - PROJECT-TASK-002

| Field | Value |
|---|---|
| API code | `PROJECT-TASK-002` |
| API name | `Transition task status and optionally record work evidence` |
| Module | `PROJECT` |
| Method | `PATCH` |
...
API này dùng để transition task status and optionally record work evidence.. Bản DD này là draft từ BD, API checklist và diagrams; chi tiết chưa đủ nguồn được ghi thành `OPEN_QUESTION`.
...
- Tuân thủ rule/focus: Enforce `BR-PROJECT-002`, optimistic concurrency and project/team scope.
...
| API checklist | `docs/checklists/API.md` | `PROJECT-TASK-002` canonical row. |
...
- Thành công: response trả envelope với `businessCode` `PROJECT-TASK-002-SUCCESS`.
...
| `work_log` | Y | Y | `PROJECT` | Lấy từ API checklist và BD data dictionary. |
...
|---|---|---|---|---|
| `TaskStatusChanged` | Sau khi transaction nguồn commit. | after_commit/outbox/Celery | Consumer liên quan. | Retry theo worker/outbox policy. |

## Open Questions

| ID | Owner | Status | Question |
|---|---|---|---|
| PROJECT-TASK-002-OQ-001 | BA/PO + Tech Lead | OPEN | Xác nhận schema request/response chi tiết trước khi chuyển DD sang IN_REVIEW hoặc APPROVED. |
