# Data Mapping - PROJECT-SUBMIT-001
...
| 4 | Application | Load entity/read model cần thiết. | Repository ports | project_submission, file_asset, audit_log | E404 | N |
...
|---|---|---|---|---|---|
| `project_submission` | read/write | id, status, owner/scope, timestamps, business fields cần thiết | project_submission_repository | inside transaction for writes | Cột chi tiết cần review khi APPROVED. |
| `file_asset` | read/write | id, status, owner/scope, timestamps, business fields cần thiết | file_asset_repository | inside transaction for writes | Cột chi tiết cần review khi APPROVED. |
...
handler(command_or_query, actor_context):
    ...
    if decision.has_writes:
        with transaction:
            ...
            audit_repository.append(trace_id, actor_context, "PROJECT-SUBMIT-001")
...
|---|---|---|---|---|---|---|
| Audit | `PROJECT-SUBMIT-001` | Khi API thành công hoặc bị từ chối bởi rule quan trọng. | traceId, actorId, apiCode, resourceId | transaction/log sink | audit viewer | N/A |
| Event/job | `ProjectSubmitted` | Sau commit thành công. | traceId, actorId, resourceId, occurredAt | outbox/Celery | consumer liên quan | Retry/DLQ theo worker policy |

## Tests Derived From Mapping

| Test ID | Scenario | Expected result |
|---|---|---|
| `PROJECT-SUBMIT-001-TC-001` | Happy path đúng actor và scope hợp lệ. | Success envelope với `PROJECT-SUBMIT-001-SUCCESS`. |
| `PROJECT-SUBMIT-001-TC-002` | Validation failure. | `422` với safe field error và không ghi partial state. |
| `PROJECT-SUBMIT-001-TC-003` | Missing/invalid auth khi endpoint cần auth. | `401` với `AUTH-RESP-UNAUTHORIZED`. |
| `PROJECT-SUBMIT-001-TC-004` | Permission hoặc ownership/scope bị từ chối. | `403` và không mutation. |
| `PROJECT-SUBMIT-001-TC-005` | Resource không tồn tại hoặc không visible. | `404` với safe message. |
| `PROJECT-SUBMIT-001-TC-006` | State conflict, duplicate hoặc concurrency conflict. | `409` và state cũ được giữ nguyên. |
| `PROJECT-SUBMIT-001-TC-007` | Dependency/system failure. | Safe error envelope có `traceId`; log không lộ secret/PII. |
