# Overview - PROJECT-TASK-001

| Field | Value |
|---|---|
| API code | `PROJECT-TASK-001` |
| API name | `Create a project task within project/sprint boundary` |
| Module | `PROJECT` |
| Method | `POST` |
...
| Permission | `project.project.task` |
| Status | `DRAFT` |
| Version | `v0.1` |
| Updated | `2026-07-02` |
...
API này dùng để create a project task within project/sprint boundary.. Bản DD này là draft từ BD, API checklist và diagrams; chi tiết chưa đủ nguồn được ghi thành `OPEN_QUESTION`.
...
- Tuân thủ rule/focus: Enforce `BR-PROJECT-003`; assignee/team scope and sprint boundary.
...
| API checklist | `docs/checklists/API.md` | `PROJECT-TASK-001` canonical row. |
...
- Thành công: response trả envelope với `businessCode` `PROJECT-TASK-001-SUCCESS`.
...
|---|---|---|
| `Student/Mentor` | `project.project.task` | Actor chỉ được xem/ghi dữ liệu của chính mình hoặc scope học tập/team được cấp quyền. |

## Data Ownership

| Table / aggregate | Read | Write | Owner module | Note |
|---|---:|---:|---|---|
| `task` | Y | Y | `PROJECT` | Lấy từ API checklist và BD data dictionary. |
...
|---|---|---|---|---|
| `TaskAssigned` | Sau khi transaction nguồn commit. | after_commit/outbox/Celery | Consumer liên quan. | Retry theo worker/outbox policy. |

## Open Questions

| ID | Owner | Status | Question |
|---|---|---|---|
| PROJECT-TASK-001-OQ-001 | BA/PO + Tech Lead | OPEN | Xác nhận schema request/response chi tiết trước khi chuyển DD sang IN_REVIEW hoặc APPROVED. |
| PROJECT-TASK-001-OQ-003 | BA/PO | OPEN | Xác nhận điều kiện phát sinh side effect tùy chọn trong flow này. |
