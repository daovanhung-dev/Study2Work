# Overview - PROJECT-PR-001

| Field | Value |
|---|---|
| API code | `PROJECT-PR-001` |
| API name | `Record verified PR review evidence against a task` |
| Module | `PROJECT` |
| Method | `POST` |
| Endpoint | `/api/v1/tasks/{taskId}/pull-request-reviews` |
| Primary actor | `Student/Mentor` |
| Caller app | `Study client phù hợp với actor Student/Mentor` |
| Auth scheme | `Bearer JWT.` |
| Permission | `project.project.pr` |
| Status | `DRAFT` |
| Version | `v0.1` |
| Updated | `2026-07-02` |
...
API này dùng để record verified PR review evidence against a task.. Bản DD này là draft từ BD, API checklist và diagrams; chi tiết chưa đủ nguồn được ghi thành `OPEN_QUESTION`.
...
- Tuân thủ rule/focus: Verify Git provider data; do not trust webhook/client raw data; task/project scope.
...
| API checklist | `docs/checklists/API.md` | `PROJECT-PR-001` canonical row. |
| Activity diagram | `docs/diagrams/02_activity/13_Git_PR_project_submission.puml` | Flow hành vi liên quan. |
| Sequence diagram | `docs/diagrams/04_sequence/15_Task_PR_Review.puml` | Tương tác runtime liên quan. |
...
- Thành công: response trả envelope với `businessCode` `PROJECT-PR-001-SUCCESS`.
...
|---|---|---|
| `Student/Mentor` | `project.project.pr` | Actor chỉ được xem/ghi dữ liệu của chính mình hoặc scope học tập/team được cấp quyền. |

## Data Ownership

| Table / aggregate | Read | Write | Owner module | Note |
|---|---:|---:|---|---|
| `pull_request_review` | Y | Y | `PROJECT` | Lấy từ API checklist và BD data dictionary. |
| `task_comment` | Y | Y | `PROJECT` | Lấy từ API checklist và BD data dictionary. |
...
|---|---|---|---|
| PROJECT-PR-001-OQ-001 | BA/PO + Tech Lead | OPEN | Xác nhận schema request/response chi tiết trước khi chuyển DD sang IN_REVIEW hoặc APPROVED. |
