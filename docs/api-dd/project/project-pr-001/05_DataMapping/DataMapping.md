...
| 4 | Application | Load entity/read model cần thiết. | Repository ports | pull_request_review, task_comment, audit_log | E404 | N |
...
|---|---|---|---|---|---|
| `pull_request_review` | read/write | id, status, owner/scope, timestamps, business fields cần thiết | pull_request_review_repository | inside transaction for writes | Cột chi tiết cần review khi APPROVED. |
| `task_comment` | read/write | id, status, owner/scope, timestamps, business fields cần thiết | task_comment_repository | inside transaction for writes | Cột chi tiết cần review khi APPROVED. |
...
handler(command_or_query, actor_context):
    ...
    if decision.has_writes:
        with transaction:
            ...
            audit_repository.append(trace_id, actor_context, "PROJECT-PR-001")
...
|---|---|---|---|---|---|---|
| Audit | `PROJECT-PR-001` | Khi API thành công hoặc bị từ chối bởi rule quan trọng. | traceId, actorId, apiCode, resourceId | transaction/log sink | audit viewer | N/A |
...
|---|---|---|
| `PROJECT-PR-001-TC-001` | Happy path đúng actor và scope hợp lệ. | Success envelope với `PROJECT-PR-001-SUCCESS`. |
| `PROJECT-PR-001-TC-002` | Validation failure. | `422` với safe field error và không ghi partial state. |
| `PROJECT-PR-001-TC-003` | Missing/invalid auth khi endpoint cần auth. | `401` với `AUTH-RESP-UNAUTHORIZED`. |
| `PROJECT-PR-001-TC-004` | Permission hoặc ownership/scope bị từ chối. | `403` và không mutation. |
| `PROJECT-PR-001-TC-005` | Resource không tồn tại hoặc không visible. | `404` với safe message. |
| `PROJECT-PR-001-TC-006` | State conflict, duplicate hoặc concurrency conflict. | `409` và state cũ được giữ nguyên. |
| `PROJECT-PR-001-TC-007` | Dependency/system failure. | Safe error envelope có `traceId`; log không lộ secret/PII. |
