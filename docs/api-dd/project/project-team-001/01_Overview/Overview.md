# Overview - PROJECT-TEAM-001

| Field | Value |
|---|---|
| API code | `PROJECT-TEAM-001` |
| API name | `Create a team for a project according to project policy` |
| Module | `PROJECT` |
| Method | `POST` |
| Endpoint | `/api/v1/projects/{projectId}/teams` |
...
API này dùng để create a team for a project according to project policy.. Bản DD này là draft từ BD, API checklist và diagrams; chi tiết chưa đủ nguồn được ghi thành `OPEN_QUESTION`.
...
- Tuân thủ rule/focus: Enforce project availability, team policy and member role rules.
...
| API checklist | `docs/checklists/API.md` | `PROJECT-TEAM-001` canonical row. |
...
- Thành công: response trả envelope với `businessCode` `PROJECT-TEAM-001-SUCCESS`.
...
|---|---|---|
| `Student` | `project.project.team` | Actor chỉ được xem/ghi dữ liệu của chính mình hoặc scope học tập/team được cấp quyền. |

## Data Ownership

| Table / aggregate | Read | Write | Owner module | Note |
|---|---:|---:|---|---|
| `project` | Y | Y | `PROJECT` | Lấy từ API checklist và BD data dictionary. |
| `team` | Y | Y | `PROJECT` | Lấy từ API checklist và BD data dictionary. |
...
|---|---|---|---|---|
| `TeamCreated` | Sau khi transaction nguồn commit. | after_commit/outbox/Celery | Consumer liên quan. | Retry theo worker/outbox policy. |

## Open Questions

| ID | Owner | Status | Question |
|---|---|---|---|
| PROJECT-TEAM-001-OQ-001 | BA/PO + Tech Lead | OPEN | Xác nhận schema request/response chi tiết trước khi chuyển DD sang IN_REVIEW hoặc APPROVED. |
