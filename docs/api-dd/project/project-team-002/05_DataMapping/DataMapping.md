...
| 4 | Application | Load entity/read model cần thiết. | Repository ports | team_member, audit_log | E404 | N |
...
handler(command_or_query, actor_context):
    ...
    if decision.has_writes:
        with transaction:
            ...
            audit_repository.append(trace_id, actor_context, "PROJECT-TEAM-002")
...
|---|---|---|---|---|---|---|
| Audit | `PROJECT-TEAM-002` | Khi API thành công hoặc bị từ chối bởi rule quan trọng. | traceId, actorId, apiCode, resourceId | transaction/log sink | audit viewer | N/A |
| Event/job | `TeamMemberJoined` | Sau commit thành công. | traceId, actorId, resourceId, occurredAt | outbox/Celery | consumer liên quan | Retry/DLQ theo worker policy |

## Tests Derived From Mapping

| Test ID | Scenario | Expected result |
|---|---|---|
| `PROJECT-TEAM-002-TC-001` | Happy path đúng actor và scope hợp lệ. | Success envelope với `PROJECT-TEAM-002-SUCCESS`. |
| `PROJECT-TEAM-002-TC-002` | Validation failure. | `422` với safe field error và không ghi partial state. |
| `PROJECT-TEAM-002-TC-003` | Missing/invalid auth khi endpoint cần auth. | `401` với `AUTH-RESP-UNAUTHORIZED`. |
| `PROJECT-TEAM-002-TC-004` | Permission hoặc ownership/scope bị từ chối. | `403` và không mutation. |
| `PROJECT-TEAM-002-TC-005` | Resource không tồn tại hoặc không visible. | `404` với safe message. |
| `PROJECT-TEAM-002-TC-006` | State conflict, duplicate hoặc concurrency conflict. | `409` và state cũ được giữ nguyên. |
| `PROJECT-TEAM-002-TC-007` | Dependency/system failure. | Safe error envelope có `traceId`; log không lộ secret/PII. |
