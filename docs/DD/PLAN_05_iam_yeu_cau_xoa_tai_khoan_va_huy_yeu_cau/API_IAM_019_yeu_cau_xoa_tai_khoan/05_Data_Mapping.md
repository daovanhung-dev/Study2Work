...
- `Body.confirmText`: required=`Yes`, type=`string`, format=`typed confirmation`, valid=`N/A`; basis=`DERIVED`.
...
|---|---|---|
| `users.status` | `DELETION_PENDING` | `DIRECT` |
| `users.deletion_requested_at` | `current_timestamp` | `DIRECT` |
...
| `auth_sessions.revoke_reason` | `ACCOUNT_DELETION_REQUESTED` | `DIRECT` |
