...
|---|---|---|---|
| `identity_db` | `principal/resource scope` | `Authenticated` | Không dùng frontend guard làm hàng rào bảo mật |
...
- `Body.token`: required=`Yes`, type=`string`, format=`opaque token`, valid=`N/A`; basis=`DIRECT`.
...
### 2.1. Query `TBL-IAM-004 — email_verification_tokens`

| Target table | Column get | Chú thích | Remarks |
|---|---|---|---|
| `email_verification_tokens AS t_emailver` | `t_emailver.id` | `id` | `DIRECT` |
| `email_verification_tokens AS t_emailver` | `t_emailver.user_id` | `user_id` | `DIRECT` |
| `email_verification_tokens AS t_emailver` | `t_emailver.email_id` | `email_id` | `DIRECT` |
| `email_verification_tokens AS t_emailver` | `t_emailver.purpose` | `purpose` | `DIRECT` |
| `email_verification_tokens AS t_emailver` | `t_emailver.token_hash` | `token_hash` | `DIRECT` |
| `email_verification_tokens AS t_emailver` | `t_emailver.status` | `status` | `DIRECT` |
| `email_verification_tokens AS t_emailver` | `t_emailver.expires_at` | `expires_at` | `DIRECT` |
| `email_verification_tokens AS t_emailver` | `t_emailver.consumed_at` | `consumed_at` | `DIRECT` |
| `email_verification_tokens AS t_emailver` | `t_emailver.revoked_at` | `revoked_at` | `DIRECT` |

**WHERE / predicate cho `email_verification_tokens`**
...
### 2.4. Query `TBL-IAM-009 — auth_sessions`
...
### 2.5. Query `TBL-IAM-010 — refresh_tokens`
...
- `TBL-IAM-004 — email_verification_tokens` operation `UPDATE`.
- `TBL-IAM-002 — user_emails` operation `UPDATE`.
...
- `UPDATE email_verification_tokens`: refer [07_email_verification_tokens_update.md](./07_email_verification_tokens_update.md).
- `UPDATE user_emails`: refer [08_user_emails_update.md](./08_user_emails_update.md).
- `UPDATE auth_sessions`: refer [09_auth_sessions_update.md](./09_auth_sessions_update.md).
- `UPDATE refresh_tokens`: refer [10_refresh_tokens_update.md](./10_refresh_tokens_update.md).
...
|---|---|---|
| `email_verification_tokens.status` | `CONSUMED` | `DIRECT` |
| `email_verification_tokens.consumed_at` | `current_timestamp` | `DIRECT` |
| `user_emails.is_primary` | `true for verified new email; false for old primary` | `DIRECT` |
| `user_emails.verified_at` | `current_timestamp for new email` | `DIRECT` |
| `user_emails.replaced_at` | `current_timestamp for old primary` | `DIRECT` |
| `user_emails.updated_at` | `current_timestamp` | `DIRECT` |
| `user_emails.row_version` | `row_version + 1` | `DIRECT` |
| `auth_sessions.status` | `REVOKED` | `DIRECT` |
| `auth_sessions.revoked_at` | `current_timestamp` | `DIRECT` |
| `auth_sessions.revoke_reason` | `EMAIL_CHANGED` | `DIRECT` |
