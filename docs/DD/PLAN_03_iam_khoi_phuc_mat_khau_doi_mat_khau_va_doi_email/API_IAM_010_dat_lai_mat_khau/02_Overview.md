...
| Purpose | `Token + password mới → success` |
...
| Side effects | `Audit theo nguồn endpoint; payload phải redact PII/secret.; Outbox/event `identity.password.changed`; payload/version còn Q-15 nếu chưa có schema.` |
...
- Contract summary: TX + L token/user; policy/history check; update Argon2id; consume token; revoke all sessions; increment authVersion.
...
- Idempotency; event`identity.password.changed`; security audit
