...
| Purpose | `Current + new password → success` |
| Consumer/Actor | `Authenticated + current password/MFA` |
| Authentication | `Bearer access token ES256; kiểm iss, aud, exp, nbf, jti, sid, authVersion theo quy ước chung.` |
| Authorization | `Authenticated + current password/MFA` |
...
- Contract summary: TX L credential; verify current; update hash; revoke other sessions; authVersion++.
...
- Idempotency; event/audit password changed
