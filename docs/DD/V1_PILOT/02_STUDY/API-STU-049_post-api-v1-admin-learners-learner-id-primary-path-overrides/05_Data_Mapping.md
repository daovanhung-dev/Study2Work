...
format: "markdown"
dd_id: "API-STU-049"
api_name: "POST /api/v1/admin/learners/{learnerId}/primary-path-overrides"
...
| `learnerId` | Endpoint URI | S01 | Path parsing | Resource lookup | N/A | N/A | N/A |
| `bypassCooldown` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| Phiên bản đích,  kiểu Boolean, lý do >=20 | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | kỳ | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-STU-009` + MFA.
...
- Endpoint input contract: Phiên bản đích, `bypassCooldown` kiểu Boolean, lý do >=20 → kỳ.

<a id="s04"></a>
### S04. Execute source-defined processing

> Cùng khóa/bất biến như API-STU-014; chỉ bỏ qua khi có quyền rõ ràng; luôn tạo thời gian chờ 168 giờ mới

- Operations/observability contract: Khóa lặp; sự kiện/kiểm toán rủi ro cao
