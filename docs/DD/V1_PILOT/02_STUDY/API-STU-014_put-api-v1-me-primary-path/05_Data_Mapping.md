...
| `pathVersionId` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| kỳ/phiên bản hiện hành dự kiến, lý do tùy chọn | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | kỳ mới đang hoạt động | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Người học đang hoạt động + đã hoàn tất thiết lập ban đầu.
...
- Endpoint input contract: `pathVersionId`, kỳ/phiên bản hiện hành dự kiến, lý do tùy chọn → kỳ mới đang hoạt động.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX khóa tư vấn theo người dùng; kiểm xuất bản; lần chọn đầu hoặc áp dụng thời gian chờ 168 giờ theo UTC; đóng kỳ cũ thành `SWITCHED_OUT`; chèn kỳ mới `ACTIVE`; giữ nguyên mọi ghi danh/tiến độ

- Operations/observability contract: Khóa lặp bắt buộc; `study.primary_path.changed`; kiểm toán; không lưu đệm
