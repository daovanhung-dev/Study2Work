...
| ---: | --- |
| HTTP method | `POST` |
| URI | `/api/v1/enterprises/{enterpriseId}/jobs` |
...
| ---: | --- | --- | --- |
| 1 | Tiêu đề, chủ sở hữu, JD có cấu trúc đầu tiên | Physical key, location, type or rule | `SOURCE_REQUIRED`; not included in JSON example. |

## Contract source

> Tiêu đề, chủ sở hữu, JD có cấu trúc đầu tiên → việc làm ổn định + bản sửa đổi `DRAFT`
