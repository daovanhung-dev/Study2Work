...
| Bộ lọc/từ khóa/trang | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | ứng viên đã che dữ liệu + giải thích đối sánh tự nhiên; hồ sơ tài trợ riêng/gắn nhãn | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-WRK-020` + quyền tìm kiếm đang hoạt động.
...
- Endpoint input contract: Bộ lọc/từ khóa/trang → ứng viên đã che dữ liệu + giải thích đối sánh tự nhiên; hồ sơ tài trợ riêng/gắn nhãn.

<a id="s04"></a>
### S04. Execute source-defined processing

> Tư cách thành viên tổ chức lấy ở máy chủ; chỉ truy vấn đồng ý `ACTIVE`/chưa hết hạn và điều kiện hiển thị CSDL; tài liệu tìm kiếm không có liên hệ, CV gốc, minh chứng Study, trường được bảo vệ/nhạy cảm. Chèn tài trợ tách khỏi điểm tự nhiên

- Operations/observability contract: Kiểm toán ID truy vấn/bộ lọc/kết quả; 60/phút; không dùng bộ nhớ đệm chung
