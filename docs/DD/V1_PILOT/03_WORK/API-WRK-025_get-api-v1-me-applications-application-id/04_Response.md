...
| 4 | `data` | Response data | `data` | `object \| array \| null` | Yes | N/A | N/A | [S06](./05_Data_Mapping.md#s06) | ảnh chụp dữ liệu, trạng thái/lịch sử hiện tại, phỏng vấn, minh chứng `PENDING`/`READY`/`UNAVAILABLE`/`HIDDEN`/`REVOKED`, ID cuộc hội thoại | SOURCE_REQUIRED for nested shape | Catalog output phrase retained below. |
...
> ảnh chụp dữ liệu, trạng thái/lịch sử hiện tại, phỏng vấn, minh chứng `PENDING`/`READY`/`UNAVAILABLE`/`HIDDEN`/`REVOKED`, ID cuộc hội thoại
...
{
  "success": false,
  "businessCode": "APPLICATION_NOT_FOUND",
  "message": "<SOURCE_REQUIRED>",
  "data": null,
  "meta": {
    "fieldErrors": []
