...
format: "markdown"
dd_id: "API-WRK-062"
api_name: "POST /api/v1/enterprises/{enterpriseId}/interviews/{id}/status-transitions"
...
| đích //, lý do/phản hồi | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | phỏng vấn | SOURCE_REQUIRED for nested shape | API catalog |
...
- Endpoint input contract: `scheduleVersion`, đích `CANCELLED`/`NO_SHOW`/`COMPLETED`, lý do/phản hồi → phỏng vấn.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX áp dụng thời gian/trạng thái; `NO_SHOW` chỉ sau bắt đầu+thời gian ân hạn; phản hồi hoàn tất có thể nội bộ; ứng viên từ chối khác với hủy

- Operations/observability contract: Khóa lặp; kiểm toán/thông báo
