...
| `before` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| Con trỏ , giới hạn | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | lịch sử và bản ghi che theo thứ tự | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Ứng viên chủ sở hữu hoặc nhà tuyển dụng được phân công.
...
- Endpoint input contract: Con trỏ `before`, giới hạn → lịch sử và bản ghi che theo thứ tự.

<a id="s04"></a>
### S04. Execute source-defined processing

> Kiểm quyền người tham gia + đơn ứng tuyển của cuộc hội thoại; truy vấn `(conversation_id,sequence_no desc)`; REST là nguồn dữ liệu chuẩn

- Operations/observability contract: Riêng tư, không lưu; dùng con trỏ
