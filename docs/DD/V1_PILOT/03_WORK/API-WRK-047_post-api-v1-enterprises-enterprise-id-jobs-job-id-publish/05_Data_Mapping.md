...
format: "markdown"
dd_id: "API-WRK-047"
api_name: "POST /api/v1/enterprises/{enterpriseId}/jobs/{jobId}/publish"
...
| Bản sửa đổi, ID duyệt/cấp | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | `PUBLISHED` | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-WRK-013`; cấp tin cậy hoặc duyệt đã chấp thuận.
...
- Endpoint input contract: Bản sửa đổi, ID duyệt/cấp, `If-Match` → `PUBLISHED`.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX khóa việc làm/bản sửa đổi; kiểm lượt duyệt hiện hành đã chấp thuận hoặc cấp tin cậy đang hiệu lực theo thời hạn, chính sách tự động, tổ chức/quyền sử dụng; xuất bản bản sửa đổi bất biến, thay bản hiện hành cũ; cấp tin cậy không bao giờ bỏ qua kiểm tra

- Operations/observability contract: Khóa lặp; hộp thư đi xuất bản/lập chỉ mục tìm kiếm/kiểm toán
