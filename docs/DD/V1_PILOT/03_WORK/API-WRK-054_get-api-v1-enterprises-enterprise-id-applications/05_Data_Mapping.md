...
format: "markdown"
dd_id: "API-WRK-054"
api_name: "GET /api/v1/enterprises/{enterpriseId}/applications"
...
| Trang/việc làm/trạng thái/người được giao/ngày/ | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | hàng ATS | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-WRK-030`.
...
- Endpoint input contract: Trang/việc làm/trạng thái/người được giao/ngày/`q` → hàng ATS.

<a id="s04"></a>
### S04. Execute source-defined processing

> Điều kiện tổ chức qua `application.enterprise_id`; chỉ mục ghép; `q` chỉ tìm trường an toàn trong ảnh chụp

- Operations/observability contract: Riêng tư; xuất dữ liệu cần quyền riêng
