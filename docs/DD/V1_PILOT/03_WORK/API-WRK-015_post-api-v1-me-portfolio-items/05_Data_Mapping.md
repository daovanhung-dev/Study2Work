...
| Loại /, tiêu đề, URL HTTPS hoặc ID tệp , chế độ hiển thị | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | mục | SOURCE_REQUIRED for nested shape | API catalog |
...
- Endpoint input contract: Loại `LINK`/`FILE`, tiêu đề, URL HTTPS hoặc ID tệp `CLEAN`, chế độ hiển thị → mục.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX kiểm chính xác một tải tin; liên kết chỉ kiểm cú pháp, máy chủ không tự tải; kiểm chủ sở hữu/tình trạng `CLEAN` của tệp
