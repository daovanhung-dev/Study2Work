...
format: "markdown"
dd_id: "API-WRK-032"
api_name: "POST /api/v1/applications/{id}/conversation/messages"
...
| `replyTo` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| ID tin nhắn ứng dụng khách, văn bản thuần 1–5000 ký tự,  tùy chọn | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | tin nhắn/trình tự được chấp nhận | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Ứng viên chủ sở hữu hoặc nhà tuyển dụng đang được phân công.
...
- Endpoint input contract: ID tin nhắn ứng dụng khách, văn bản thuần 1–5000 ký tự, `replyTo` tùy chọn → tin nhắn/trình tự được chấp nhận.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX khóa cuộc hội thoại; kiểm đơn chưa kết thúc/cuộc hội thoại có thể ghi/phân công; làm sạch và kiểm ký tự điều khiển; khử trùng lặp ID ứng dụng khách; chèn tin `TEXT` + hộp thư đi; phát WebSocket sau cam kết. V1 không hỗ trợ tệp/HTML trong trò chuyện; chỉ tiến trình nền tạo tin `SYSTEM`

- Operations/observability contract: Khóa lặp bắt buộc; xác nhận SLO <=2 giây; kiểm toán siêu dữ liệu kiểm duyệt; 30/phút
