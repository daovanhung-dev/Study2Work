...
| Văn bản <=5000, mức hiển thị , tệp sạch tùy chọn | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | ghi chú | SOURCE_REQUIRED for nested shape | API catalog |
...
- Endpoint input contract: Văn bản <=5000, mức hiển thị `INTERNAL`, tệp sạch tùy chọn → ghi chú.

<a id="s04"></a>
### S04. Execute source-defined processing

> Điều kiện tổ chức/phân công; làm sạch nội dung, lưu lịch sử chỉnh sửa/xóa theo kiểm toán; không bao giờ hiển thị cho ứng viên

- Operations/observability contract: Xử lý lặp an toàn; kiểm toán PII
