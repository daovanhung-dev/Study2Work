...
format: "markdown"
dd_id: "API-WRK-045"
api_name: "PATCH /api/v1/enterprises/{enterpriseId}/jobs/{jobId}/revisions/{revisionId}"
...
| `revisionId` | Endpoint URI | S01 | Path parsing | Resource lookup | N/A | N/A | N/A |
| JD có cấu trúc, kỹ năng, lương, địa điểm, câu hỏi | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | bản nháp | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Tác giả/`PERM-WRK-011`.
...
- Endpoint input contract: JD có cấu trúc, kỹ năng, lương, địa điểm, câu hỏi, `If-Match` → bản nháp.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX khóa bản sửa đổi; chỉ `DRAFT`; kiểm khoảng lương, danh mục, hết hạn, nội dung bị cấm, Markdown đã làm sạch

- Operations/observability contract: Tự lưu; kiểm toán chênh lệch; chưa lập chỉ mục đến khi xuất bản
