...
format: "markdown"
dd_id: "API-WRK-038"
api_name: "POST /api/v1/enterprises/{enterpriseId}/verification-submissions"
...
| Thông tin pháp lý, ID tài liệu riêng tư , sự đồng ý | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | hồ sơ `SUBMITTED` | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-WRK-002`.
...
- Endpoint input contract: Thông tin pháp lý, ID tài liệu riêng tư `CLEAN`, sự đồng ý → hồ sơ `SUBMITTED`.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX kiểm chủ sở hữu/tệp; thêm ảnh chụp xác minh; tổ chức giữ `PENDING_VERIFICATION` cho đến khi quản trị viên đặt `VERIFIED` hoặc `REJECTED`

- Operations/observability contract: Xử lý lặp an toàn; hàng đợi người duyệt/kiểm toán
