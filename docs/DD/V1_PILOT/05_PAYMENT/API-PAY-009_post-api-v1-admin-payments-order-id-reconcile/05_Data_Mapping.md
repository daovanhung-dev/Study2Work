...
format: "markdown"
dd_id: "API-PAY-009"
api_name: "POST /api/v1/admin/payments/{orderId}/reconcile"
...
| Tham chiếu minh chứng nhà cung cấp, thao tác /, lý do | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | vụ việc | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-PAY-003` + MFA.
...
- Endpoint input contract: Tham chiếu minh chứng nhà cung cấp, thao tác `REQUERY`/`MARK_REVIEWED`, lý do → vụ việc.

<a id="s04"></a>
### S04. Execute source-defined processing

> Không bao giờ tự đặt `SETTLED` nếu không có bản ghi nhà cung cấp đã xác minh; thêm lượt/kết quả đối soát

- Operations/observability contract: Khóa lặp; kiểm toán tài chính
