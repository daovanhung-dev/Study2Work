...
format: "markdown"
dd_id: "API-PAY-016"
api_name: "GET /api/v1/billing/returns/{provider}"
...
| Chỉ tham chiếu/đơn nhà cung cấp | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | tải tin trang trạng thái đã làm sạch | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Chuyển hướng trình duyệt.
...
- Endpoint input contract: Chỉ tham chiếu/đơn nhà cung cấp → tải tin trang trạng thái đã làm sạch.

<a id="s04"></a>
### S04. Execute source-defined processing

> Không bao giờ tin truy vấn thành công; tra đơn nội bộ cho chủ sở hữu đã xác thực hoặc nonce trả về mờ; trả `PENDING` đến khi webhook/đối soát

- Operations/observability contract: Không cấp quyền sử dụng; không lưu
