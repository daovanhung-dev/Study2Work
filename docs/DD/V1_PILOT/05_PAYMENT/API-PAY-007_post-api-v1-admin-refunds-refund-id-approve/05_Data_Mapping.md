...
format: "markdown"
dd_id: "API-PAY-007"
api_name: "POST /api/v1/admin/refunds/{refundId}/approve"
...
| `refundId` | Endpoint URI | S01 | Path parsing | Resource lookup | N/A | N/A | N/A |
| Số tiền/lý do/ | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | hoàn tiền đang xử lý | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-PAY-002` + MFA gần đây, không phải người yêu cầu.
...
- Endpoint input contract: Số tiền/lý do/`If-Match` → hoàn tiền đang xử lý.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX phân tách nhiệm vụ; giữ số tiền có thể hoàn; thêm quyết định; gọi bộ điều hợp nhà cung cấp sau cam kết

- Operations/observability contract: Khóa lặp bắt buộc; kiểm toán tài chính bất biến
