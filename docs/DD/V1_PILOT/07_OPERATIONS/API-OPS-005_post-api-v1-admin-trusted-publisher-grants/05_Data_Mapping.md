...
| `validFrom` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| `validUntil` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| Chủ thể/tổ chức, miền /, phạm vi, /, minh chứng, lý do | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | cấp quyền | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-OPS-003` + MFA gần đây.
...
- Endpoint input contract: Chủ thể/tổ chức, miền `STUDY`/`WORK`, phạm vi, `validFrom`/`validUntil`, minh chứng, lý do → cấp quyền.

<a id="s04"></a>
### S04. Execute source-defined processing

> Kiểm chỉ số đủ điều kiện hoặc yêu cầu `bootstrapPilot=true` + lý do; thêm quyền cấp; tách người cấp khỏi người nhận

- Operations/observability contract: Khóa lặp; sự kiện/kiểm toán bảo mật
