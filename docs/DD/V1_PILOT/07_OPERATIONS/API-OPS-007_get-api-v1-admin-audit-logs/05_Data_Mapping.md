...
format: "markdown"
dd_id: "API-OPS-007"
api_name: "GET /api/v1/admin/audit-logs"
...
| Con trỏ, dịch vụ, tác nhân, đích, hành động, ngày <=31 ngày | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | luồng đã che dữ liệu | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Quyền kiểm toán theo phạm vi + MFA.
...
- Endpoint input contract: Con trỏ, dịch vụ, tác nhân, đích, hành động, ngày <=31 ngày → luồng đã che dữ liệu.

<a id="s04"></a>
### S04. Execute source-defined processing

> Chỉ truy vấn kiểm toán của từng dịch vụ; UI gọi riêng máy chủ dịch vụ; `(occurred_at,id)` bất biến; truy cập trường PII theo phạm vi

- Operations/observability contract: Không lưu đệm; mọi truy vấn/xuất đều kiểm toán
