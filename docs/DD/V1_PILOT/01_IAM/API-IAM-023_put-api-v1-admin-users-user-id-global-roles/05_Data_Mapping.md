...
| `userId` | Endpoint URI | S01 | Path parsing | Resource lookup | N/A | N/A | N/A |
| Tập mã vai trò chính xác, lý do | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | vai trò/phiên bản | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-IAM-003` + MFA gần đây.
...
- Endpoint input contract: Tập mã vai trò chính xác, lý do, `If-Match` → vai trò/phiên bản.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX khóa người dùng; tính chênh lệch phân công; cấm xóa quản trị viên nền tảng cuối cùng; tăng `authVersion`; ghi hộp thư đi

- Operations/observability contract: Khóa lặp; kiểm toán đầy đủ; không dùng bộ nhớ đệm
