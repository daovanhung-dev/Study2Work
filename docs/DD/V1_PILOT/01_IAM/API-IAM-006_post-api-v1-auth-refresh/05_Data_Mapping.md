...
| Mã làm mới luân phiên gốc | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | cặp mã mới | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Cookie/mã làm mới.
...
- Endpoint input contract: Mã làm mới luân phiên gốc → cặp mã mới.

<a id="s04"></a>
### S04. Execute source-defined processing

> Tra cứu hàm băm; TX + L hàng mã làm mới/phiên; kiểm hết hạn/thu hồi; đặt `consumed_at`, chèn mã thay thế và liên kết `rotated_to_id`. Nếu mã đã được dùng: thu hồi toàn bộ họ mã và tăng `authVersion`

- Operations/observability contract: Không thử lại bằng mã cũ; sự kiện bảo mật `identity.refresh.reuse_detected`; 30/phút/phiên
