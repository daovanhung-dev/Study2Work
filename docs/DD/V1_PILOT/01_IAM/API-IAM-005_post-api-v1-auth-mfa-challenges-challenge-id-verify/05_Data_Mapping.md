...
| `challengeId` | Endpoint URI | S01 | Path parsing | Resource lookup | N/A | N/A | N/A |
| TOTP 6 số hoặc mã khôi phục | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | mã truy cập/mã làm mới | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Người dùng có thử thách.
...
- Endpoint input contract: TOTP 6 số hoặc mã khôi phục → mã truy cập/mã làm mới.

<a id="s04"></a>
### S04. Execute source-defined processing

> Khóa thử thách; so sánh hàm băm; tối đa 5 lần; đánh dấu đã dùng; tạo phiên và họ mã làm mới; mã khôi phục chỉ dùng một lần

- Operations/observability contract: Khóa lặp bắt buộc; kiểm toán; 10/15 phút/thử thách
