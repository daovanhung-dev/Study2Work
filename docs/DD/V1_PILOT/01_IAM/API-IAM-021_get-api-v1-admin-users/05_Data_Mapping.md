...
format: "markdown"
dd_id: "API-IAM-021"
api_name: "GET /api/v1/admin/users"
...
| Trang/bộ lọc trạng thái/hàm băm email/vai trò | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | người dùng đã che dữ liệu | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-IAM-001`.
...
- Endpoint input contract: Trang/bộ lọc trạng thái/hàm băm email/vai trò → người dùng đã che dữ liệu.

<a id="s04"></a>
### S04. Execute source-defined processing

> Truy vấn chỉ mục `(status,created_at,id)` hoặc chỉ mục email chuẩn hóa chính xác; xem PII cần quyền riêng

- Operations/observability contract: Không dùng bộ nhớ đệm chung; kiểm toán tìm kiếm; 60/phút
