...
| `token` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| 32–512 ký tự | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | mã truy cập/mã làm mới và tài khoản `ACTIVE` | SOURCE_REQUIRED for nested shape | API catalog |
...
- Endpoint input contract: `token` 32–512 ký tự → mã truy cập/mã làm mới và tài khoản `ACTIVE`.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX + L hàm băm mã thông báo `FOR UPDATE`; kiểm `purpose`/hết hạn/chưa dùng; đánh dấu đã xác minh/trạng thái, dùng mã, tạo phiên/mã làm mới, hộp thư đi

- Operations/observability contract: Khóa lặp bắt buộc; sự kiện `identity.user.verified`; 10/phút/IP
