...
format: "markdown"
dd_id: "API-IAM-019"
api_name: "POST /api/v1/me/deletion-requests"
...
| Văn bản xác nhận, lý do tùy chọn | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | `graceEndsAt` | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Đã xác thực + xác minh tăng cường.
...
- Endpoint input contract: Văn bản xác nhận, lý do tùy chọn → `graceEndsAt`.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX khóa người dùng; đặt trạng thái `DELETION_PENDING`; thu hồi phiên; tạo hộp thư đi cho Study/Work; giữ thời hạn hủy 30 ngày

- Operations/observability contract: Khóa lặp; `identity.deletion.requested`; kiểm toán
