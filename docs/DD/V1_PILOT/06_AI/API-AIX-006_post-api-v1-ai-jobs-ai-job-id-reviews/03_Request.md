...
| ---: | --- |
| HTTP method | `POST` |
| URI | `/api/v1/ai-jobs/{aiJobId}/reviews` |
...
| ---: | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `aiJobId` | `aiJobId` | `string` | Yes | N/A | N/A | Path segment | SOURCE_REQUIRED | N/A | DIRECT from URI. | [S01](./05_Data_Mapping.md#s01) |
...
| ---: | --- | --- | --- |
| 1 | Quyết định //, đầu ra/lý do đã sửa | Physical key, location, type or rule | `SOURCE_REQUIRED`; not included in JSON example. |

## Contract source

> Quyết định `ACCEPTED`/`EDITED_ACCEPT`/`REJECTED`, đầu ra/lý do đã sửa, `If-Match` → lượt duyệt
