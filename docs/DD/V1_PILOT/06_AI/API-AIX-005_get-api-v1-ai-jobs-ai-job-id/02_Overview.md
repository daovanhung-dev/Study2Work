...
| Module | `AI` |
| Method | `GET` |
| Endpoint | `/api/v1/ai-jobs/{aiJobId}` |
| Purpose | tác vụ `QUEUED`/`RUNNING`/`SUCCEEDED`/`FAILED`/`CANCELLED`; khi `SUCCEEDED` có duyệt đầu ra `DRAFT`/`ACCEPTED`/`EDITED_ACCEPT`/`REJECTED`/`EXPIRED`, bản nháp đầu ra/nguồn gốc |
| Consumer/Actor | Người yêu cầu/thành viên tổ chức được quyền |
| Authentication | Người yêu cầu/thành viên tổ chức được quyền |
| Authorization | Người yêu cầu/thành viên tổ chức được quyền |
...
| Side effects | Riêng tư, không lưu; thăm dò 1/2 giây |

## Sources

- [04_DAC_TA_API.md](../../../../BD/04_DAC_TA_API.md): `L409` — endpoint contract.
...
- [OQ-AIX-EVALUATION-KILL-SWITCH](../../OPEN_QUESTIONS.md#oq-aix-evaluation-kill-switch).
