...
| 4 | `data` | Response data | `data` | `object \| array \| null` | Yes | N/A | N/A | [S06](./05_Data_Mapping.md#s06) | tác vụ `QUEUED`/`RUNNING`/`SUCCEEDED`/`FAILED`/`CANCELLED`; khi `SUCCEEDED` có duyệt đầu ra `DRAFT`/`ACCEPTED`/`EDITED_ACCEPT`/`REJECTED`/`EXPIRED`, bản nháp đầu ra/nguồn gốc | SOURCE_REQUIRED for nested shape | Catalog output phrase retained below. |
...
> tác vụ `QUEUED`/`RUNNING`/`SUCCEEDED`/`FAILED`/`CANCELLED`; khi `SUCCEEDED` có duyệt đầu ra `DRAFT`/`ACCEPTED`/`EDITED_ACCEPT`/`REJECTED`/`EXPIRED`, bản nháp đầu ra/nguồn gốc
