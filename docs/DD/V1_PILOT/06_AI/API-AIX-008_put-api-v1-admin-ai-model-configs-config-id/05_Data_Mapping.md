...
format: "markdown"
dd_id: "API-AIX-008"
api_name: "PUT /api/v1/admin/ai/model-configs/{configId}"
...
| `configId` | Endpoint URI | S01 | Path parsing | Resource lookup | N/A | N/A | N/A |
| Tham số nhà cung cấp/mô hình | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | cấu hình có phiên bản | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-AIX-002` + MFA.
...
- Endpoint input contract: Tham số nhà cung cấp/mô hình, `If-Match` → cấu hình có phiên bản.

<a id="s04"></a>
### S04. Execute source-defined processing

> Bí mật được tham chiếu qua trình quản lý bí mật, không nằm trong phần thân/CSDL; kiểm thử xác nhận ngoài TX; kích hoạt tách riêng

- Operations/observability contract: Khóa lặp; kiểm toán
