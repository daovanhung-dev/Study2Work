...
format: "markdown"
dd_id: "API-INT-006"
api_name: "POST work /internal/v1/identity-events"
...
| Lô CloudEvents đã ký <=100 | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
- Actor/authorization source: Bộ điều phối Identity.
...
- Endpoint input contract: Lô CloudEvents đã ký <=100.

<a id="s04"></a>
### S04. Execute source-defined processing

> Hộp thư đến có `eventId` duy nhất; áp dụng bản chiếu chủ thể đơn điệu theo `sourceVersion`; đình chỉ/xóa chặn API và đưa tác vụ phiên/riêng tư vào hàng đợi
