...
|---|---|---|---|
| `identity_db` | `principal/resource scope` | `Owner` | Không dùng frontend guard làm hàng rào bảo mật |
...
- `Path.id`: required=`Yes`, type=`uuid`, format=`UUID v7`, valid=`N/A`; basis=`DIRECT`.
- `Body.code`: required=`Yes`, type=`string`, format=`numeric`, valid=`N/A`; basis=`DERIVED`.
...
### 2.1. Query `TBL-IAM-006 — mfa_methods`
...
### 2.2. Query `TBL-IAM-007 — mfa_recovery_codes`
...
- `TBL-IAM-006 — mfa_methods` operation `UPDATE`.
- `TBL-IAM-007 — mfa_recovery_codes` operation `INSERT`.
...
- `UPDATE mfa_methods`: refer [07_mfa_methods_update.md](./07_mfa_methods_update.md).
- `INSERT mfa_recovery_codes`: refer [08_mfa_recovery_codes_insert.md](./08_mfa_recovery_codes_insert.md).
...
|---|---|---|
| `mfa_methods.verified_at` | `current_timestamp` | `DIRECT` |
| `mfa_methods.updated_at` | `current_timestamp` | `DIRECT` |
| `mfa_methods.row_version` | `row_version + 1` | `DIRECT` |
| `mfa_recovery_codes.id` | `Generated UUID v7` | `DIRECT` |
| `mfa_recovery_codes.created_at` | `current_timestamp` | `DIRECT` |
| `mfa_recovery_codes.method_id` | `Path enrollment ID` | `DIRECT` |
| `mfa_recovery_codes.code_hash` | `Hash each generated recovery code` | `DIRECT` |
| `mfa_recovery_codes.consumed_at` | `NULL` | `DIRECT` |
| `mfa_recovery_codes.batch_id` | `Generated batch UUID` | `DIRECT` |
