...
|---|---|---|---|
| `identity_db` | `principal/resource scope` | `Privileged user + step-up` | Không dùng frontend guard làm hàng rào bảo mật |
...
- `Body.method`: required=`Yes`, type=`string`, format=`N/A`, valid=`TOTP`; basis=`DIRECT`.
...
- `TBL-IAM-006 — mfa_methods` operation `INSERT`.
...
- `INSERT mfa_methods`: refer [07_mfa_methods_insert.md](./07_mfa_methods_insert.md).
...
|---|---|---|
| `mfa_methods.id` | `Generated UUID v7` | `DIRECT` |
| `mfa_methods.created_at` | `current_timestamp` | `DIRECT` |
| `mfa_methods.updated_at` | `current_timestamp` | `DIRECT` |
| `mfa_methods.row_version` | `1` | `DIRECT` |
| `mfa_methods.user_id` | `Authenticated users.id` | `DIRECT` |
| `mfa_methods.type` | `TOTP` | `DIRECT` |
| `mfa_methods.label` | `request.deviceLabel or NULL` | `DIRECT` |
| `mfa_methods.secret_ciphertext` | `KMS-envelope encrypted secret` | `DIRECT` |
| `mfa_methods.verified_at` | `NULL` | `DIRECT` |
| `mfa_methods.disabled_at` | `NULL` | `DIRECT` |
