...
|---|---|---|---|
| `study_db` | `principal/resource scope` | `Learner active + onboarding complete` | Không dùng frontend guard làm hàng rào bảo mật |
...
- `Body.pathVersionId`: required=`Yes`, type=`uuid`, format=`UUID v7`, valid=`N/A`; basis=`DIRECT`.
- `Body.expectedCurrentPeriodId`: required=`No`, type=`uuid`, format=`UUID v7`, valid=`N/A`; basis=`DERIVED`.
- `Body.expectedCurrentVersion`: required=`No`, type=`integer`, format=`integer`, valid=`N/A`; basis=`DERIVED`.
- `Body.reason`: required=`No`, type=`string`, format=`text`, valid=`N/A`; basis=`DIRECT`.
...
### 2.3. Query `TBL-STU-026 — primary_path_periods`
...
### 2.4. Query `TBL-STU-010 — learning_path_versions`
...
- `TBL-STU-026 — primary_path_periods` operation `UPDATE`.
- `TBL-STU-026 — primary_path_periods` operation `INSERT`.
...
- Dùng advisory lock theo learner và partial unique constraint để giữ tối đa một period `ACTIVE`.
- Q-12: không tạo course enrollment trong API này khi chưa có approved source.

### 3.3. Items update/insert

- `UPDATE primary_path_periods`: refer [07_primary_path_periods_update.md](./07_primary_path_periods_update.md).
- `INSERT primary_path_periods`: refer [08_primary_path_periods_insert.md](./08_primary_path_periods_insert.md).
...
|---|---|---|
| `primary_path_periods.status` | `SWITCHED_OUT` | `CONFLICT` |
| `primary_path_periods.ended_at` | `current_timestamp` | `CONFLICT` |
| `primary_path_periods.end_reason` | `request.reason or SELF_SWITCH` | `CONFLICT` |
| `primary_path_periods.updated_at` | `SOURCE_REQUIRED — table uses IMMUTABLE set` | `CONFLICT` |
| `primary_path_periods.row_version` | `SOURCE_REQUIRED — table uses IMMUTABLE set` | `CONFLICT` |
| `primary_path_periods.id` | `Generated UUID v7` | `DIRECT` |
| `primary_path_periods.created_at` | `current_timestamp` | `DIRECT` |
| `primary_path_periods.learner_id` | `Authenticated learner ID` | `DIRECT` |
| `primary_path_periods.path_version_id` | `request.pathVersionId` | `DIRECT` |
| `primary_path_periods.status` | `ACTIVE` | `DIRECT` |
| `primary_path_periods.started_at` | `current_timestamp` | `DIRECT` |
| `primary_path_periods.ended_at` | `NULL` | `DIRECT` |
| `primary_path_periods.end_reason` | `NULL` | `DIRECT` |
| `primary_path_periods.supersedes_period_id` | `Previous active period ID or NULL` | `DIRECT` |
| `primary_path_periods.switch_request_id` | `SOURCE_REQUIRED` | `DIRECT` |
| `primary_path_periods.selected_from_recommendation_id` | `SOURCE_REQUIRED or NULL` | `DIRECT` |
...
| `audit_events.action` | `PRIMARY_PATH_CHANGED` | `DERIVED` |
| `audit_events.resource_type` | `PRIMARY_PATH_PERIOD` | `DERIVED` |
| `audit_events.resource_id` | `new primary_path_periods.id` | `DERIVED` |
...
| `audit_events.changes` | `Reason and before/after IDs` | `DERIVED` |
