...
| 2 | Idempotency key | `Idempotency-Key` | Yes | Random/UUID `16–128` ký tự | Endpoint yêu cầu idempotency. | [1.1](./05_Data_Mapping.md#dm-1-1) |
...
| ---: | --- | --- | --- | ---: | ---: | ---: | --- | --- | --- | --- | --- | --- |
| 1 | Verification token | `token` | string | Yes | 32 | 512 | ASCII/URL-safe — exact alphabet SOURCE_REQUIRED | Opaque token | One-time registration token | N/A | Hash để lookup; không log/persist raw token. | [1.2](./05_Data_Mapping.md#dm-1-2) |

> Mỗi field nằm trên một row riêng. Token alphabet/hash algorithm chưa được source định nghĩa.

## Ví dụ Request data

```json
{
  "token": "SOURCE_REQUIRED_RAW_TOKEN"
}
```
