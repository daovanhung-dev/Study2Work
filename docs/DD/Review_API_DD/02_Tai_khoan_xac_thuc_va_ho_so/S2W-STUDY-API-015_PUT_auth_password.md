# Review API DD — S2W-STUDY-API-015

- DD nguồn: `docs/DD/Study2Work_DD_API/02_Tai_khoan_xac_thuc_va_ho_so/S2W-STUDY-API-015_PUT_auth_password.xlsx`
...
| P0 | `3.Data mapping!A12:H16`, các sheet DB | Change password lại map vào `password_reset_tokens`; còn đưa current/new/confirm password vào mapping ghi. Credential/session thuộc Identity. | `docs/BD/base/0. Study2Work_System_Architecture.md:61-75,150-220,294-301`; `docs/BD/diagram/CLASS/study2work_study_full_schema_seed.sql:260-281` | Xác minh current password tại Identity, hash password mới, update credential và revoke session theo policy; không persist/log raw password. |
| P0 | `2.Response!A15:H19` | `changed` và `sessions_revoked` dùng String thay vì Boolean/count; chưa nói revoke current hay other sessions. | `docs/BD/02. Study2Work_Study_BasicDesign_Tai_Khoan_Xac_Thuc_Ho_So.md:88-102` | Sửa type và chốt session policy, re-auth/step-up và audit. |
| P0 | `4.Error` | Thiếu wrong current password, password reuse/history, weak password, recent-auth required, lock/rate limit. | `docs/BD/02. Study2Work_Study_BasicDesign_Tai_Khoan_Xac_Thuc_Ho_So.md:37-145,163-186` | Thêm businessCode và kiểm soát abuse; dùng response không tiết lộ credential details. |
