...
- Kiểm permission `PERM-IAM-002` server-side.

### 0.2. Query/check quyền

| Target table/API | Column/Field | Condition/Value | Remarks |
|---|---|---|---|
| `identity_db` | `principal/resource scope` | ``PERM-IAM-002` + MFA` | Không dùng frontend guard làm hàng rào bảo mật |
...
- `Body.status`: required=`Yes`, type=`string`, format=`N/A`, valid=`ACTIVE|SUSPENDED`; basis=`DIRECT`.
...
### 2.3. Query `TBL-IAM-010 — refresh_tokens`
...
|---|---|---|
| `users.status` | `request.status` | `DIRECT` |
| `users.suspended_at` | `current_timestamp when SUSPENDED else NULL` | `DIRECT` |
| `users.suspension_reason` | `request.reason when SUSPENDED else NULL` | `DIRECT` |
...
| `auth_sessions.revoke_reason` | `ACCOUNT_SUSPENDED` | `DIRECT` |
