...
- Kiểm permission `PERM-IAM-003` server-side.

### 0.2. Query/check quyền

| Target table/API | Column/Field | Condition/Value | Remarks |
|---|---|---|---|
| `identity_db` | `principal/resource scope` | ``PERM-IAM-003` + recent MFA` | Không dùng frontend guard làm hàng rào bảo mật |
...
- `Header.If-Match`: required=`Yes`, type=`string`, format=`"<row_version>"`, valid=`N/A`; basis=`DIRECT`.
- `Header.Idempotency-Key`: required=`Yes`, type=`string`, format=`UUID/random`, valid=`N/A`; basis=`DIRECT`.
- `Path.userId`: required=`Yes`, type=`uuid`, format=`UUID v7`, valid=`N/A`; basis=`DIRECT`.
- `Body.roleCodes`: required=`Yes`, type=`array<string>`, format=`role code array`, valid=`N/A`; basis=`DERIVED`.
- `Body.reason`: required=`Yes`, type=`string`, format=`text`, valid=`N/A`; basis=`DIRECT`.
...
### 2.2. Query `TBL-IAM-015 — user_role_assignments`
...
- `TBL-IAM-015 — user_role_assignments` operation `UPDATE`.
- `TBL-IAM-015 — user_role_assignments` operation `INSERT`.
...
- `UPDATE user_role_assignments`: refer [07_user_role_assignments_update.md](./07_user_role_assignments_update.md).
- `INSERT user_role_assignments`: refer [08_user_role_assignments_insert.md](./08_user_role_assignments_insert.md).
- `UPDATE users`: refer [09_users_update.md](./09_users_update.md).
...
|---|---|---|
| `user_role_assignments.revoked_at` | `current_timestamp` | `DIRECT` |
| `user_role_assignments.revoked_by` | `Authenticated admin ID` | `DIRECT` |
| `user_role_assignments.id` | `Generated UUID v7` | `DIRECT` |
| `user_role_assignments.created_at` | `current_timestamp` | `DIRECT` |
| `user_role_assignments.user_id` | `Path userId` | `DIRECT` |
| `user_role_assignments.role_id` | `Resolved roles.id` | `DIRECT` |
| `user_role_assignments.scope_type` | `PLATFORM` | `DIRECT` |
| `user_role_assignments.scope_id` | `NULL` | `DIRECT` |
| `user_role_assignments.valid_from` | `current_timestamp` | `DIRECT` |
| `user_role_assignments.valid_until` | `NULL` | `DIRECT` |
| `user_role_assignments.granted_by` | `Authenticated admin ID` | `DIRECT` |
| `user_role_assignments.revoked_at` | `NULL` | `DIRECT` |
| `user_role_assignments.revoked_by` | `NULL` | `DIRECT` |
| `user_role_assignments.reason` | `request.reason` | `DIRECT` |
