...
| Purpose | `Exact role code set, reason,`If-Match` → roles/version` |
| Consumer/Actor | ``PERM-IAM-003` + recent MFA` |
| Authentication | `Bearer access token ES256; kiểm iss, aud, exp, nbf, jti, sid, authVersion theo quy ước chung.` |
| Authorization | ``PERM-IAM-003` + recent MFA` |
...
- Contract summary: TX L user; diff assignments; forbid last platform admin removal; authVersion++; outbox.
...
- Idempotency; full audit; no cache
