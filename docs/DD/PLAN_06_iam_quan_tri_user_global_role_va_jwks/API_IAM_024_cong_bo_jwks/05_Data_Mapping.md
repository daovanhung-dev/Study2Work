...
|---|---|---|---|
| `identity_db` | `principal/resource scope` | `Public` | Không dùng frontend guard làm hàng rào bảo mật |
...
- `N/A — API không có input nghiệp vụ`.

## 2. Get thông tin

### 2.1. Query `TBL-IAM-011 — signing_keys`

| Target table | Column get | Chú thích | Remarks |
|---|---|---|---|
| `signing_keys AS t_signingk` | `t_signingk.kid` | `kid` | `DIRECT` |
| `signing_keys AS t_signingk` | `t_signingk.algorithm` | `algorithm` | `DIRECT` |
| `signing_keys AS t_signingk` | `t_signingk.public_jwk` | `public_jwk` | `DIRECT` |
| `signing_keys AS t_signingk` | `t_signingk.not_before` | `not_before` | `DIRECT` |
| `signing_keys AS t_signingk` | `t_signingk.not_after` | `not_after` | `DIRECT` |
| `signing_keys AS t_signingk` | `t_signingk.activated_at` | `activated_at` | `DIRECT` |
| `signing_keys AS t_signingk` | `t_signingk.retired_at` | `retired_at` | `DIRECT` |

**WHERE / predicate cho `signing_keys`**

- `activated_at IS NOT NULL`.
- `not_before <= current_timestamp`.
- `not_after > current_timestamp` hoặc nằm trong rollover cache window.
