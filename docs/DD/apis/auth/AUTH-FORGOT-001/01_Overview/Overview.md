# Overview — AUTH-FORGOT-001

## 1. Business intent

**Yêu cầu đặt lại mật khẩu** thuộc module **02. Tài khoản, xác thực và hồ sơ**.
...
1. API gateway/controller nhận `POST /api/v1/auth/forgot-password` và gắn `traceId`.
...
- Ghi chú endpoint: Response không tiết lộ email tồn tại.
