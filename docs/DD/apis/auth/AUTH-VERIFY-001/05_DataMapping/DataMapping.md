...
| --- | --- | --- | --- |
| channel | channel | Input DTO → application command | Validate + normalize |
| target | target | Input DTO → application command | Validate + normalize |
| code | code | Input DTO → application command | Validate + normalize |
| verified | - | Read model / response DTO | Filter by scope before serialization |
| verifiedAt | - | Read model / response DTO | Filter by scope before serialization |
