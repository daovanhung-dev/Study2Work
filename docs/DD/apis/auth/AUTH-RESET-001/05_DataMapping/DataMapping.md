...
| --- | --- | --- | --- |
| resetToken | reset_token | Input DTO → application command | Validate + normalize |
| newPassword | new_password | Input DTO → application command | Validate + normalize |
| reset | - | Read model / response DTO | Filter by scope before serialization |
