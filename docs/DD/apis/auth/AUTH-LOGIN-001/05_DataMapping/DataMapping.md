...
| --- | --- | --- | --- |
| email | email | Input DTO → application command | Validate + normalize |
| password | password | Input DTO → application command | Validate + normalize |
| deviceName | device_name | Input DTO → application command | Validate + normalize |
| accessToken | - | Read model / response DTO | Filter by scope before serialization |
| refreshToken | - | Read model / response DTO | Filter by scope before serialization |
| expiresIn | - | Read model / response DTO | Filter by scope before serialization |
| user | - | Read model / response DTO | Filter by scope before serialization |
