...
| acceptTerms | accept_terms | Input DTO → application command | Validate + normalize |
| userId | - | Read model / response DTO | Filter by scope before serialization |
| verificationRequired | - | Read model / response DTO | Filter by scope before serialization |
| nextStep | - | Read model / response DTO | Filter by scope before serialization |
