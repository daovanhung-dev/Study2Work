...
**Làm mới access token** thuộc module **02. Tài khoản, xác thực và hồ sơ**.
...
1. API gateway/controller nhận `POST /api/v1/auth/refresh` và gắn `traceId`.
...
- Ghi chú endpoint: Refresh token rotation.
