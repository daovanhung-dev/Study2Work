# Data Mapping — AUTH-PASSWORD-CHANGE-001
...
| --- | --- | --- | --- |
| currentPassword | current_password | Input DTO → application command | Validate + normalize |
| newPassword | new_password | Input DTO → application command | Validate + normalize |
| changed | - | Read model / response DTO | Filter by scope before serialization |
| revokedSessions | - | Read model / response DTO | Filter by scope before serialization |
