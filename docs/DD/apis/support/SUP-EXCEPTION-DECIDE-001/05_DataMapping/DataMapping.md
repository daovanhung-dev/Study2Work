# Data Mapping — SUP-EXCEPTION-DECIDE-001
...
| --- | --- | --- | --- |
| decision | decision | Input DTO → application command | Validate + normalize |
| reason | reason | Input DTO → application command | Validate + normalize |
| effectiveUntil | effective_until | Input DTO → application command | Validate + normalize |
...
| decidedAt | - | Read model / response DTO | Filter by scope before serialization |
