# Overview — SUP-DETAIL-001

## 1. Business intent

**Lấy chi tiết ticket hỗ trợ** thuộc module **11. Admin học viên, hỗ trợ và ngoại lệ**.
...
1. API gateway/controller nhận `GET /api/v1/support-requests/{requestId}` và gắn `traceId`.
...
- Ghi chú endpoint: Owner hoặc staff có scope.
