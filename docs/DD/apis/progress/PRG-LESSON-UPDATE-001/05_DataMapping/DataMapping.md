# Data Mapping — PRG-LESSON-UPDATE-001

## 1. Aggregates / tables

`learner_path_progress, course_progress, lesson_progress, learning_events, completion_certificates`

## 2. Field mapping

| API field | Persistence / projection | Application handling | Rule |
| --- | --- | --- | --- |
| eventType | event_type | Input DTO → application command | Validate + normalize |
| positionSeconds | position_seconds | Input DTO → application command | Validate + normalize |
| durationSeconds | duration_seconds | Input DTO → application command | Validate + normalize |
| clientEventId | client_event_id | Input DTO → application command | Validate + normalize |
| occurredAt | occurred_at | Input DTO → application command | Validate + normalize |
| progress | - | Read model / response DTO | Filter by scope before serialization |
| acceptedEventId | - | Read model / response DTO | Filter by scope before serialization |
| nextUnlock | - | Read model / response DTO | Filter by scope before serialization |


## 3. Persistence behavior

- Thao tác ghi chạy trong transaction; validate authorization và state transition trước khi commit.
- Có log sự kiện nghiệp vụ theo người dùng và traceId.
- Sensitive values phải được masking/encryption/hashing theo loại dữ liệu.
- Đồng thời ghi `traceId`, actor và source context vào observability metadata khi phù hợp.

## 4. Side effects

- Có thể phát domain event để cập nhật notification, progress projection, report snapshot hoặc audit pipeline.
- Side effect bất đồng bộ phải idempotent và không làm thay đổi response contract đã trả về.
