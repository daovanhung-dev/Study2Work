# Overview — PRG-LESSON-UPDATE-001

## 1. Business intent

**Cập nhật sự kiện tiến độ bài học** thuộc module **07. Tiến độ và hoàn thành**.

## 2. Authorization

- Yêu cầu `Bearer JWT; endpoint internal dùng service token`.
...
1. API gateway/controller nhận `PATCH /api/v1/lessons/{lessonId}/progress` và gắn `traceId`.
...
- Ghi chú endpoint: Dedupe clientEventId; server quyết định completion.
