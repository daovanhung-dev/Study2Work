# Overview — ADM-CONTENT-RESTORE-001

## 1. Business intent

**Khôi phục phiên bản nội dung** thuộc module **10. Admin quản trị nội dung**.
...
1. API gateway/controller nhận `POST /api/v1/admin/content/{contentType}/{contentId}/restore-version` và gắn `traceId`.
...
- Ghi chú endpoint: Tạo version mới từ snapshot, không mutate lịch sử.
