# Overview — ADM-CONTENT-CHECK-001

## 1. Business intent

**Kiểm tra trước khi publish** thuộc module **10. Admin quản trị nội dung**.
...
1. API gateway/controller nhận `POST /api/v1/admin/content/{contentType}/{contentId}/pre-publish-check` và gắn `traceId`.
...
- Ghi chú endpoint: Không publish nếu passed=false.
