# Data Mapping — ADM-CONTENT-CHECK-001
...
| --- | --- | --- | --- |
| targetVersion | target_version | Input DTO → application command | Validate + normalize |
| passed | - | Read model / response DTO | Filter by scope before serialization |
| checks | - | Read model / response DTO | Filter by scope before serialization |
| blockingIssues | - | Read model / response DTO | Filter by scope before serialization |
