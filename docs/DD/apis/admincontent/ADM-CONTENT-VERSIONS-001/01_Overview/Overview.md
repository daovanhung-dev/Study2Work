# Overview — ADM-CONTENT-VERSIONS-001

## 1. Business intent

**Lấy lịch sử phiên bản nội dung** thuộc module **10. Admin quản trị nội dung**.
...
1. API gateway/controller nhận `GET /api/v1/admin/content/{contentType}/{contentId}/versions` và gắn `traceId`.
...
- Ghi chú endpoint: Không được sửa version đã publish trực tiếp.
