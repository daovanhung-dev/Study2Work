# Overview — ADM-CONTENT-DETAIL-001

## 1. Business intent

**Lấy chi tiết nội dung quản trị** thuộc module **10. Admin quản trị nội dung**.
...
1. API gateway/controller nhận `GET /api/v1/admin/content/{contentType}/{contentId}` và gắn `traceId`.
...
- Ghi chú endpoint: Gồm draft metadata, không đưa ra public.
