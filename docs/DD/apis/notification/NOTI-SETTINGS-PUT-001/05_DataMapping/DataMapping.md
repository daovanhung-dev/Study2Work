# Data Mapping — NOTI-SETTINGS-PUT-001
...
| --- | --- | --- | --- |
| channels | channels | Input DTO → application command | Validate + normalize |
| topics | topics | Input DTO → application command | Validate + normalize |
| quietHours | quiet_hours | Input DTO → application command | Validate + normalize |
| settings | - | Read model / response DTO | Filter by scope before serialization |
