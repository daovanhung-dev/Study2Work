# Data Mapping — CNT-RESOURCE-DOWNLOAD-001
...
| --- | --- | --- | --- |
| purpose | purpose | Input DTO → application command | Validate + normalize |
| url | - | Read model / response DTO | Filter by scope before serialization |
| expiresAt | - | Read model / response DTO | Filter by scope before serialization |
| fileName | - | Read model / response DTO | Filter by scope before serialization |
