# Overview — COM-GROUP-LIST-001

## 1. Business intent

**Lấy danh sách nhóm cộng đồng phù hợp learner** thuộc module **08. Cộng đồng Zalo**.

## 2. Authorization

- Yêu cầu `Bearer JWT`.
- Actor/role hợp lệ: **Learner, Community Moderator, Admin**.
- Authorization thực hiện tại backend, không tin trạng thái guard của frontend.

## 3. Main flow

1. API gateway/controller nhận `GET /api/v1/community-groups` và gắn `traceId`.
...
- Ghi chú endpoint: Filters scopeType, scopeId; không trả link nếu chưa đủ quyền.
