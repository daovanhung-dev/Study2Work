# Overview — LRN-ACTIVATE-001

## 1. Business intent

**Kích hoạt lộ trình** thuộc module **04. Lộ trình học**.
...
1. API gateway/controller nhận `POST /api/v1/learning-paths/{pathId}/activate` và gắn `traceId`.
...
- Ghi chú endpoint: Yêu cầu contact verified, onboarding completed, không có ACTIVE khác.
