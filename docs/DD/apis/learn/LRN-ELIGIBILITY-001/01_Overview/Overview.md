# Overview — LRN-ELIGIBILITY-001

## 1. Business intent

**Kiểm tra khả năng kích hoạt lộ trình** thuộc module **04. Lộ trình học**.
...
1. API gateway/controller nhận `GET /api/v1/learning-paths/{pathId}/eligibility` và gắn `traceId`.
...
- Ghi chú endpoint: Chỉ để hiển thị, không thay thế kiểm tra khi activate.
