# Overview — RBA-ROLE-REVOKE-001

## 1. Business intent

**Thu hồi role của người dùng** thuộc module **13. RBAC và Audit**.
...
1. API gateway/controller nhận `DELETE /api/v1/admin/users/{userId}/roles/{roleId}` và gắn `traceId`.
...
- Ghi chú endpoint: Không được xóa role cuối gây mất quyền quản trị theo policy.
