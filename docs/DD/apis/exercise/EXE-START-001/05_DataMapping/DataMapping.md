...
| --- | --- | --- | --- |
| clientStartedAt | client_started_at | Input DTO → application command | Validate + normalize |
| attemptId | - | Read model / response DTO | Filter by scope before serialization |
| status | - | Read model / response DTO | Filter by scope before serialization |
| startedAt | - | Read model / response DTO | Filter by scope before serialization |
