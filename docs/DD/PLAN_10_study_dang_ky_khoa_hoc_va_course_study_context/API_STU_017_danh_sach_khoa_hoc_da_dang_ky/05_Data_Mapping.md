...
- `Query.status`: required=`No`, type=`string`, format=`N/A`, valid=`ENROLLED|IN_PROGRESS|COMPLETED`; basis=`DIRECT`.
- `Query.source`: required=`No`, type=`string`, format=`N/A`, valid=`STANDALONE|PRIMARY_PATH|ADMIN`; basis=`DIRECT`.
...
### 2.5. Query `TBL-STU-030 — progress_snapshots`
