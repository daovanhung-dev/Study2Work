...
|---|---|---|---|
| `study_db` | `principal/resource scope` | `Enrolled learner` | Không dùng frontend guard làm hàng rào bảo mật |
...
- `Query.enrollmentId`: required=`No`, type=`uuid`, format=`UUID v7`, valid=`N/A`; basis=`DIRECT`.
...
### 2.2. Query `TBL-STU-027 — course_enrollments`
...
### 2.5. Query `TBL-STU-014 — chapters`
...
### 2.7. Query `TBL-STU-016 — content_blocks`
