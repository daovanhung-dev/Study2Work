...
| Module | `Study` |
| Method | `POST` |
| Endpoint | `/api/v1/me/onboarding/complete` |
| Purpose | ``If-Match` → completed record + recommendation run ID` |
...
| Side effects | `Audit theo nguồn endpoint; payload phải redact PII/secret.; Outbox/event `study.onboarding.completed`; payload/version còn Q-15 nếu chưa có schema.` |
...
- Contract summary: TX validate required answers; state COMPLETED; snapshot answers; create deterministic recommendation run/items.
...
- Idempotency required; event`study.onboarding.completed`; audit
