...
- `Body.displayName`: required=`No`, type=`string`, format=`text`, valid=`N/A`; basis=`DIRECT`.
- `Body.bio`: required=`No`, type=`string`, format=`text`, valid=`N/A`; basis=`DIRECT`.
- `Body.locale`: required=`No`, type=`string`, format=`BCP 47`, valid=`N/A`; basis=`DIRECT`.
- `Body.timezone`: required=`No`, type=`string`, format=`IANA timezone`, valid=`N/A`; basis=`DIRECT`.
- `Body.skillIds`: required=`No`, type=`array<uuid>`, format=`UUID array`, valid=`N/A`; basis=`DIRECT`.
...
### 2.3. Query `TBL-STU-056 — study_skills`
...
- `TBL-STU-002 — learner_profiles` operation `UPDATE`.
...
- `UPDATE learner_profiles`: refer [07_learner_profiles_update.md](./07_learner_profiles_update.md).
...
|---|---|---|
| `learner_profiles.full_name` | `CONFLICT — request.displayName mapping not approved` | `CONFLICT` |
| `learner_profiles.bio` | `request.bio with API max 1000` | `CONFLICT` |
| `learner_profiles.updated_at` | `current_timestamp` | `CONFLICT` |
| `learner_profiles.row_version` | `row_version + 1` | `CONFLICT` |
...
| `audit_events.action` | `STUDY_PROFILE_UPDATED` | `DERIVED` |
| `audit_events.resource_type` | `LEARNER_PROFILE` | `DERIVED` |
| `audit_events.resource_id` | `learner_profiles.id` | `DERIVED` |
...
| `audit_events.changes` | `Redacted before/after profile fields` | `DERIVED` |
