...
| Purpose | `Không body → số session revoked` |
| Consumer/Actor | `Authenticated + recent password/MFA` |
| Authentication | `Bearer access token ES256; kiểm iss, aud, exp, nbf, jti, sid, authVersion theo quy ước chung.` |
| Authorization | `Authenticated + recent password/MFA` |
...
| Side effects | `Audit theo nguồn endpoint; payload phải redact PII/secret.; Outbox/event `identity.sessions.revoked`; payload/version còn Q-15 nếu chưa có schema.` |
...
- Contract summary: TX L user; increment`authVersion`, revoke sessions.
...
- Idempotency; event`identity.sessions.revoked`; audit
