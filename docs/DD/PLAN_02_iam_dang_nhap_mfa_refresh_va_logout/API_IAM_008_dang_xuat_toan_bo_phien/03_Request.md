...
| 2 | Access token | `Authorization` | `Yes` | `Bearer token` | Authenticated session. | `05_Data_Mapping.md#1-validate-data-input` |
| 3 | Idempotency key | `Idempotency-Key` | `Yes` | `UUID/random` | Scope principal + method + normalized path. | `05_Data_Mapping.md#1-validate-data-input` |
| 4 | Step-up proof | `SOURCE_REQUIRED` | `Yes` | `recent password/MFA proof` | Field/claim biểu diễn step-up chưa có schema. | `05_Data_Mapping.md#1-validate-data-input` |
