...
|---|---|---|---|
| `identity_db` | `principal/resource scope` | `Authenticated + recent password/MFA` | Không dùng frontend guard làm hàng rào bảo mật |
...
- `Header.SOURCE_REQUIRED`: required=`Yes`, type=`string`, format=`recent password/MFA proof`, valid=`N/A`; basis=`SOURCE_REQUIRED`.
...
| `auth_sessions.revoke_reason` | `LOGOUT_ALL` | `DIRECT` |
