...
|---|---|---|---|
| `identity_db` | `principal/resource scope` | `Refresh cookie/token` | Không dùng frontend guard làm hàng rào bảo mật |
...
- `Header.SOURCE_REQUIRED`: required=`Yes`, type=`string`, format=`opaque token`, valid=`N/A`; basis=`SOURCE_REQUIRED`.
- Không triển khai field `SOURCE_REQUIRED` trước khi source gap được quyết định.

## 2. Get thông tin

### 2.1. Query `TBL-IAM-010 — refresh_tokens`
...
### 2.2. Query `TBL-IAM-009 — auth_sessions`
...
- Lock đúng refresh-token row và session/family. Chỉ transaction đầu được rotate; transaction sau đi reuse branch và revoke family.

### 3.3. Items update/insert

- `UPDATE refresh_tokens`: refer [07_refresh_tokens_update.md](./07_refresh_tokens_update.md).
- `INSERT refresh_tokens`: refer [08_refresh_tokens_insert.md](./08_refresh_tokens_insert.md).
...
|---|---|---|
| `refresh_tokens.status` | `CONSUMED or REVOKED by branch` | `DIRECT` |
| `refresh_tokens.rotated_to_id` | `New child token ID on success` | `DIRECT` |
| `refresh_tokens.consumed_at` | `current_timestamp on success` | `DIRECT` |
| `refresh_tokens.reuse_detected_at` | `current_timestamp on reuse` | `DIRECT` |
| `refresh_tokens.id` | `Generated UUID v7` | `DIRECT` |
| `refresh_tokens.created_at` | `current_timestamp` | `DIRECT` |
| `refresh_tokens.session_id` | `Existing session_id` | `DIRECT` |
| `refresh_tokens.family_id` | `Existing family_id` | `DIRECT` |
| `refresh_tokens.parent_token_id` | `Current token ID` | `DIRECT` |
| `refresh_tokens.token_hash` | `Hash new raw token` | `DIRECT` |
...
| `auth_sessions.status` | `COMPROMISED` | `DIRECT` |
| `auth_sessions.revoked_at` | `current_timestamp` | `DIRECT` |
| `auth_sessions.revoke_reason` | `REFRESH_TOKEN_REUSE_DETECTED` | `DIRECT` |
