...
- `Body.password`: required=`Yes`, type=`string`, format=`password`, valid=`N/A`; basis=`DIRECT`.
- `Body.deviceLabel`: required=`No`, type=`string`, format=`text`, valid=`N/A`; basis=`DERIVED`.
...
### 2.2. Query `TBL-IAM-003 — password_credentials`
...
### 2.4. Query `TBL-IAM-015 — user_role_assignments`
...
### 2.6. Query `TBL-IAM-006 — mfa_methods`
...
- `TBL-IAM-008 — mfa_challenges` operation `INSERT`.
...
- `INSERT mfa_challenges`: refer [08_mfa_challenges_insert.md](./08_mfa_challenges_insert.md).
...
|---|---|---|
| `password_credentials.failed_count` | `Increment/reset according to password verification branch` | `DIRECT` |
| `password_credentials.locked_until` | `Set by lock policy or NULL` | `DIRECT` |
| `password_credentials.updated_at` | `current_timestamp` | `DIRECT` |
| `password_credentials.row_version` | `row_version + 1` | `DIRECT` |
| `mfa_challenges.id` | `Generated UUID v7` | `DIRECT` |
| `mfa_challenges.created_at` | `current_timestamp` | `DIRECT` |
| `mfa_challenges.updated_at` | `current_timestamp` | `DIRECT` |
| `mfa_challenges.row_version` | `1` | `DIRECT` |
| `mfa_challenges.user_id` | `Matched users.id` | `DIRECT` |
| `mfa_challenges.session_id` | `NULL before authenticated session` | `DIRECT` |
| `mfa_challenges.purpose` | `LOGIN` | `DIRECT` |
| `mfa_challenges.challenge_hash` | `Hash generated challenge` | `DIRECT` |
| `mfa_challenges.expires_at` | `Generated expiry from policy` | `DIRECT` |
| `mfa_challenges.attempt_count` | `0` | `DIRECT` |
| `mfa_challenges.max_attempts` | `5` | `DIRECT` |
| `mfa_challenges.verified_at` | `NULL` | `DIRECT` |
| `mfa_challenges.invalidated_at` | `NULL` | `DIRECT` |
...
| `auth_sessions.user_id` | `Matched users.id` | `DIRECT` |
| `auth_sessions.status` | `ACTIVE` | `DIRECT` |
| `auth_sessions.session_epoch` | `SOURCE_REQUIRED — relation with authVersion unresolved` | `DIRECT` |
| `auth_sessions.device_name` | `request.deviceLabel when provided` | `DIRECT` |
| `auth_sessions.last_seen_at` | `current_timestamp` | `DIRECT` |
| `auth_sessions.expires_at` | `Session expiry policy` | `DIRECT` |
| `auth_sessions.revoked_at` | `NULL` | `DIRECT` |
| `auth_sessions.revoke_reason` | `NULL` | `DIRECT` |
| `refresh_tokens.id` | `Generated UUID v7` | `DIRECT` |
| `refresh_tokens.created_at` | `current_timestamp` | `DIRECT` |
| `refresh_tokens.session_id` | `Created auth_sessions.id` | `DIRECT` |
| `refresh_tokens.family_id` | `Generated UUID v7` | `DIRECT` |
| `refresh_tokens.parent_token_id` | `NULL` | `DIRECT` |
| `refresh_tokens.token_hash` | `Hash generated raw refresh token` | `DIRECT` |
...
| `refresh_tokens.rotated_to_id` | `NULL` | `DIRECT` |
| `refresh_tokens.consumed_at` | `NULL` | `DIRECT` |
| `refresh_tokens.reuse_detected_at` | `NULL` | `DIRECT` |
