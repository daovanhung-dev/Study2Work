...
| Purpose | `Page,`q`, skill, level → published path cards` |
...
- Contract summary: R current`learning_paths` + `learning_path_versions`, aggregate courses; FTS/trigram + `(status,published_at)`; auth user gets own summary by separate indexed query.
