...
- `Query.q`: required=`No`, type=`string`, format=`Unicode text`, valid=`N/A`; basis=`DIRECT`.
- `Query.skill`: required=`No`, type=`string`, format=`skill filter`, valid=`N/A`; basis=`DIRECT`.
- `Query.level`: required=`No`, type=`string`, format=`N/A`, valid=`SOURCE_REQUIRED`; basis=`DIRECT`.
...
### 2.5. Query `TBL-STU-057 — course_skill_outcomes`

| Target table | Column get | Chú thích | Remarks |
|---|---|---|---|
| `course_skill_outcomes AS t_coursesk` | `t_coursesk.course_version_id` | `course_version_id` | `DIRECT` |
| `course_skill_outcomes AS t_coursesk` | `t_coursesk.skill_id` | `skill_id` | `DIRECT` |
| `course_skill_outcomes AS t_coursesk` | `t_coursesk.outcome_level` | `outcome_level` | `DIRECT` |
| `course_skill_outcomes AS t_coursesk` | `t_coursesk.description` | `description` | `DIRECT` |
| `course_skill_outcomes AS t_coursesk` | `t_coursesk.position` | `position` | `DIRECT` |

**WHERE / predicate cho `course_skill_outcomes`**

- Điều kiện cụ thể theo endpoint summary và khóa/index canonical; exact SQL giữ `DERIVED` nếu catalog không nêu field-level.

### 2.6. Query `TBL-STU-056 — study_skills`

| Target table | Column get | Chú thích | Remarks |
|---|---|---|---|
| `study_skills AS t_studyski` | `t_studyski.id` | `id` | `DIRECT` |
| `study_skills AS t_studyski` | `t_studyski.code` | `code` | `DIRECT` |
| `study_skills AS t_studyski` | `t_studyski.name` | `name` | `DIRECT` |
| `study_skills AS t_studyski` | `t_studyski.normalized_name` | `normalized_name` | `DIRECT` |

**WHERE / predicate cho `study_skills`**

- Điều kiện cụ thể theo endpoint summary và khóa/index canonical; exact SQL giữ `DERIVED` nếu catalog không nêu field-level.

### 2.7. Query `TBL-STU-027 — course_enrollments`
...
### 2.8. Query `TBL-STU-030 — progress_snapshots`

| Target table | Column get | Chú thích | Remarks |
|---|---|---|---|
| `progress_snapshots AS t_progress` | `t_progress.id` | `id` | `DIRECT` |
| `progress_snapshots AS t_progress` | `t_progress.learner_id` | `learner_id` | `DIRECT` |
| `progress_snapshots AS t_progress` | `t_progress.scope_type` | `scope_type` | `DIRECT` |
| `progress_snapshots AS t_progress` | `t_progress.scope_id` | `scope_id` | `DIRECT` |

**WHERE / predicate cho `progress_snapshots`**
