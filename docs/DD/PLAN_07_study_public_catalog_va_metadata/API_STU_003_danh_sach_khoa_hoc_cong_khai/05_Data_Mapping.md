...
- `Query.level`: required=`No`, type=`string`, format=`N/A`, valid=`BEGINNER|INTERMEDIATE|ADVANCED`; basis=`DIRECT`.

## 2. Get thông tin

### 2.1. Query `TBL-STU-011 — courses`
...
### 2.2. Query `TBL-STU-012 — course_versions`
...
### 2.3. Query `TBL-STU-057 — course_skill_outcomes`
...
### 2.4. Query `TBL-STU-056 — study_skills`
