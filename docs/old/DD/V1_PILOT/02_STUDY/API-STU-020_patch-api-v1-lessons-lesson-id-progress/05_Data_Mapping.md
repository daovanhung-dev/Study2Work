...
| If-Match | BD global/endpoint convention | S01/S02 | As stated | Authentication/idempotency/concurrency | N/A | N/A | N/A |
| `lessonId` | Endpoint URI | S01 | Path parsing | Resource lookup | N/A | N/A | N/A |
| `blockId` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| `status` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| `position` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| `completedAt` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| dữ kiện khối | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | ảnh chụp bài học/khóa học/lộ trình | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Người học đã ghi danh.
...
- Endpoint input contract: `If-Match`, dữ kiện khối `{blockId,status,position?,completedAt?}` → ảnh chụp bài học/khóa học/lộ trình.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX khóa các hàng ghi danh/tiến độ; kiểm khối thuộc bài học được ghim; gộp giá trị đơn điệu; tính lại quy tắc từ dữ kiện; cập nhật chèn ảnh chụp; không tin phần trăm từ ứng dụng khách

- Operations/observability contract: Nên dùng khóa lặp; hộp thư đi cho hoàn thành/hoạt động; chỉ kiểm toán điều chỉnh
