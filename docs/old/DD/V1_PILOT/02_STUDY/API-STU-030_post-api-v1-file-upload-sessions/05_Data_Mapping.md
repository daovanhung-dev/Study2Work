...
| Mục đích, tên tệp, MIME/kích thước dự kiến <=25MiB, SHA-256 | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | ID tải lên + PUT đã ký | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Người dùng Study đã xác thực.
...
- Endpoint input contract: Mục đích, tên tệp, MIME/kích thước dự kiến <=25MiB, SHA-256 → ID tải lên + PUT đã ký.

<a id="s04"></a>
### S04. Execute source-defined processing

> Kiểm danh sách cho phép PDF/PNG/JPEG/TXT/MD/CSV/ZIP; tạo khóa cách ly riêng; ghi phiên/tài sản `CREATED`

- Operations/observability contract: Khóa lặp bắt buộc; URL có chữ ký 15 phút; 20/giờ
