...
format: "markdown"
dd_id: "API-STU-043"
api_name: "POST /api/v1/support-requests"
...
| Danh mục, tiêu đề <=200, nội dung <=5000, ID tệp đính kèm sạch | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | yêu cầu hỗ trợ | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Người học.
...
- Endpoint input contract: Danh mục, tiêu đề <=200, nội dung <=5000, ID tệp đính kèm sạch → yêu cầu hỗ trợ.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX kiểm chủ sở hữu/tệp; tạo yêu cầu/sự kiện; gán lớp SLA
