...
| Câu trả lời theo loại, ; văn bản <=20k; liên kết HTTPS <=2048; ID tệp sạch/của chủ sở hữu; ID lựa chọn trắc nghiệm hợp lệ | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
- Endpoint input contract: Câu trả lời theo loại, `If-Match`; văn bản <=20k; liên kết HTTPS <=2048; ID tệp sạch/của chủ sở hữu; ID lựa chọn trắc nghiệm hợp lệ → bản nháp.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX khóa bản nháp; kiểm phiên bản và loại bài đánh giá; chỉ đính kèm tệp khi `CLEAN`; cập nhật chèn phiên bản

- Operations/observability contract: Khóa lặp tùy chọn; kiểm toán siêu dữ liệu, không kiểm toán nội dung câu trả lời
