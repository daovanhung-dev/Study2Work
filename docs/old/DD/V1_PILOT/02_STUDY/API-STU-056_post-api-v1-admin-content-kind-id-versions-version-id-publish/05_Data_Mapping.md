...
| `versionId` | Endpoint URI | S01 | Path parsing | Resource lookup | N/A | N/A | N/A |
| ID kiểm tra, , ghi chú xuất bản | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | phiên bản đã xuất bản | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-STU-003` + MFA; cấp tin cậy vẫn phải kiểm tra.
...
- Endpoint input contract: ID kiểm tra, `If-Match`, ghi chú xuất bản → phiên bản đã xuất bản.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX khóa thực thể ổn định/phiên bản; kiểm lại báo cáo kiểm tra mới và thành công/quyền người yêu cầu/cấp tin cậy; `DRAFT→PUBLISHED`, bản trước → `SUPERSEDED`, hoán đổi con trỏ hiện hành nguyên tử

- Operations/observability contract: Khóa lặp bắt buộc; sự kiện/làm mới bộ nhớ đệm/kiểm toán; không gọi ngoài TX
