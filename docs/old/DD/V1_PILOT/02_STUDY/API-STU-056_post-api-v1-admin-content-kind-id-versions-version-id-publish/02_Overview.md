...
| Module | `Study` |
| Method | `POST` |
| Endpoint | `/api/v1/admin/content/{kind}/{id}/versions/{versionId}/publish` |
| Purpose | phiên bản đã xuất bản |
| Consumer/Actor | `PERM-STU-003` + MFA; cấp tin cậy vẫn phải kiểm tra |
| Authentication | `PERM-STU-003` + MFA; cấp tin cậy vẫn phải kiểm tra |
| Authorization | `PERM-STU-003` + MFA; cấp tin cậy vẫn phải kiểm tra |
...
| Side effects | Khóa lặp bắt buộc; sự kiện/làm mới bộ nhớ đệm/kiểm toán; không gọi ngoài TX |

## Sources

- [04_DAC_TA_API.md](../../../../BD/04_DAC_TA_API.md): `L196` — endpoint contract.
