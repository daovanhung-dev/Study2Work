...
| Mã kiểm tra/kích thước thực tế | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | tài sản `SCANNING` | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Chủ sở hữu lượt tải lên.
...
- Endpoint input contract: Mã kiểm tra/kích thước thực tế → tài sản `SCANNING`.

<a id="s04"></a>
### S04. Execute source-defined processing

> Xác minh khóa/kích thước/mã kiểm tra/MIME từ HEAD của đối tượng; TX đánh dấu đã tải, tạo tác vụ quét qua hộp thư đi; không đồng bộ đánh dấu sạch

- Operations/observability contract: Khóa lặp; sự kiện `study.file.scan_requested`
