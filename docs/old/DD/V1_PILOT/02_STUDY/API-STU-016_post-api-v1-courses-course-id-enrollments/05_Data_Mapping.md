...
format: "markdown"
dd_id: "API-STU-016"
api_name: "POST /api/v1/courses/{courseId}/enrollments"
...
| `courseId` | Endpoint URI | S01 | Path parsing | Resource lookup | N/A | N/A | N/A |
| ID khóa học ổn định hoặc phiên bản đã xuất bản chỉ định | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | lần ghi danh ghim phiên bản | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Người học đang hoạt động; không bắt buộc thiết lập ban đầu.
...
- Endpoint input contract: ID khóa học ổn định hoặc phiên bản đã xuất bản chỉ định → lần ghi danh ghim phiên bản.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX khóa lặp; chọn phiên bản đã xuất bản hiện hành; chèn duy nhất `(user,course_version)` hoặc trả bản có sẵn; nguồn độc lập/lộ trình

- Operations/observability contract: Khóa lặp bắt buộc; `study.course.enrolled`; kiểm toán
