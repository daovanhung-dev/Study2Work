...
| Dữ kiện đích, giá trị đã sửa, lý do/minh chứng | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | điều chỉnh + ảnh chụp dựng lại | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-STU-010` + MFA.
...
- Endpoint input contract: Dữ kiện đích, giá trị đã sửa, lý do/minh chứng, `If-Match` → điều chỉnh + ảnh chụp dựng lại.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX thêm điều chỉnh; không xóa bản gốc; tính lại hoàn thành phụ thuộc dưới khóa người học

- Operations/observability contract: Khóa lặp; sự kiện/kiểm toán bất biến
