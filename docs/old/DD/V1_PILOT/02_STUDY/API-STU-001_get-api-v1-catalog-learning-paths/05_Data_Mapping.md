...
format: "markdown"
dd_id: "API-STU-001"
api_name: "GET /api/v1/catalog/learning-paths"
...
| Trang, , kỹ năng, cấp độ | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |

### Query Matrix

| Query ID | Mục đích | Type | Base table/view | Alias | Columns | JOIN | WHERE | GROUP/HAVING | ORDER | Pagination | Result variable | Branch |
| ---: | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `Q01` | SOURCE_REQUIRED — complete query contract not specified. | SOURCE_REQUIRED | `learning_paths` | N/A | N/A | N/A | N/A | N/A | N/A | N/A | N/A | S04 |
| `Q02` | SOURCE_REQUIRED — complete query contract not specified. | SOURCE_REQUIRED | `learning_path_versions` | N/A | N/A | N/A | N/A | N/A | N/A | N/A | N/A | S04 |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | thẻ lộ trình đã xuất bản | SOURCE_REQUIRED for nested shape | API catalog |
...
- Endpoint input contract: Trang, `q`, kỹ năng, cấp độ → thẻ lộ trình đã xuất bản.

<a id="s04"></a>
### S04. Execute source-defined processing

> Đọc `learning_paths` và `learning_path_versions` hiện hành, tổng hợp khóa học; FTS/trigram cùng `(status,published_at)`; người dùng đã xác thực có bản tóm tắt riêng từ truy vấn chỉ mục tách biệt

- Operations/observability contract: CDN/Redis 60 giây, ETag
