...
format: "markdown"
dd_id: "API-STU-061"
api_name: "GET /api/v1/me/evidence"
...
| Trang, loại/trạng thái | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | minh chứng `ISSUED` của chính mình; mặc định không trả tệp bí mật | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Người học; mã thông báo `aud=study-evidence`, phạm vi `evidence:self:read`.
...
- Endpoint input contract: Trang, loại/trạng thái → minh chứng `ISSUED` của chính mình; mặc định không trả tệp bí mật.

<a id="s04"></a>
### S04. Execute source-defined processing

> Điều kiện chủ thể nền tảng → bản chiếu Study → chủ sở hữu; đọc minh chứng/phiên bản/kỹ năng; mã thông báo 5 phút; không nhận ID người học tùy ý

- Operations/observability contract: Riêng tư, không lưu; kiểm toán truy cập; 60/phút
