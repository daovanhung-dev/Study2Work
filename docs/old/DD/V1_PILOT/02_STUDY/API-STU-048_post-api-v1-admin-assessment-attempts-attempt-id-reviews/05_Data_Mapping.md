...
format: "markdown"
dd_id: "API-STU-048"
api_name: "POST /api/v1/admin/assessment-attempts/{attemptId}/reviews"
...
| `attemptId` | Endpoint URI | S01 | Path parsing | Resource lookup | N/A | N/A | N/A |
| Quyết định //, điểm thang chấm, phản hồi | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | SOURCE_REQUIRED — endpoint row does not split input and output fields. | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-STU-006`.
...
- Endpoint input contract: Quyết định `PASSED`/`NEEDS_REVISION`/`FAILED`, điểm thang chấm, phản hồi, `If-Match`.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX khóa lượt làm; kiểm thang chấm/khoảng giá trị/trạng thái `UNDER_REVIEW` hiện hành; thêm lượt duyệt; cập nhật kết quả/phiên bản; tính lại hoàn thành từ dữ kiện

- Operations/observability contract: Khóa lặp bắt buộc; sự kiện duyệt/hoàn thành; kiểm toán đầy đủ
