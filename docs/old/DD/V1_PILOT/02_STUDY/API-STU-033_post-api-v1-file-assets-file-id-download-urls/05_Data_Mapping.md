...
| `fileId` | Endpoint URI | S01 | Path parsing | Resource lookup | N/A | N/A | N/A |
| Cách trình bày mong muốn | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | GET có chữ ký <=5 phút | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Chủ sở hữu hoặc người duyệt được cấp quyền rõ ràng.
...
- Endpoint input contract: Cách trình bày mong muốn → GET có chữ ký <=5 phút.

<a id="s04"></a>
### S04. Execute source-defined processing

> Yêu cầu trạng thái `CLEAN` và tệp đính kèm được quyền; kiểm toán truy cập; tạo URL lưu trữ sau khi kiểm CSDL

- Operations/observability contract: Khóa lặp tùy chọn; 30/phút
