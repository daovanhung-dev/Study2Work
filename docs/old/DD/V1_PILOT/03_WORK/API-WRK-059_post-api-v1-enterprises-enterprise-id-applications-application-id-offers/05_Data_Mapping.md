...
format: "markdown"
dd_id: "API-WRK-059"
api_name: "POST /api/v1/enterprises/{enterpriseId}/applications/{applicationId}/offers"
...
| Đề nghị có cấu trúc, hết hạn, tài liệu sạch | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | ảnh chụp `OFFERED` | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-WRK-034` + được phân công.
...
- Endpoint input contract: Đề nghị có cấu trúc, hết hạn, tài liệu sạch → ảnh chụp `OFFERED`.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX yêu cầu trạng thái hiện hành đúng `INTERVIEWING`; thêm đề nghị bất biến và chuyển trạng thái; quyền thù lao tách biệt. Không được nhảy trực tiếp từ `SHORTLISTED` sang `OFFERED`

- Operations/observability contract: Khóa lặp; kiểm toán/thông báo rủi ro cao
