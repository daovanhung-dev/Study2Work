...
format: "markdown"
dd_id: "API-WRK-035"
api_name: "POST /api/v1/enterprises"
...
| Tên pháp lý/công khai, ID thuế, liên hệ, điều khoản | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | tổ chức `PENDING_VERIFICATION` + tư cách thành viên chủ sở hữu | SOURCE_REQUIRED for nested shape | API catalog |
...
- Endpoint input contract: Tên pháp lý/công khai, ID thuế, liên hệ, điều khoản → tổ chức `PENDING_VERIFICATION` + tư cách thành viên chủ sở hữu.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX chuẩn hóa ID thuế, chặn trùng đang hoạt động, tạo tổ chức/thành viên; chủ sở hữu không thể tự xác minh

- Operations/observability contract: Khóa lặp bắt buộc; kiểm toán
