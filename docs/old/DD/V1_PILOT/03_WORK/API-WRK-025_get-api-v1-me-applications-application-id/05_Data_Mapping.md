...
format: "markdown"
dd_id: "API-WRK-025"
api_name: "GET /api/v1/me/applications/{applicationId}"
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | ảnh chụp dữ liệu, trạng thái/lịch sử hiện tại, phỏng vấn, minh chứng `PENDING`/`READY`/`UNAVAILABLE`/`HIDDEN`/`REVOKED`, ID cuộc hội thoại | SOURCE_REQUIRED for nested shape | API catalog |
...
- Endpoint input contract: → ảnh chụp dữ liệu, trạng thái/lịch sử hiện tại, phỏng vấn, minh chứng `PENDING`/`READY`/`UNAVAILABLE`/`HIDDEN`/`REVOKED`, ID cuộc hội thoại.

<a id="s04"></a>
### S04. Execute source-defined processing

> Điều kiện theo ứng viên; loại trừ ghi chú/xếp hạng của nhà tuyển dụng

- Operations/observability contract: Riêng tư, không lưu
