...
| Module | `Work` |
| Method | `GET` |
| Endpoint | `/api/v1/me/applications/{applicationId}` |
| Purpose | ảnh chụp dữ liệu, trạng thái/lịch sử hiện tại, phỏng vấn, minh chứng `PENDING`/`READY`/`UNAVAILABLE`/`HIDDEN`/`REVOKED`, ID cuộc hội thoại |
| Consumer/Actor | Chủ sở hữu |
| Authentication | Chủ sở hữu |
| Authorization | Chủ sở hữu |
...
- [04_DAC_TA_API.md](../../../../BD/04_DAC_TA_API.md): `L263` — endpoint contract.
