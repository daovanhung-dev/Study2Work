...
format: "markdown"
dd_id: "API-WRK-007"
api_name: "PUT /api/v1/me/candidate-search-consent"
...
| `enabled` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| thời hạn tùy chọn, danh sách trường hiển thị cho phép, phiên bản chính sách | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | trạng thái | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Ứng viên + xác minh tăng cường khi cho phép.
...
- Endpoint input contract: `enabled`, thời hạn tùy chọn, danh sách trường hiển thị cho phép, phiên bản chính sách, `If-Match` → trạng thái.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX khóa hồ sơ; thêm sự kiện đồng ý. Cho phép cần bản xem trước/điều khoản rõ ràng. Từ chối đặt riêng tư đồng bộ và gửi yêu cầu gỡ chỉ mục ưu tiên cao; lượt đọc API loại ngay bằng điều kiện CSDL cả trước khi gỡ chỉ mục

- Operations/observability contract: Khóa lặp; kiểm toán đồng ý; SLO gỡ chỉ mục <=5 phút
