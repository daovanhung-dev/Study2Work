...
| ---: | --- |
| HTTP method | `POST` |
| URI | `/api/v1/enterprises/{enterpriseId}/jobs/{jobId}/close` |
...
| ---: | --- | --- | --- |
| 1 | Lý do // | Physical key, location, type or rule | `SOURCE_REQUIRED`; not included in JSON example. |

## Contract source

> Lý do `FILLED`/`CANCELLED`/`OTHER`, `If-Match` → `CLOSED`
