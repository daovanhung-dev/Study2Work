...
format: "markdown"
dd_id: "API-WRK-050"
api_name: "POST /api/v1/enterprises/{enterpriseId}/jobs/{jobId}/close"
...
| Lý do // | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | `CLOSED` | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-WRK-013`.
...
- Endpoint input contract: Lý do `FILLED`/`CANCELLED`/`OTHER`, `If-Match` → `CLOSED`.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX chuyển trạng thái kết thúc; không nhận đơn mới; giữ ATS/cuộc trò chuyện hiện có, cuộc trò chuyện chỉ thành chỉ đọc khi từng đơn ứng tuyển kết thúc

- Operations/observability contract: Khóa lặp; sự kiện/kiểm toán
