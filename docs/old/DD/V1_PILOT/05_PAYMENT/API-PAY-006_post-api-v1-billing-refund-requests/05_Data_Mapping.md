...
format: "markdown"
dd_id: "API-PAY-006"
api_name: "POST /api/v1/billing/refund-requests"
...
| Đơn hàng, số tiền <= có thể hoàn, lý do | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | yêu cầu | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Người mua/thành viên lập hóa đơn của tổ chức.
...
- Endpoint input contract: Đơn hàng, số tiền <= có thể hoàn, lý do → yêu cầu.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX L đơn hàng đã `SETTLED`; kiểm chính sách quyền đã dùng; tạo yêu cầu `REQUESTED`, không đổi số dư ngay lập tức

- Operations/observability contract: Xử lý lặp an toàn; kiểm toán rủi ro cao/hàng đợi tài chính
