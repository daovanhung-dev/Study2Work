...
| Truy vấn/biểu mẫu gốc đúng theo nhà cung cấp | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | xác nhận theo yêu cầu nhà cung cấp | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Chữ ký VNPAY/danh sách IP là lớp phòng thủ bổ sung.
...
- Endpoint input contract: Truy vấn/biểu mẫu gốc đúng theo nhà cung cấp → xác nhận theo yêu cầu nhà cung cấp.

<a id="s04"></a>
### S04. Execute source-defined processing

> Xác minh chữ ký trên các trường gốc chuẩn trước khi phân tích; thêm sự kiện duy nhất theo giao dịch/hàm băm sự kiện nhà cung cấp; đưa vào hàng đợi xử lý; trả nhanh. Bộ xử lý khóa đơn, kiểm người bán/đơn/số tiền/tiền tệ, áp dụng trạng thái đơn điệu và sổ cái/quyền sử dụng đúng một lần

- Operations/observability contract: Khóa lặp webhook; hàm băm gốc/kiểm toán; không xác thực người dùng
