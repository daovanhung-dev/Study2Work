...
| `platformUserId` | Endpoint URI | S01 | Path parsing | Resource lookup | N/A | N/A | N/A |
| Chủ thể + phiên bản đã biết tùy chọn | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | trạng thái/`authVersion`/vai trò/hàm băm email/phiên bản tối thiểu | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: JWT dịch vụ Study/Work `aud=identity-internal`.
...
- Endpoint input contract: Chủ thể + phiên bản đã biết tùy chọn → trạng thái/`authVersion`/vai trò/hàm băm email/phiên bản tối thiểu.

<a id="s04"></a>
### S04. Execute source-defined processing

> Đọc theo khóa chính; chỉ bắt kịp khi thiếu sự kiện đã ký; tối thiểu hóa PII; kiểm toán bên gọi
