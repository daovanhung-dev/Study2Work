...
| ID yêu cầu/đơn ứng tuyển, từng mục / + ảnh chụp/mã kiểm tra/mã lý do an toàn tối thiểu bất biến | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
- Actor/authorization source: JWT dịch vụ Study + chữ ký tách rời.
...
- Endpoint input contract: ID yêu cầu/đơn ứng tuyển, từng mục `READY`/`UNAVAILABLE` + ảnh chụp/mã kiểm tra/mã lý do an toàn tối thiểu bất biến.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX kết quả/sự kiện duy nhất; khóa yêu cầu đơn ứng tuyển; từ chối lệch chủ thể/đơn; lưu ảnh chụp mã hóa theo đơn; nếu rút đồng ý thì lưu biên nhận kiểm toán nhưng giữ ẩn; không bao giờ đổi ATS/đối sánh
