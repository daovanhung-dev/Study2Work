...
format: "markdown"
dd_id: "API-AIX-005"
api_name: "GET /api/v1/ai-jobs/{aiJobId}"
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | tác vụ `QUEUED`/`RUNNING`/`SUCCEEDED`/`FAILED`/`CANCELLED`; khi `SUCCEEDED` có duyệt đầu ra `DRAFT`/`ACCEPTED`/`EDITED_ACCEPT`/`REJECTED`/`EXPIRED`, bản nháp đầu ra/nguồn gốc | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Người yêu cầu/thành viên tổ chức được quyền.
...
- Endpoint input contract: → tác vụ `QUEUED`/`RUNNING`/`SUCCEEDED`/`FAILED`/`CANCELLED`; khi `SUCCEEDED` có duyệt đầu ra `DRAFT`/`ACCEPTED`/`EDITED_ACCEPT`/`REJECTED`/`EXPIRED`, bản nháp đầu ra/nguồn gốc.

<a id="s04"></a>
### S04. Execute source-defined processing

> Điều kiện chủ sở hữu/tổ chức; thực thi tác vụ và duyệt của con người là hai máy trạng thái tách biệt; hiển thị đầu ra như văn bản không tin cậy, không thực thi HTML

- Operations/observability contract: Riêng tư, không lưu; thăm dò 1/2 giây
