...
format: "markdown"
dd_id: "API-AIX-006"
api_name: "POST /api/v1/ai-jobs/{aiJobId}/reviews"
...
| `aiJobId` | Endpoint URI | S01 | Path parsing | Resource lookup | N/A | N/A | N/A |
| Quyết định //, đầu ra/lý do đã sửa | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | lượt duyệt | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Người được quyền là con người.
...
- Endpoint input contract: Quyết định `ACCEPTED`/`EDITED_ACCEPT`/`REJECTED`, đầu ra/lý do đã sửa, `If-Match` → lượt duyệt.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX thêm lượt duyệt; gợi ý CV/JD được chấp thuận vẫn cần lưu riêng qua trình biên tập; chấp thuận danh sách ngắn không chuyển ATS

- Operations/observability contract: Khóa lặp; kiểm toán bất biến; sự kiện chỉ số chất lượng
