...
format: "markdown"
dd_id: "API-AIX-004"
api_name: "POST /api/v1/enterprises/{enterpriseId}/jobs/{jobId}/ai-shortlist-jobs"
...
| ID đơn ứng tuyển nêu rõ <=100, phiên bản tiêu chí | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | gợi ý bất đồng bộ | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: `PERM-WRK-071` + người duyệt là con người.
...
- Endpoint input contract: ID đơn ứng tuyển nêu rõ <=100, phiên bản tiêu chí → gợi ý bất đồng bộ.

<a id="s04"></a>
### S04. Execute source-defined processing

> Chụp trường cho phép; chỉ trả dải xếp hạng/lý do/độ không chắc chắn. Không bao giờ ghi trạng thái đơn, từ chối, đề nghị hoặc tuyển dụng

- Operations/observability contract: Khóa lặp; kiểm toán tập đã chọn/trường loại trừ
