...
format: "markdown"
dd_id: "API-IAM-008"
api_name: "POST /api/v1/auth/logout-all"
...
| Không có phần thân | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | số phiên bị thu hồi | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Đã xác thực + mật khẩu/MFA gần đây.
...
- Endpoint input contract: Không có phần thân → số phiên bị thu hồi.

<a id="s04"></a>
### S04. Execute source-defined processing

> TX L người dùng; tăng `authVersion`, thu hồi các phiên

- Operations/observability contract: Xử lý lặp an toàn; sự kiện `identity.sessions.revoked`; kiểm toán
