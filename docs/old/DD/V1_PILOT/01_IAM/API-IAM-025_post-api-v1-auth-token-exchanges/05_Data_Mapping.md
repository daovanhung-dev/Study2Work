...
format: "markdown"
dd_id: "API-IAM-025"
api_name: "POST /api/v1/auth/token-exchanges"
...
| `audience` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| `study-evidence` | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Physical key direct; location/type not locked. |
| phạm vi thuộc danh sách cho phép, nonce | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | JWT 5 phút | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Đã xác thực và đang hoạt động.
...
- Endpoint input contract: `audience=study-evidence`, phạm vi thuộc danh sách cho phép, nonce → JWT 5 phút.

<a id="s04"></a>
### S04. Execute source-defined processing

> Kiểm ứng dụng khách Work đã đăng ký, phiên hiện hành/chính sách `authVersion`/MFA; không gọi Study; ký mã có `act`, `aud`, `scope`, `nonce`

- Operations/observability contract: Khóa lặp; kiểm toán bảo mật; 20/phút
