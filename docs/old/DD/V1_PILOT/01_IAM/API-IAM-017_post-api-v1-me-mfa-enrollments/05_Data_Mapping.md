...
format: "markdown"
dd_id: "API-IAM-017"
api_name: "POST /api/v1/me/mfa/enrollments"
...
| Phương thức ; nhãn thiết bị | API catalog | S01/S03 | SOURCE_REQUIRED | SOURCE_REQUIRED | N/A | N/A | Not materialized as a physical key. |
...
| `data` | `object \| array \| null` | Endpoint output phrase | N/A | S06 | bí mật và tải tin QR một lần, mã khôi phục một lần | SOURCE_REQUIRED for nested shape | API catalog |
...
- Actor/authorization source: Người dùng đặc quyền + xác minh tăng cường.
...
- Endpoint input contract: Phương thức `TOTP`; nhãn thiết bị → bí mật và tải tin QR một lần, mã khôi phục một lần.

<a id="s04"></a>
### S04. Execute source-defined processing

> Mã hóa bí mật chờ xác minh bằng phong bì KMS; tạo đăng ký nhưng chưa kích hoạt đến khi xác minh

- Operations/observability contract: Khóa lặp; `no-store`; kiểm toán
