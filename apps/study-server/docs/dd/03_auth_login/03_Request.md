...
|---:|---|---|---:|---|---|---|
| 1 | Contents type | `Content-Type` | `Yes` | `application/json` | Request body dùng JSON | [`1.1`](./05_Data_Mapping.md#11-get-request-header) |
| 2 | Authorization | `Authorization` | `No` | `N/A — public endpoint` | Không yêu cầu Bearer token | [`1.1`](./05_Data_Mapping.md#11-get-request-header) |
...
|---:|---|---|---|---:|---|---|---|---|---|---|---|---|
| 1 | Email | `email` | `email` | `Yes` | `N/A — chưa đặc tả` | `N/A — chưa đặc tả` | `text` | `email` | `N/A` | `N/A` | Định danh đăng nhập dùng để lookup `users` | [`2.1`](./05_Data_Mapping.md#21-validate-email) |
| 2 | Password | `password` | `string` | `Yes` | `N/A — chưa đặc tả` | `N/A — chưa đặc tả` | `text` | `N/A — policy TBD` | `N/A` | `N/A` | Plaintext credential chỉ dùng để verify, không persist/response | [`2.2`](./05_Data_Mapping.md#22-validate-password) |

> Mỗi field phải nằm trên một row riêng. Contract không đặc tả min/max hoặc password character policy.
