...
|---:|---|---|---:|---|---|---|
| 1 | Authorization | `Authorization` | `No` | `N/A — public endpoint` | Không yêu cầu Bearer token | [`2.1`](./05_Data_Mapping.md#21-get-request-header) |
| 2 | Contents type | `Content-Type` | `No` | `N/A — request không có body` | GET không có request body theo contract | [`2.1`](./05_Data_Mapping.md#21-get-request-header) |
...
> Mỗi field phải nằm trên một row riêng. Không thêm `page`, `size`, `sort` hoặc field body ngoài contract V1.

## Ví dụ Request

```http
GET /api/v1/categories?locale=vi-VN HTTP/1.1
