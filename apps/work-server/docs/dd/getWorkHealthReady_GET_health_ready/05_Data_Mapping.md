...
| ---: | --- | --- | --- | --- | --- | --- | --- |
| 1 | probeDatabase | database dependency | SELECT 1 | N/A | N/A | N/A | N/A |
...
| ---: | --- | --- | --- | --- |
| 1 | service | data field | Static service identifier. | src/app.ts ready route and core/database.ts |
| 2 | environment | data field | config.appEnv. | src/app.ts ready route and core/database.ts |
| 3 | dependencies.database | data field | configured after probeDatabase succeeds. | src/app.ts ready route and core/database.ts |
| 4 | dependencies.redis | data field | configured when REDIS_URL exists; otherwise not_configured. | src/app.ts ready route and core/database.ts |

## Validation and branch rules

- await probeDatabase(dependencies.prisma).
- Catch probe failure and throw ApiError(503, DEPENDENCY_UNAVAILABLE).
- On success return configured database and Redis display state.
