...
- Owner query phải tìm thấy `JD.id = jobId` và `JD.doanhnghiep_id = user.id`.
...
- Gọi `JDService.updateJD(jobId, data)`.
- Prisma update theo `JD.id = jobId`.
- Không có transaction explicit trong source.

## 6. Map response và error

- `data` là record `JD` sau update.
- `reply` trả HTTP `200` và business code `JOB_UPDATED`.
- Thành công: `reply` trả HTTP `200`, `businessCode = JOB_UPDATED`, `data` theo [04_Response.md](./04_Response.md).
...
- DB mapping: [07_JD_update.md](./07_JD_update.md).
