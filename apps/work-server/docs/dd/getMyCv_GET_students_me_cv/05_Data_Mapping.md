...
- Gọi `CVService.getCvById(user.id)`.
- Prisma đọc `Cv.findFirst({ where: { sinhvien_id: BigInt(user.id) } })`.
...
- `data` là record `Cv`.
- BigInt được serialize thành number; Date được serialize thành ISO string.
