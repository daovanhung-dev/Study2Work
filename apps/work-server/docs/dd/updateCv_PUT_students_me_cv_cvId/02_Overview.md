...
| Purpose | Partially update an owned CV. |
| Consumer/Actor | Authenticated student |
| Authentication | Yes |
| Authorization | Valid Bearer JWT with role=student and owned CV |
| Basis | DIRECT — current registered route |
| Status | Draft — Ready for Review |
| Transaction | Prisma $transaction checks ownership then updates Cv. |
| Side effects | Updates only normalized whitelist fields for owned CV. |
...
- Ownership is checked by Cv.id and sinhvien_id in the transaction.
