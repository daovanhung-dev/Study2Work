...
| businessCode | string | `BUSINESS_APPLICATIONS_LOADED` | Business code do view/system route trả về. | successResponse |
| message | string | Đã tải danh sách ứng viên. | Message source-confirmed. | successResponse |
...
| data[].SinhVien | string \| null | source value | UngVien field or included relation. | applications query include |
...
```json
{
  ...
  "message": "Đã tải danh sách ứng viên.",
  "data": [
    {
      "id": 1,
