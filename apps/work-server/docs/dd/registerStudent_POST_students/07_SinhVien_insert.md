---
title: "Định nghĩa table"
order: 7
dd_id: "registerStudent"
api_name: "students.view.registerStudent"
source_workbook: "DD_API_Template(1).xlsx"
source_sheet: "table"
status: "Draft — Ready for Review"
---
# Định nghĩa table

## Table metadata

| Thuộc tính | Giá trị |
| ---: | --- |
| Prisma model | SinhVien |
| Operation | INSERT |
| Transaction | No explicit transaction |
| Status | Draft — Ready for Review |

## Field mapping

| No | DB column | Operation | Value source | Prisma/schema type |
| ---: | --- | --- | --- | --- |
| 1 | hoten | INSERT | input.hoten | VARCHAR(50) |
| 2 | email | INSERT | input.email | VARCHAR(50) UNIQUE |
| 3 | matkhau | INSERT | hashPassword(input.matkhau) | VARCHAR(255) |
| 4 | chuyennganh | INSERT | input.chuyennganh ?? '' | VARCHAR(50) |
| 5 | avt | INSERT | uploaded filename or null | VARCHAR(191) |

## Insert/Update/Delete behavior

- Public projection excludes matkhau.
- No migration is introduced.


---
## Phụ lục đối chiếu nguồn Excel
- Workbook nguồn: `DD_API_Template(1).xlsx`
- Sheet nguồn: `table`
- Dimension: `A1:BA35`
- Trạng thái: `visible`
- Số ô có dữ liệu/công thức: `19`
- Số vùng merge: `5`

<details>
<summary>Danh sách vùng merge</summary>

- `W34:BA34`
- `W14:BA14`
- `W15:BA15`
- `W19:BA19`
- `W32:BA32`

</details>

<details>
<summary>Bản ghi từng ô có dữ liệu hoặc công thức</summary>

| Hàng | Ô | Giá trị nguồn | Công thức nguồn |
|---:|---|---|---|
| 2 | `A2` | № |  |
| 2 | `B2` | Tên table |  |
| 3 | `B3` | SB |  |
| 3 | `S3` | Độ dài |  |
| 3 | `T3` | Dấu phẩy thập phân |  |
| 3 | `U3` | Bắt buộc |  |
| 3 | `V3` | Main key |  |
| 3 | `W3` | Nội dung setting |  |
| 4 | `B4` | № |  |
| 4 | `C4` | Item ID |  |
| 4 | `I4` | Item name |  |
| 4 | `O4` | Kiểu |  |
| 5 | `A5` | 1 |  |
| 5 | `B5` | table id |  |
| 5 | `I5` | table name |  |
| 6 | `B6` | Update |  |
| 6 | `I6` | Trường hợp số record get được từ xử lý 3. của sheet [３．Data mapping]  > 0 |  |
| 21 | `B21` | Insert |  |
| 21 | `I21` | Trường hợp số record get được từ xử lý 3. của sheet [３．Data mapping]  = 0 |  |

</details>
